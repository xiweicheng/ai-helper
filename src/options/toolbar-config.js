...
export function renderToolbarToolsList(tools) {
  ...
  const sorted = [...tools].filter(item => item.id !== 'ai-search' && item.id !== 'copy').sort((a, b) => a.order - b.order);
  ...
  listEl.innerHTML = sorted.map((tool, index) => {
    ...
    let actionsHtml = '';
    if (!isBuiltin) {
      actionsHtml += `<button class="tool-action-btn" data-action="edit" data-index="${index}">${t('common.edit')}</button>`;
      actionsHtml += `<button class="tool-action-btn danger" data-action="delete" data-index="${index}">${t('common.delete')}</button>`;
    }

    return `
      <div class="tool-item" data-tool-id="${tool.id}" draggable="true" data-sorted-index="${index}">
        <div class="tool-drag-handle" title="${t('toolbar.dragSort')}">⋮⋮</div>
        <div class="tool-order-btns">
          <button class="tool-order-btn" data-action="moveUp" data-index="${index}" ${isFirst ? 'disabled' : ''} title="${t('toolbar.moveUp')}">▲</button>
  ...
  items.forEach(item => {
    ...
    item.addEventListener('drop', (e) => {
      ...
      chrome.storage.local.get(['toolbarTools'], (result) => {
        ...
        // 合并回完整列表
        const fixedTools = tools.filter(item => item.id === 'ai-search' || item.id === 'copy');
...
export function moveTool(tools, index, direction) {
  ...
  const fixedTools = tools.filter(item => item.id === 'ai-search' || item.id === 'copy');
  const nonFixedTools = [...tools].filter(item => item.id !== 'ai-search' && item.id !== 'copy').sort((a, b) => a.order - b.order);
...
export function openToolEditModal(tools, index = -1) {
  ...
  if (index >= 0) {
    const sorted = [...tools].filter(item => item.id !== 'ai-search' && item.id !== 'copy').sort((a, b) => a.order - b.order);
    const tool = sorted[index];
    title.textContent = t('toolbar.editCustomToolTitle');
    nameInput.value = tool.name;
    promptInput.value = tool.systemPrompt || '';
  } else {
    title.textContent = t('toolbar.toolEditModalTitle');
    nameInput.value = '';
    promptInput.value = '';
  }
...
export function saveToolEdit(tools) {
  ...
  if (!name) return { error: t('toolbar.toolNameEmpty') };
  if (!prompt) return { error: t('toolbar.toolPromptEmpty') };

  const newTools = [...tools].sort((a, b) => a.order - b.order);

  if (editingToolIndex >= 0) {
    // 编辑时索引基于过滤后的列表（不含AI搜索和复制），先过滤再查找
    const filtered = newTools.filter(item => item.id !== 'ai-search' && item.id !== 'copy');
    const tool = filtered[editingToolIndex];
    if (!tool) return { error: t('toolbar.toolNotFound') };
    if (tool.builtin) return { error: t('toolbar.cannotEditBuiltin') };
    tool.name = name;
...
function renderBlockedDomainsList(list) {
  ...
  if (list.length === 0) {
    listEl.innerHTML = `<div class="domain-blocklist-empty">${t('toolbar.noBlockedDomains')}</div>`;
  } else {
    listEl.innerHTML = list.map(domain => `
      <div class="domain-blocklist-item">
        ...
        <button class="domain-blocklist-item-remove" data-domain="${escapeHtml(domain)}" title="${t('toolbar.unblock')}">
...
export function addBlockedDomain(domain, callback) {
  ...
  if (!domain) {
    if (callback) callback({ error: t('toolbar.domainEmpty') });
    return;
  }
  if (!domain.includes('.')) {
    if (callback) callback({ error: t('toolbar.domainInvalid') });
    return;
  }
  ...
  chrome.storage.local.get(['blockedDomains'], (result) => {
    ...
    if (list.includes(domain)) {
      if (callback) callback({ error: t('toolbar.domainExists') });
      return;
    }
    list.push(domain);
