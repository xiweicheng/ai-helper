...
当用户要求更新/完善已有技能时：
1. 确认技能名称
2. 直接读取 ~/.ai-helper-agent/skills/<skill-name>/SKILL.md
3. 分析对话上下文，识别需要更新的部分（新增触发条件、修改步骤、添加示例等）
4. 合并内容，保留核心结构，融入新内容
5. 更新 frontmatter 中的 description（如有必要）
6. 覆盖写入 SKILL.md
7. 告知用户更新完成，建议重新加载
...
1. **确认技能名称**：与用户确认要更新的技能名称（如 "code-reviewer"）

2. **读取现有内容**：直接读取 ~/.ai-helper-agent/skills/<skill-name>/SKILL.md 文件
...
3. **分析更新需求**：结合当前对话上下文，识别需要更新的部分：
   ...
   - 添加新示例
   - 修复错误或改进流程
   - 更新 description 使其更准确

4. **合并内容**：保留原技能的核心结构，将新内容融入适当位置：
   - 将新的触发条件追加到 "When to Use This Skill" 部分
   - 将改进的步骤更新到 "Usage" 部分
   - 将新示例添加到 "Examples" 部分
   - 如果流程有重大变化，重新组织整个技能结构

5. **更新 description**：如果触发条件或核心能力有变化，更新 frontmatter 中的 description

6. **直接写入文件**：覆盖写入 ~/.ai-helper-agent/skills/<skill-name>/SKILL.md

7. **告知用户**：告知用户技能已更新，说明更新了哪些内容，并建议点击「重新加载」使更新生效

## 更新注意事项

- 保留原技能的名称（frontmatter 中的 name 字段不变）
- 保留原技能的核心结构，只在必要时重构
- 更新后技能的触发条件应比以前更完善，不应遗漏原有条件
- 每次更新都应使技能质量提升，而不是倒退
