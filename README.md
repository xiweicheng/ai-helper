...
> 基于大语言模型（LLM）的 Chrome 浏览器智能助手扩展。采用 ReAct（Reasoning + Acting）推理循环架构，支持自然语言对话、浏览器自动化操作、网页内容处理等 **54 项内建工具 + MCP 动态扩展**。可搭配本地代理服务实现文件系统操作、终端命令执行、Skill 技能系统和 MCP 协议扩展，同时具备多模态文件问答、图片识别与标注、长期记忆系统、会话导入/导出等高级能力。
...
- **工具预筛选**：54+ 个工具定义会消耗大量 Token，AI Helper 在每次调用主力模型前用一次**轻量 API 预判**，将工具缩减为 5-10 个相关项，大幅节省成本。
...
| 多模态输入 | 图片识别（Vision API）+ 文件提取（PDF/Word/Excel） |
...
│   │   ├── config.js                    # 配置读写
│   │   ├── constants.js                # 默认配置、54 个内建工具定义、分类映射
...
2. **工具预筛选**：正式调用主力模型前，用一次轻量 API 调用判断需要哪些工具，将 54+ 个工具缩减为 5-10 个相关工具，大幅减少 Token 消耗。简单问题可直接回答跳过推理循环
