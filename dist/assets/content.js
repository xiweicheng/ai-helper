...
`).map(e=>e.replace(/^[\d]+[\.\、\s]+/,``).trim()).filter(e=>e.length>0).slice(0,3));let a=typeof marked<`u`?marked.parse(n):escapeHtml(n).replace(/\n/g,`<br>`);showResultPanel(lastPanelPos.x,lastPanelPos.y,a,r)}});function blockCurrentDomain(){let e=window.location.hostname;chrome.storage.local.get([`blockedDomains`],t=>{let n=t.blockedDomains||[];n.includes(e)||(n.push(e),chrome.storage.local.set({blockedDomains:n},()=>{blockedDomains=n,hideToolbar(),hideResultPanel(),currentSelectedText=``}))})}function loadToggleState(){isExtensionValid()&&chrome.storage.local.get([`enableSelectionToolbar`,`blockedDomains`],e=>{enableSelectionToolbar=e.enableSelectionToolbar===void 0?!0:!!e.enableSelectionToolbar,blockedDomains=e.blockedDomains||[],console.log(`[SelectionToolbar] 开关状态:`,enableSelectionToolbar?`已启用`:`已禁用`,`屏蔽域名:`,blockedDomains.length)})}chrome.storage.onChanged.addListener((e,t)=>{isExtensionValid()&&(t===`local`&&e.enableSelectionToolbar&&(enableSelectionToolbar=!!e.enableSelectionToolbar.newValue,enableSelectionToolbar||(hideToolbar(),hideResultPanel(),currentSelectedText=``)),t===`local`&&e.blockedDomains&&(blockedDomains=e.blockedDomains.newValue||[]),t===`local`&&(e.toolbarTools||e.toolbarMaxVisible)&&refreshToolbarCache())});function initSelectionToolbar(){injectStyles(),createToolbar(),createResultPanel(),loadToggleState(),document.addEventListener(`selectionchange`,onSelectionChange),document.addEventListener(`click`,onDocumentClick,!0),document.addEventListener(`mouseup`,onMouseUp,!0),window.addEventListener(`scroll`,onScrollOrResize,!0),window.addEventListener(`resize`,onResize),console.log(`[SelectionToolbar] 初始化完成`)}document.addEventListener(`keydown`,e=>{(e.ctrlKey||e.metaKey)&&e.shiftKey&&e.key===`A`&&(e.preventDefault(),chrome.action.click())});var HANDLERS={GET_PAGE_TEXT:e=>getPageText(e),GET_FULL_HTML:e=>getFullHtml(e),QUERY_INTERACTIVE_ELEMENTS:e=>queryInteractiveElements(e),GET_SELECTED_CONTENT:e=>getSelectedContent(e.format),CLICK_ELEMENT:e=>clickElement(e.selector,e.waitTime,e.timeout),FILL_FORM:e=>fillForm(e.fields,e.waitTime),SCROLL_TO:e=>scrollToPosition(e),HOVER_ELEMENT:e=>hoverElement(e.selector),KEYBOARD_INPUT:e=>keyboardInput(e),FILE_UPLOAD:e=>fileUpload(e.selector,e.fileName,e.fileContent,e.fileType),EXTRACT_TABLE:e=>extractTable(e.selector,e.includeHeaders,e.format),EXTRACT_METADATA:()=>extractMetadata(),EXTRACT_LINKS:e=>extractLinks(e.filterType,e.includeImages),EXTRACT_FORMS:e=>extractForms(e.formSelector),EXTRACT_IMAGES:e=>extractImages(e),SEARCH_IN_PAGE:e=>searchInPage(e),PAGE_TO_MARKDOWN:e=>pageToMarkdown(e.selector,e.includeImages,e.includeLinks,e.maxLength),PAGE_TO_JSON:e=>pageToJson(e.selector,e.maxItems),FIND_SIMILAR_ELEMENTS:e=>findSimilarElements(e.selector,e.maxResults),GET_IFRAME_CONTENT:e=>getIframeContent(e.selector,e.includeNested,e.maxLength),READ_ACCESSIBILITY_TREE:e=>readAccessibilityTree(e.maxResults),HIGHLIGHT_TEXT:e=>highlightText(e.text,e.color),REMOVE_HIGHLIGHTS:()=>removeHighlights(),GET_ELEMENT_RECT:e=>getElementRect(e.selector),DIFF_PAGE:e=>diffPage(e.action,e.snapshotName),SHADOW_DOM_QUERY:e=>shadowDomQuery(e.selector,e.deep,e.maxDepth,e.maxResults),MANAGE_STORAGE:e=>manageStorage(e),TEXT_TO_SPEECH:e=>textToSpeech(e.text,e.lang,e.rate,e.pitch),INJECT_CSS:e=>injectCss(e.css,e.targetSelector,e.injectMode),PERFORMANCE_AUDIT:e=>performanceAudit(e.includeResourceTiming,e.includePaintTiming,e.includeMemoryInfo),VIDEO_CONTROL:e=>videoControl(e.action,e.selector,e.value),COPY_TO_CLIPBOARD:e=>copyToClipboard$1(e.text),PASTE_FROM_CLIPBOARD:()=>pasteFromClipboard(),WAIT_FOR_ELEMENT:e=>waitForElement(e.selector,e.state,e.timeout),DRAG_AND_DROP:e=>dragAndDrop(e.sourceSelector,e.targetSelector),WATCH_ELEMENT:e=>watchElement(e.selector,e.duration),COLOR_PICKER:()=>pickColor(),GENERATE_QRCODE:e=>generateQRCode(e.content,e.size,e.errorCorrection,e.showImage),SCREENSHOT_ELEMENT:e=>screenshotElement(e.selector,e.quality,e.format),PAGE_TO_PDF:e=>pageToPdf(e.fileName,e.landscape,e.scale,e.printBackground,e.margins),RUN_JAVASCRIPT:e=>runJavascript(e.code,e.timeout),CLEAR_PAGE_DATA:e=>{try{let t=[];return e.site?window.location.href.includes(new URL(e.site).hostname)&&(localStorage.clear(),sessionStorage.clear(),t.push(`localStorage`,`sessionStorage`)):(localStorage.clear(),sessionStorage.clear(),t.push(`localStorage`,`sessionStorage`)),{success:!0,cleared:t}}catch(e){return{success:!1,error:e.message}}}},ASYNC_HANDLERS=new Set([`COPY_TO_CLIPBOARD`,`PASTE_FROM_CLIPBOARD`,`WAIT_FOR_ELEMENT`,`DRAG_AND_DROP`,`WATCH_ELEMENT`,`COLOR_PICKER`,`GENERATE_QRCODE`,`SCREENSHOT_ELEMENT`,`PAGE_TO_PDF`,`RUN_JAVASCRIPT`]);chrome.runtime.onMessage.addListener((e,t,n)=>{if(e.action===`getSelectedText`){n({text:window.getSelection()?.toString()||``});return}let r=HANDLERS[e.type];if(!r)return;let i=r(e);if(ASYNC_HANDLERS.has(e.type)||i instanceof Promise)return Promise.resolve(i).then(n),!0;n(i)}),initSelectionToolbar();
//# sourceMappingURL=content-BN-NXeB1.js.map