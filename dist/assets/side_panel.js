...
... (点击展开查看完整输出)`:a,s=a,c=document.createElement(`div`);if(c.className=`tool-call-result`,c.innerHTML=`
  ...
  `,n.appendChild(c),s.length>500){let e=c.querySelector(`code`),t=!1;c.style.cursor=`pointer`,c.addEventListener(`click`,()=>{t=!t,e.textContent=t?s:o})}requestAnimationFrame(()=>{let e=document.getElementById(`chatContainer`);e&&(e.scrollTop=e.scrollHeight)})}function yi(e,t,n=[],r=null,i=null){if(!e)return;e.querySelectorAll(`.thinking-indicator`).forEach(e=>e.classList.add(`hidden`));let a=e.querySelector(`.stream-status`);a&&a.remove(),e.classList.remove(`streaming`);let o=e.querySelector(`.message-content`),s=e.querySelector(`.stream-content`);if(o&&o.querySelector(`.tool-call-item`)||s&&s.querySelector(`.tool-call-item`)){let e=Qr>0?((Date.now()-Qr)/1e3).toFixed(1)+`s`:``,n=document.createElement(`div`);n.className=`thinking-process collapsed`;let r=document.createElement(`div`);r.className=`thinking-process-header`,r.innerHTML=`
...
//# sourceMappingURL=side_panel-Dk7YuFHy.js.map