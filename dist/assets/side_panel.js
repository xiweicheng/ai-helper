...
`).trim()}}async function Ve(){chrome.storage.local.get([`chatHistory`,`scrollPosition`],e=>{if(e.chatHistory&&e.chatHistory.length>0){F.messageHistory=e.chatHistory,F.messageHistory.forEach(e=>{K(e.role,e.content,!1,e.executionLog||[])});let t=document.querySelector(`.welcome-message`);t&&t.remove(),je(),e.scrollPosition!==void 0&&setTimeout(()=>{let t=document.getElementById(`chatContainer`);t.scrollTop=e.scrollPosition},100)}})}function U(){let e=F.messageHistory.slice(-F.chatConfig.maxHistoryMessages).map(e=>({role:e.role,content:e.content.substring(0,F.chatConfig.maxMessageLength),executionLog:e.executionLog||[]}));try{chrome.storage.local.set({chatHistory:e},t=>{if(chrome.runtime.lastError){if(console.error(`[SidePanel] 保存对话历史失败:`,chrome.runtime.lastError.message),F.messageHistory.length>5){F.messageHistory=F.messageHistory.slice(-5);let e=F.messageHistory.map(e=>({role:e.role,content:e.content.substring(0,2e3),executionLog:e.executionLog||[]}));chrome.storage.local.set({chatHistory:e},()=>{chrome.runtime.lastError?console.error(`[SidePanel] 再次保存失败:`,chrome.runtime.lastError.message):console.log(`[SidePanel] 截断后保存成功，消息数:`,e.length)})}}else console.log(`[SidePanel] 对话历史已保存，消息数:`,e.length)})}catch(e){console.error(`[SidePanel] 保存对话历史异常:`,e)}}function He(){F.messageHistory=[],chrome.storage.local.remove([`chatHistory`,`scrollPosition`],()=>{let e=document.getElementById(`chatContainer`);e.innerHTML=`
  ...
  `,e.appendChild(n),e.scrollTop=e.scrollHeight;let r=n.querySelector(`.stop-task-btn`),i=n.querySelector(`.loading-text`);r&&r.addEventListener(`click`,e=>{e.stopPropagation(),r.disabled=!0,r.style.opacity=`0.6`,r.style.cursor=`not-allowed`,i&&(i.textContent=`停止中...`),chrome.runtime.sendMessage({type:`CANCEL_REACT`,tabId:null})}),F.executionLogListener=(e,n,r)=>e.type===`EXECUTION_STATUS_UPDATE`?(console.log(`[SidePanel] 收到执行状态更新:`,e.nodeName,e.status,`日志数量:`,e.executionLog?.length),et(t,e.nodeName,e.status,e.executionLog),!1):!1,chrome.runtime.onMessage.addListener(F.executionLogListener),chrome.storage.local.get(`enableExecutionLog`,e=>{if(e.enableExecutionLog){let e=n.querySelector(`.execution-status-container`);e&&(e.style.display=`flex`)}});let a=n.querySelector(`.execution-log-toggle-btn`);return a&&a.addEventListener(`click`,e=>{e.stopPropagation(),$e(t)}),t}function q(e){let t=document.getElementById(e);if(t){let e=t.querySelector(`.loading-text`);e&&(e.textContent=`思考中...`),t.remove()}F.executionLogListener&&=(chrome.runtime.onMessage.removeListener(F.executionLogListener),null),F.currentExecutionStatus=null;let n=document.querySelector(`.realtime-execution-log-panel`);n&&n.remove()}async function nt(e,t,n=!1,r={}){let i=(await ue()).loopTimeout;return new Promise((a,o)=>{let s=[],c=Math.round(i/1e3),l=setTimeout(()=>{g(),chrome.runtime.sendMessage({type:`CANCEL_REACT`,tabId:F.currentTabId}).catch(e=>{console.log(`[SidePanel] 发送取消请求失败:`,e.message)}),o({message:`请求超时（${c}秒）`,executionLog:s})},i),u=Date.now(),d=0,f=null,p=()=>{f===null&&l!==null&&(f=Date.now(),clearTimeout(l),l=null,console.log(`[SidePanel] 前端超时已暂停（澄清工具执行中）`))},m=()=>{if(f!==null){let e=Date.now()-f;d+=e,f=null;let t=Date.now()-u,n=i+d-t;if(n<=0){g(),o({message:`请求超时（${c}秒）`,executionLog:s});return}l=setTimeout(()=>{g(),chrome.runtime.sendMessage({type:`CANCEL_REACT`,tabId:F.currentTabId}).catch(e=>{console.log(`[SidePanel] 发送取消请求失败:`,e.message)}),o({message:`请求超时（${c}秒）`,executionLog:s})},n),console.log(`[SidePanel] 前端超时已恢复，暂停时长:`,Math.round(e/1e3),`s，剩余时间:`,Math.round(n/1e3),`s`)}},h=e=>(console.log(`[SidePanel] 收到消息:`,e),e.type===`EXECUTION_STATUS_UPDATE`?(s=e.executionLog||[],!1):e.type===`CLARIFY_START`?(p(),!1):e.type===`CLARIFY_END`?(m(),!1):e.type===`API_COMPLETE`?(l&&clearTimeout(l),chrome.runtime.onMessage.removeListener(h),a({content:e.content,executionLog:e.executionLog||s}),!1):e.type===`API_ERROR`?(l&&clearTimeout(l),chrome.runtime.onMessage.removeListener(h),o({message:e.error,executionLog:e.executionLog||s}),!1):!1);chrome.runtime.onMessage.addListener(h);let g=()=>{chrome.runtime.onMessage.removeListener(h)};console.log(`[SidePanel] 发送 CALL_API 消息，useTools:`,n,`tabId:`,F.currentTabId,`apiParams:`,r,`timeout:`,i),chrome.runtime.sendMessage({type:`CALL_API`,messages:e,model:t,useTools:n,tabId:F.currentTabId,apiParams:r})})}function rt(e){let t=document.querySelector(`.execution-log-panel`);t&&t.remove();let n=document.createElement(`div`);n.className=`execution-log-panel`;let r=e.reduce((e,t)=>e+(t.duration||0),0),i=e.filter(e=>e.status===`success`).length,a=e.filter(e=>e.status===`failed`).length,o=e.filter(e=>e.nodeType===`subtask`).length,s=e.filter(e=>e.nodeType===`subtask`&&e.status===`success`).length;n.innerHTML=`
  ...
  `,document.body.appendChild(n),n.addEventListener(`click`,e=>{e.target===n&&n.remove()}),n.querySelector(`.log-close`).addEventListener(`click`,()=>{n.remove()});let c=n.querySelector(`.toggle-expand-btn`),l=n.querySelectorAll(`.timeline-content`),u=!1;c.addEventListener(`click`,()=>{u=!u,l.forEach(e=>{u?e.classList.add(`expanded`):e.classList.remove(`expanded`)});let e=c.querySelector(`svg`);u?(e.innerHTML=`<polyline points="17 11 12 6 7 11"></polyline><polyline points="17 18 12 13 7 18"></polyline>`,c.setAttribute(`title`,`收起全部节点`)):(e.innerHTML=`<polyline points="7 13 12 18 17 13"></polyline><polyline points="7 6 12 11 17 6"></polyline>`,c.setAttribute(`title`,`展开全部节点`))}),n.querySelectorAll(`.timeline-header`).forEach(e=>{e.addEventListener(`click`,()=>{e.parentElement.classList.toggle(`expanded`)})});let d=n.querySelectorAll(`.combo-stat`),f=n.querySelectorAll(`.timeline-item`);d.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.status,n=e.classList.contains(`active`);d.forEach(e=>e.classList.remove(`active`)),n?f.forEach(e=>{e.style.display=``}):(e.classList.add(`active`),f.forEach(e=>{if(t===`subtask`)e.classList.contains(`subtask-level`)?e.style.display=``:e.style.display=`none`;else{let n=e.querySelector(`.timeline-dot`);n&&n.classList.contains(t)?e.style.display=``:e.style.display=`none`}}))})})}function it(e,t){try{let n=e.dataset.rawContent||``;navigator.clipboard.writeText(n).then(()=>{let e=t.innerHTML;t.innerHTML=`
      ...
      `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}).catch(e=>{console.error(`[SidePanel] 复制失败:`,e);let r=document.createElement(`textarea`);r.value=n,r.style.position=`fixed`,r.style.left=`-999999px`,document.body.appendChild(r),r.select();try{document.execCommand(`copy`);let e=t.innerHTML;t.innerHTML=`
        ...
        `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}catch{L(`复制失败`,`error`)}document.body.removeChild(r)})}catch(e){console.error(`[SidePanel] 复制失败:`,e),L(`复制失败`,`error`)}}function at(e){try{let t=e.dataset.rawContent||``;if(!t){L(`无法获取消息内容`,`error`);return}let n=document.getElementById(`userInput`);n.value=t,re(),n.focus(),n.selectionStart=n.selectionEnd=n.value.length,console.log(`[SidePanel] 已加载消息内容到输入框，等待用户编辑后发送`)}catch(e){console.error(`[SidePanel] 编辑消息失败:`,e),L(`编辑失败: `+e.message,`error`)}}function ot(e,t){try{let n=e.dataset.rawMarkdown||e.dataset.rawContent||``;if(!n){let t=F.messageHistory.find(e=>e.role===`assistant`);if(t)n=t.content;else{let t=e.querySelector(`.markdown-body`);n=t?t.innerText:e.innerText}}navigator.clipboard.writeText(n).then(()=>{let e=t.innerHTML;t.innerHTML=`
        <svg viewBox="0 0 16 16" fill="currentColor">
          <path d="M13.78 4.22a.75.75 0 0 1 0 1.06l-7.25 7.25a.75.75 0 0 1-1.06 0L2.22 9.28a.75.75 0 0 1 1.06-1.06L6 10.94l6.72-6.72a.75.75 0 0 1 1.06 0z"/>
        </svg>
        <span>已复制</span>
      `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}).catch(e=>{let r=document.createElement(`textarea`);r.value=n,r.style.position=`fixed`,r.style.left=`-999999px`,document.body.appendChild(r),r.select();try{document.execCommand(`copy`);let e=t.innerHTML;t.innerHTML=`
          ...
          <span>已复制</span>
        `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}catch{L(`复制失败，请手动选择内容复制`,`error`)}document.body.removeChild(r)})}catch(e){console.error(`[SidePanel] 复制失败:`,e),L(`复制失败`,`error`)}}function st(e,t){try{let n=e.dataset.rawMarkdown||e.dataset.rawContent||``;if(!n){let t=e.querySelector(`.markdown-body`);n=t?t.innerText:e.innerText}let r=`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
    ...
    `,i=new Blob([`﻿`,r],{type:`application/msword`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`word-${new Date().getTime()}.doc`,document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(a);let s=t.innerHTML;t.innerHTML=`
      ...
      <span>已导出</span>
    `,setTimeout(()=>{t.innerHTML=s},2e3),console.log(`[SidePanel] Word 文档导出成功`)}catch(e){console.error(`[SidePanel] 导出 Word 失败:`,e),L(`导出失败: `+e.message,`error`)}}function ct(e,t){try{let n=e.dataset.rawMarkdown||e.dataset.rawContent||``;if(!n){let t=e.querySelector(`.markdown-body`);n=t?t.innerText:e.innerText}let r=`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
...
`),t.dispatchEvent(new Event(`input`)),console.log(`[SidePanel] 已移除输入框中的选中内容前缀`))}F.lastSelectedText=``,F.currentSelectionRange=null}function dt(){let e=document.getElementById(`promptManageList`).querySelectorAll(`.prompt-manage-item`);e.forEach(t=>{t.addEventListener(`dragstart`,e=>{F.draggedItemIndex=parseInt(t.dataset.index),t.classList.add(`dragging`),e.dataTransfer.effectAllowed=`move`,e.dataTransfer.setData(`text/plain`,F.draggedItemIndex)}),t.addEventListener(`dragend`,()=>{t.classList.remove(`dragging`),e.forEach(e=>e.classList.remove(`drag-over`)),F.draggedItemIndex=null}),t.addEventListener(`dragover`,e=>{e.preventDefault(),e.dataTransfer.dropEffect=`move`,t.classList.add(`drag-over`)}),t.addEventListener(`dragleave`,()=>{t.classList.remove(`drag-over`)}),t.addEventListener(`drop`,e=>{e.stopPropagation(),e.preventDefault();let n=parseInt(t.dataset.index);if(F.draggedItemIndex!==null&&F.draggedItemIndex!==n){let e=F.customPrompts[F.draggedItemIndex];F.customPrompts.splice(F.draggedItemIndex,1),F.customPrompts.splice(n,0,e),chrome.storage.local.set({customPrompts:F.customPrompts}),Y()}t.classList.remove(`drag-over`)})})}function ft(){let e=document.querySelector(`.input-toolbar-right`);if(!e)return;let t=document.createElement(`button`);t.className=`prompt-manage-btn`,t.title=`提示词管理`,t.innerHTML=`<svg t="1781177976746" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5076" width="16" height="16"><path d="M674.56 231.552l101.568 56.96-56.896-101.632 56.96-101.568-101.632 56.896-101.632-56.896 56.96 101.568-56.896 101.632 101.568-56.96zM186.944 629.76l-101.504-56.896 56.832 101.632-56.832 101.568 101.504-56.96 101.632 56.96-56.896-101.568 56.896-101.568-101.568 56.832zM85.44 85.312l56.832 101.568-56.832 101.632 101.504-56.96 101.632 56.96L231.68 186.88l56.896-101.568-101.568 56.896-101.568-56.896z m351.872 438.016l-99.2-99.136L424.32 337.984l99.072 99.264-86.08 86.144m-41.856-223.04L300.352 395.392a40.448 40.448 0 0 0 0 57.28l474.24 474.112a40.448 40.448 0 0 0 57.344 0l94.912-95.04a40.448 40.448 0 0 0 0-57.344L452.736 300.288a40.448 40.448 0 0 0-57.28 0z" p-id="5077" fill="#777"></path></svg>`,t.addEventListener(`click`,()=>{bt()}),e.appendChild(t)}function pt(e=``){let t=document.getElementById(`promptSelector`),n=document.getElementById(`promptDropdown`);t.style.display=`block`,n.classList.add(`show`),gt(e)}function J(){let e=document.getElementById(`promptSelector`),t=document.getElementById(`promptDropdown`);e.style.display=`none`,t.classList.remove(`show`),F.selectedPromptIndex=-1}function mt(){let e=document.getElementById(`promptSelector`),t=document.getElementById(`promptDropdown`),n=document.getElementById(`userInput`);e.style.display!==`none`&&t.classList.contains(`show`)?J():(pt(),n.focus())}function ht(e=``){gt(e)}function gt(e=``){let t=document.getElementById(`promptList`),n=e.toLowerCase(),r=F.customPrompts.filter(t=>e?t.code.toLowerCase().includes(n)||t.content.toLowerCase().includes(n):!0);if(r.length===0){t.innerHTML=`<div class="prompt-empty">暂无匹配的提示词</div>`,F.selectedPromptIndex=-1;return}F.selectedPromptIndex=0,t.innerHTML=r.map((e,t)=>`
  ...
  `).join(``),t.querySelectorAll(`.prompt-item`).forEach(e=>{e.addEventListener(`click`,t=>{let n=e.dataset.code;t.ctrlKey||t.metaKey?vt(n):yt(n)})})}function _t(e){e.forEach((e,t)=>{t===F.selectedPromptIndex?e.classList.add(`selected`):e.classList.remove(`selected`)})}function vt(e){let t=F.customPrompts.find(t=>t.code===e);if(!t)return;let n=document.getElementById(`userInput`),r=n.value,i=r.lastIndexOf(`/`),a=r;if(i!==-1){let e=-1;for(let t=i-1;t>=0;t--)if(r[t]===`
`){e=t;break}a=e===-1?r.substring(0,i).trimEnd():r.substring(0,e+1).trimEnd()}n.value=a+(a&&!a.endsWith(`
`)?`

`:``)+t.content,n.focus(),n.selectionStart=n.selectionEnd=n.value.length,J(),re(),console.log(`[SidePanel] 已追加提示词到输入框:`,t.code,t.content)}async function yt(e){let t=F.customPrompts.find(t=>t.code===e);if(!t)return;if(F.isGenerating){console.log(`[SidePanel] 正在生成中，请稍候...`);return}J();let n=document.getElementById(`chatContainer`).querySelector(`.welcome-message`);n&&n.remove();let r=t.content,i=F.enableSelectionQuery&&F.selectedContextText&&F.selectedContextText.trim(),a=F.quotedContextText&&F.quotedContextText.trim();if(a){let e=F.quotedContextText.trim();r=`[引用内容]\n${e}\n\n[用户问题]\n${t.content}`,G(`quoted`,e,!1),F.quotedContextText=``}else if(i){let e=F.selectedContextText.trim();r=`[选中内容]\n${e}\n\n[用户问题]\n${t.content}`,G(`selected`,e,!1),F.selectedContextText=``}(i||a)&&ut(),K(`user`,t.content),F.messageHistory.push({role:`user`,content:r}),U(),de(t.content);let o=document.getElementById(`userInput`);o.value=``,o.style.height=`auto`,F.isGenerating=!0;let s=document.getElementById(`sendBtn`);s.disabled=!0;let c=tt(),l=F.currentModel;try{await ce(),console.log(`[SidePanel] 发送消息调试信息:`),console.log(`  - isolateChat:`,F.isolateChat),console.log(`  - chatConfig:`,F.chatConfig),console.log(`  - messageHistory.length:`,F.messageHistory.length);let e=[{role:`system`,content:ae()}];if(F.isolateChat){let t=F.messageHistory;F.chatConfig.maxMemoryMessages!==null&&F.chatConfig.maxMemoryMessages!==void 0&&F.chatConfig.maxMemoryMessages>0?(t=[...F.messageHistory.slice(0,-1).slice(-F.chatConfig.maxMemoryMessages),F.messageHistory[F.messageHistory.length-1]],console.log(`[SidePanel] 记忆历史限制生效:`,F.chatConfig.maxMemoryMessages,`条（不含当前消息），实际发送:`,t.length,`条`)):console.log(`[SidePanel] 记忆历史限制未生效:`,F.chatConfig.maxMemoryMessages),e=[...e,...t]}else e.push({role:`user`,content:r});let t=await oe(),n,i;try{let r=await nt(e,l,F.useTools,t);n=r.content,i=r.executionLog||[]}catch(e){throw q(c),n=`❌ 请求失败：`+(e.message||`未知错误`),i=e.executionLog||[],K(`assistant`,n,!0,i),F.messageHistory.push({role:`assistant`,content:n,executionLog:i}),U(),e}q(c),await Le(K(`assistant`,n,!0,i)),F.messageHistory.push({role:`assistant`,content:n,executionLog:i}),U()}catch{}finally{F.isGenerating=!1,s.disabled=!1,o.focus()}}function bt(){document.getElementById(`promptManageModal`).classList.add(`show`),Y()}function xt(){document.getElementById(`promptManageModal`).classList.remove(`show`);let e=document.getElementById(`editPromptIndex`),t=document.getElementById(`newPromptCode`),n=document.getElementById(`newPromptContent`),r=document.getElementById(`promptManageAddBtn`);e&&(e.value=``),t&&(t.value=``),n&&(n.value=``),r&&(r.textContent=`添加提示词`,r.style.background=`#667eea`)}function Y(){let e=document.getElementById(`promptManageList`);if(F.customPrompts.length===0){e.innerHTML=`<div class="prompt-empty">暂无提示词，请添加</div>`;return}e.innerHTML=F.customPrompts.map((e,t)=>`
  ...
  `).join(``),e.querySelectorAll(`.move-up-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.index);if(n>0){let e=F.customPrompts[n];F.customPrompts[n]=F.customPrompts[n-1],F.customPrompts[n-1]=e,chrome.storage.local.set({customPrompts:F.customPrompts}),Y()}})}),e.querySelectorAll(`.move-down-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.index);if(n<F.customPrompts.length-1){let e=F.customPrompts[n];F.customPrompts[n]=F.customPrompts[n+1],F.customPrompts[n+1]=e,chrome.storage.local.set({customPrompts:F.customPrompts}),Y()}})}),e.querySelectorAll(`.menu-toggle-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.index);F.customPrompts[n].enabledInMenu=!F.customPrompts[n].enabledInMenu,chrome.storage.local.set({customPrompts:F.customPrompts}),Y()})}),e.querySelectorAll(`.edit-btn`).forEach(e=>{e.addEventListener(`click`,()=>{Tt(parseInt(e.dataset.index))})}),e.querySelectorAll(`.delete-btn`).forEach(e=>{e.addEventListener(`click`,()=>{Et(parseInt(e.dataset.index))})}),dt()}function St(e){let t=document.getElementById(`promptErrorModal`),n=document.getElementById(`promptErrorMessage`);n.textContent=e,t.classList.add(`show`)}function Ct(){document.getElementById(`promptErrorModal`).classList.remove(`show`)}function wt(){let e=document.getElementById(`editPromptIndex`),t=document.getElementById(`newPromptCode`),n=document.getElementById(`newPromptContent`),r=t.value.trim(),i=n.value.trim();if(!r||!i){St(`请填写编码和内容`);return}let a=e?parseInt(e.value):-1,o=F.customPrompts.findIndex(e=>e.code===r);if(o!==-1&&o!==a){St(`编码已存在`);return}a>=0&&a<F.customPrompts.length?F.customPrompts[a]={...F.customPrompts[a],code:r,content:i}:F.customPrompts.push({code:r,content:i,enabledInMenu:!1}),chrome.storage.local.set({customPrompts:F.customPrompts}),t.value=``,n.value=``,e&&(e.value=``);let s=document.getElementById(`promptManageAddBtn`);s.textContent=`添加提示词`,s.style.background=`#667eea`,Y()}function Tt(e){let t=F.customPrompts[e];if(!t)return;let n=document.getElementById(`editPromptIndex`),r=document.getElementById(`newPromptCode`),i=document.getElementById(`newPromptContent`),a=document.getElementById(`promptManageAddBtn`);n&&(n.value=e),r.value=t.code,i.value=t.content,a.textContent=`保存修改`,a.style.background=`#ffa502`,r.scrollIntoView({behavior:`smooth`})}function Et(e){let t=F.customPrompts[e];if(!t)return;F.pendingDeleteIndex=e;let n=document.getElementById(`deleteConfirmModal`),r=document.getElementById(`deleteConfirmMessage`);r.textContent=`确定要删除提示词 "/${t.code}" 吗？`,n.classList.add(`show`)}function X(){document.getElementById(`deleteConfirmModal`).classList.remove(`show`),F.pendingDeleteIndex=-1}function Dt(e){F.customPrompts.splice(e,1),chrome.storage.local.set({customPrompts:F.customPrompts}),Y()}function Ot(){let e=document.getElementById(`promptManageCancelBtn`),t=document.getElementById(`promptManageAddBtn`),n=document.getElementById(`promptModalCloseBtn`);e&&e.addEventListener(`click`,xt),t&&t.addEventListener(`click`,wt),n&&n.addEventListener(`click`,xt);let r=document.getElementById(`deleteCancelBtn`),i=document.getElementById(`deleteConfirmBtn`);r&&r.addEventListener(`click`,X),i&&i.addEventListener(`click`,()=>{F.pendingDeleteIndex>=0&&Dt(F.pendingDeleteIndex),X()});let a=document.getElementById(`deleteConfirmModal`);a&&a.addEventListener(`click`,e=>{e.target===a&&X()});let o=document.getElementById(`promptErrorConfirmBtn`);o&&o.addEventListener(`click`,Ct);let s=document.getElementById(`promptErrorModal`);s&&s.addEventListener(`click`,e=>{e.target===s&&Ct()})}function kt(){let e=document.getElementById(`toolsPopupOverlay`);if(!e)return;F.currentCategory=`all`,F.currentSearch=``;let t=document.getElementById(`toolsSearchInput`);t&&(t.value=``),Z(),Q(),document.querySelectorAll(`.category-btn`).forEach(e=>{e.classList.remove(`active`),e.classList.contains(`category-all`)?(e.classList.add(`active`),e.style.background=`linear-gradient(135deg, #667eea 0%, #764ba2 100%)`,e.style.color=`white`,e.style.borderColor=`transparent`):(e.style.background=`white`,e.style.color=`#555`,e.style.borderColor=`#ececec`)}),jt(),e.classList.add(`show`),console.log(`[SidePanel] 打开工具弹窗`)}function At(){let e=document.getElementById(`toolsPopupOverlay`);e&&(document.querySelectorAll(`.category-btn`).forEach(e=>{e.classList.remove(`active`),e.style.background=``,e.style.color=``,e.style.borderColor=``}),e.classList.remove(`show`),console.log(`[SidePanel] 关闭工具弹窗`))}function jt(){let t=document.getElementById(`toolsPopupList`);if(!t)return;t.innerHTML=``;let n={};e.forEach(e=>{if(F.currentCategory!==`all`&&e.category!==F.currentCategory)return;if(F.currentSearch){let t=e.name.toLowerCase().includes(F.currentSearch),n=e.description.toLowerCase().includes(F.currentSearch);if(!t&&!n)return}let t=e.category||`other`;n[t]||(n[t]=[]),n[t].push(e)});let r={page_interaction:`🖱️ 页面交互`,form_operation:`📝 表单操作`,info_extract:`📄 信息提取`,page_analysis:`🔍 页面分析`,tab_management:`📑 标签页管理`,bookmark_history:`🔖 书签历史`,storage_management:`💾 存储管理`,network_request:`🌐 网络请求`,media_process:`📷 媒体处理`,debug_dev:`🔧 调试开发`,ai_collaboration:`🤖 AI协作`,system_integration:`⚙️ 系统集成`};if([`page_interaction`,`form_operation`,`info_extract`,`page_analysis`,`tab_management`,`bookmark_history`,`storage_management`,`network_request`,`media_process`,`debug_dev`,`ai_collaboration`,`system_integration`].forEach(i=>{let a=n[i];if(!a||a.length===0)return;let o=e.filter(e=>e.category===i),s=o.length,c=o.filter(e=>F.enabledTools.includes(e.id)).length,l=document.createElement(`div`);l.className=`popup-tool-category-group`,l.dataset.category=i;let u=document.createElement(`div`);u.className=`popup-tool-category`,u.dataset.category=i;let d=F.collapsedCategories[i]||!1;u.innerHTML=`
      <span class="category-expand-icon">${d?`▶`:`▼`}</span>
      <span class="category-name">${r[i]||i}</span>
      <span class="category-count">${c}/${s}</span>
    `,u.addEventListener(`click`,()=>{Mt(i)}),l.appendChild(u);let f=document.createElement(`div`);f.className=`popup-tool-items ${d?`collapsed`:``}`,a.forEach(e=>{let t=F.enabledTools.includes(e.id),n=document.createElement(`div`);n.className=`popup-tool-item`,n.dataset.category=i,n.innerHTML=`
