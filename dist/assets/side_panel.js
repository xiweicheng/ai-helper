...
`).trim()}}async function hr(){let t=await z();if(t.activeSessionId&&t.list.length>0){N.activeSessionId=t.activeSessionId,N.sessions=t.list;let n=t.list.find(e=>e.id===t.activeSessionId);if(n){if(N.messageHistory=n.messageHistory||[],N.currentModel=n.model||N.currentModel,N.useTools=n.useTools===void 0?N.useTools:n.useTools,n.enabledTools&&n.enabledTools.length>0){let t=new Set(e.map(e=>e.id)),r=n.enabledTools.filter(e=>t.has(e)),i=e.filter(e=>e.enabled&&!r.includes(e.id)).map(e=>e.id);N.enabledTools=[...r,...i]}else N.enabledTools=n.enabledTools||N.enabledTools;N.temperature=n.temperature===void 0?N.temperature:n.temperature,N.topP=n.topP===void 0?N.topP:n.topP}N.messageHistory.forEach(e=>{let t=e.wasRevised;if(!t&&e.executionLog)try{t=(typeof e.executionLog==`string`?JSON.parse(e.executionLog):e.executionLog).some(e=>e.nodeType===`reflection`&&e.reflectionType===`post`&&e.action?.decision===`revised`)}catch{}Z(e.role,e.content,!1,e.executionLog||[],e.reflectionScore,t)});let r=document.querySelector(`.welcome-message`);r&&N.messageHistory.length>0&&r.remove(),Kt();let i=`scrollPosition_`+(N.activeSessionId||`default`);chrome.storage.local.get([i],e=>{e[i]!==void 0&&setTimeout(()=>{let t=document.getElementById(`chatContainer`);t.scrollTop=e[i]},100)}),H()}else{await yn();let e=await z();e.activeSessionId&&(N.activeSessionId=e.activeSessionId,N.sessions=e.list),H()}}function G(){try{vn().catch(e=>{console.error(`[SidePanel] 保存当前会话失败:`,e)})}catch(e){console.error(`[SidePanel] 保存对话历史异常:`,e)}}function gr(){N.messageHistory&&N.messageHistory.length>0&&wn().then(()=>{N.messageHistory=[];let e=document.getElementById(`chatContainer`);if(e){e.innerHTML=``;let t=document.createElement(`div`);t.className=`welcome-message`,t.innerHTML=`
  ...
  `,e.appendChild(n),e.scrollTop=e.scrollHeight;let r=n.querySelector(`.stop-task-btn`),i=n.querySelector(`.loading-text`);r&&r.addEventListener(`click`,e=>{e.stopPropagation(),r.disabled=!0,r.style.opacity=`0.6`,r.style.cursor=`not-allowed`,i&&(i.textContent=`停止中...`),chrome.runtime.sendMessage({type:`CANCEL_REACT`,tabId:null,sessionId:N.activeSessionId}),N.pendingCancelApi&&N.pendingCancelApi({message:`任务已被用户停止`,executionLog:N.currentExecutionStatus?.executionLog||[]})});let a=N.activeSessionId;if(N.executionLogListener=(e,n,r)=>e.sessionId&&e.sessionId!==a?!1:e.type===`EXECUTION_STATUS_UPDATE`?(console.log(`[SidePanel] 收到执行状态更新:`,e.nodeName,e.status,`日志数量:`,e.executionLog?.length),li(t,e.nodeName,e.status,e.executionLog),!1):!1,chrome.runtime.onMessage.addListener(N.executionLogListener),N.chatConfig.enableExecutionLog){let e=n.querySelector(`.execution-status-container`);e&&(e.style.display=`flex`)}let o=n.querySelector(`.execution-log-toggle-btn`);return o&&o.addEventListener(`click`,e=>{e.stopPropagation(),ci(t)}),t}function Q(e){let t=document.getElementById(e);if(t){let e=t.querySelector(`.loading-text`);e&&(e.textContent=`思考中...`),t.remove()}N.executionLogListener&&=(chrome.runtime.onMessage.removeListener(N.executionLogListener),null),N.currentExecutionStatus=null;let n=document.querySelector(`.execution-log-panel.realtime-mode`);n&&n.remove()}function di(){let e=document.getElementById(`chatContainer`),t=document.createElement(`div`);return t.className=`message-wrapper assistant streaming`,t.innerHTML=`
    <div class="message-content">
      <div class="stream-content"></div>
      <div class="stream-status">正在生成...</div>
    </div>
  `,e.appendChild(t),e.scrollTop=e.scrollHeight,t}function fi(e,t){let n=e.querySelector(`.stream-content`);if(!n)return;n.innerHTML=qt(t);let r=document.getElementById(`chatContainer`);r.scrollTop=r.scrollHeight}function pi(e,t){let n=e.querySelector(`.stream-status`);n&&(n.textContent=t)}function mi(e){let t=e.querySelector(`.stream-status`);t&&t.remove(),e.classList.remove(`streaming`),tn(),Qt(e)}async function hi(e,t,n=!1,r={}){let i=(await Je()).loopTimeout,a=N.activeSessionId,o=chrome.runtime.connect({name:`keepalive-`+a});console.log(`[SidePanel] keepalive 端口已连接, sessionId:`,a);let s={restarted:!1,rejectFn:null,cleanup:null};o.onMessage.addListener(e=>{e.type===`SW_RESTARTED`&&e.sessionId===a&&(console.warn(`[SidePanel] ⚠️ 收到 SW_RESTARTED 通知，后台已重启，API 调用已丢失`),s.restarted=!0,s.rejectFn&&s.cleanup&&(s.cleanup(),s.rejectFn({message:`后台服务异常重启，API 调用已中断，请重试`,executionLog:[]})))});let c={timeoutId:null,removeListener:()=>{}},l=()=>{try{o.disconnect()}catch{}c.timeoutId&&=(clearTimeout(c.timeoutId),null),c.removeListener(),N.pendingCancelApiMap.delete(a),N.pendingCallApiSessionIds.delete(a),ur()};return new Promise((o,u)=>{if(s.cleanup=l,s.rejectFn=u,s.restarted){l(),u({message:`后台服务异常重启，API 调用已中断，请重试`,executionLog:[]});return}let d=[],f=Math.round(i/1e3),p=null,m=``,h=e=>{l(),(!e.executionLog||e.executionLog.length===0||d.length>0)&&(e.executionLog=d),u(e)};N.pendingCancelApi=h,N.pendingCallApiSessionIds.add(a),ur(),console.log(`[SidePanel] callApi: 添加 pendingCallApiSessionIds, mySessionId =`,a,`, set:`,[...N.pendingCallApiSessionIds]),c.timeoutId=setTimeout(()=>{h({message:`请求超时（${f}秒）`,executionLog:d}),chrome.runtime.sendMessage({type:`CANCEL_REACT`,tabId:N.currentTabId,sessionId:N.activeSessionId}).catch(e=>{console.log(`[SidePanel] 发送取消请求失败:`,e.message)})},i);let g=Date.now(),_=0,v=null,y=()=>{v===null&&c.timeoutId!==null&&(v=Date.now(),clearTimeout(c.timeoutId),c.timeoutId=null,console.log(`[SidePanel] 前端超时已暂停（澄清工具执行中）`))},b=()=>{if(v!==null){let e=Date.now()-v;_+=e,v=null;let t=Date.now()-g,n=i+_-t;if(n<=0){h({message:`请求超时（${f}秒）`,executionLog:d});return}c.timeoutId=setTimeout(()=>{h({message:`请求超时（${f}秒）`,executionLog:d}),chrome.runtime.sendMessage({type:`CANCEL_REACT`,tabId:N.currentTabId,sessionId:a}).catch(e=>{console.log(`[SidePanel] 发送取消请求失败:`,e.message)})},n),console.log(`[SidePanel] 前端超时已恢复，暂停时长:`,Math.round(e/1e3),`s，剩余时间:`,Math.round(n/1e3),`s`)}},x=e=>{if(console.log(`[SidePanel] 收到消息:`,e),e.sessionId&&e.sessionId!==a)return!1;if(e.type===`EXECUTION_STATUS_UPDATE`)return d=e.executionLog||[],!1;if(e.type===`CLARIFY_START`)return y(),!1;if(e.type===`CLARIFY_END`)return b(),!1;if(e.type===`STREAM_START`){console.log(`[SidePanel] 流式输出开始`);let e=document.querySelector(`.loading-message`);return e&&e.remove(),N.executionLogListener&&=(chrome.runtime.onMessage.removeListener(N.executionLogListener),null),N.currentExecutionStatus=null,p=di(),m=``,!1}if(e.type===`STREAM_CHUNK`)return p&&(m+=e.delta,fi(p,m)),!1;if(e.type===`STREAM_TOOL_CALL`){if(p&&e.toolCalls?.length>0){let t=e.toolCalls.map(e=>e.function?.name||`unknown`).join(`, `);pi(p,`正在执行工具: ${t}...`)}return!1}return e.type===`STREAM_DONE`?(p&&mi(p),!1):e.type===`API_COMPLETE`?(l(),o({content:e.content,executionLog:e.executionLog||d,reflectionScore:e.reflectionScore,wasStreamed:!!p}),!1):e.type===`API_ERROR`?(l(),u({message:e.error,executionLog:e.executionLog||d}),!1):!1};chrome.runtime.onMessage.addListener(x),c.removeListener=()=>{chrome.runtime.onMessage.removeListener(x)},console.log(`[SidePanel] 发送 CALL_API 消息，useTools:`,n,`tabId:`,N.currentTabId,`sessionId:`,N.activeSessionId,`apiParams:`,r,`timeout:`,i),chrome.runtime.sendMessage({type:`CALL_API`,sessionId:N.activeSessionId,messages:e,model:t,useTools:n,tabId:N.currentTabId,apiParams:r,imageApiBase:N.enableImageInput&&N.attachedImages.length>0&&N.imageApiBase||``,imageApiKey:N.enableImageInput&&N.attachedImages.length>0&&N.imageApiKey||``})})}function gi(e){let t=document.querySelector(`.execution-log-panel`);t&&t.remove();let n=document.createElement(`div`);n.className=`execution-log-panel`;let r=e.reduce((e,t)=>e+(t.duration||0),0),i=e.filter(e=>e.status===`success`).length,a=e.filter(e=>e.status===`failed`).length,o=e.filter(e=>e.nodeType===`subtask`).length,s=e.filter(e=>e.nodeType===`subtask`&&e.status===`success`).length,c=e.filter(e=>e.nodeType===`tool_exec`&&e.action?.name===`plan_task`&&e.status===`success`).length,l=e.filter(e=>e.nodeType===`reflection`).find(e=>e.reflectionType===`post`);n.innerHTML=`
    <div class="log-container">
      <div class="log-header">
        <div class="log-title">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  `,document.body.appendChild(n),n.addEventListener(`click`,e=>{e.target===n&&n.remove()}),n.querySelector(`.log-close`).addEventListener(`click`,()=>{n.remove()});let u=n.querySelector(`.toggle-expand-btn`),d=n.querySelectorAll(`.timeline-content`),f=!1;u.addEventListener(`click`,()=>{f=!f,d.forEach(e=>{f?e.classList.add(`expanded`):e.classList.remove(`expanded`)});let e=u.querySelector(`svg`);f?(e.innerHTML=`<polyline points="17 11 12 6 7 11"></polyline><polyline points="17 18 12 13 7 18"></polyline>`,u.setAttribute(`title`,`收起全部节点`)):(e.innerHTML=`<polyline points="7 13 12 18 17 13"></polyline><polyline points="7 6 12 11 17 6"></polyline>`,u.setAttribute(`title`,`展开全部节点`))}),n.querySelectorAll(`.timeline-header`).forEach(e=>{e.addEventListener(`click`,()=>{e.parentElement.classList.toggle(`expanded`)})});let p=n.querySelectorAll(`.combo-stat`),m=n.querySelectorAll(`.timeline-item`);p.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.status,n=e.classList.contains(`active`);p.forEach(e=>e.classList.remove(`active`)),n?m.forEach(e=>{e.style.display=``}):(e.classList.add(`active`),m.forEach(e=>{if(t===`subtask`)e.classList.contains(`subtask-level`)?e.style.display=``:e.style.display=`none`;else{let n=e.querySelector(`.timeline-dot`);n&&n.classList.contains(t)?e.style.display=``:e.style.display=`none`}}))})})}function _i(e,t){try{let n=e.dataset.textContent_||e.dataset.rawContent||``;navigator.clipboard.writeText(n).then(()=>{let e=t.innerHTML;t.innerHTML=`
      ...
      `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}).catch(e=>{let r=document.createElement(`textarea`);r.value=n,r.style.position=`fixed`,r.style.left=`-999999px`,document.body.appendChild(r),r.select();try{document.execCommand(`copy`);let e=t.innerHTML;t.innerHTML=`
          ...
          <span>已复制</span>
        `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}catch{F(`复制失败，请手动选择内容复制`,`error`)}document.body.removeChild(r)})}catch(e){console.error(`[SidePanel] 复制失败:`,e),F(`复制失败`,`error`)}}function bi(e,t){try{let n=e.dataset.rawMarkdown||e.dataset.rawContent||``;if(!n){let t=e.querySelector(`.markdown-body`);n=t?t.innerText:e.innerText}let r=`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
    ...
    `,i=new Blob([`﻿`,r],{type:`application/msword`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`word-${new Date().getTime()}.doc`,document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(a);let s=t.innerHTML;t.innerHTML=`
      ...
      <span>已导出</span>
    `,setTimeout(()=>{t.innerHTML=s},2e3),console.log(`[SidePanel] Word 文档导出成功`)}catch(e){console.error(`[SidePanel] 导出 Word 失败:`,e),F(`导出失败: `+e.message,`error`)}}function xi(e,t){try{let n=e.dataset.rawMarkdown||e.dataset.rawContent||``;if(!n){let t=e.querySelector(`.markdown-body`);n=t?t.innerText:e.innerText}let r=`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
...
`),t.dispatchEvent(new Event(`input`)),console.log(`[SidePanel] 已移除输入框中的选中内容前缀`))}N.lastSelectedText=``,N.currentSelectionRange=null}function wi(){let e=document.getElementById(`promptManageList`).querySelectorAll(`.prompt-manage-item`);e.forEach(t=>{t.addEventListener(`dragstart`,e=>{N.draggedItemIndex=parseInt(t.dataset.index),t.classList.add(`dragging`),e.dataTransfer.effectAllowed=`move`,e.dataTransfer.setData(`text/plain`,N.draggedItemIndex)}),t.addEventListener(`dragend`,()=>{t.classList.remove(`dragging`),e.forEach(e=>e.classList.remove(`drag-over`)),N.draggedItemIndex=null}),t.addEventListener(`dragover`,e=>{e.preventDefault(),e.dataTransfer.dropEffect=`move`,t.classList.add(`drag-over`)}),t.addEventListener(`dragleave`,()=>{t.classList.remove(`drag-over`)}),t.addEventListener(`drop`,e=>{e.stopPropagation(),e.preventDefault();let n=parseInt(t.dataset.index);if(N.draggedItemIndex!==null&&N.draggedItemIndex!==n){let e=N.customPrompts[N.draggedItemIndex];N.customPrompts.splice(N.draggedItemIndex,1),N.customPrompts.splice(n,0,e),chrome.storage.local.set({customPrompts:N.customPrompts}),$()}t.classList.remove(`drag-over`)})})}function Ti(){let e=document.querySelector(`.input-toolbar-right`);if(!e)return;let t=document.createElement(`button`);t.className=`prompt-manage-btn`,t.title=`提示词管理`,t.innerHTML=`<svg t="1781177976746" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5076" width="16" height="16"><path d="M674.56 231.552l101.568 56.96-56.896-101.632 56.96-101.568-101.632 56.896-101.632-56.896 56.96 101.568-56.896 101.632 101.568-56.96zM186.944 629.76l-101.504-56.896 56.832 101.632-56.832 101.568 101.504-56.96 101.632 56.96-56.896-101.568 56.896-101.568-101.568 56.832zM85.44 85.312l56.832 101.568-56.832 101.632 101.504-56.96 101.632 56.96L231.68 186.88l56.896-101.568-101.568 56.896-101.568-56.896z m351.872 438.016l-99.2-99.136L424.32 337.984l99.072 99.264-86.08 86.144m-41.856-223.04L300.352 395.392a40.448 40.448 0 0 0 0 57.28l474.24 474.112a40.448 40.448 0 0 0 57.344 0l94.912-95.04a40.448 40.448 0 0 0 0-57.344L452.736 300.288a40.448 40.448 0 0 0-57.28 0z" p-id="5077" fill="#777"></path></svg>`,t.addEventListener(`click`,()=>{Pi()}),e.appendChild(t)}function Ei(e=``){let t=document.getElementById(`promptSelector`),n=document.getElementById(`promptDropdown`);t.style.display=`block`,n.classList.add(`show`),Ai(e)}function Di(){let e=document.getElementById(`promptSelector`),t=document.getElementById(`promptDropdown`);e.style.display=`none`,t.classList.remove(`show`),N.selectedPromptIndex=-1}function Oi(){let e=document.getElementById(`promptSelector`),t=document.getElementById(`promptDropdown`),n=document.getElementById(`userInput`);e.style.display!==`none`&&t.classList.contains(`show`)?Di():(Ei(),n.focus())}function ki(e=``){Ai(e)}function Ai(e=``){let t=document.getElementById(`promptList`),n=e.toLowerCase(),r=N.customPrompts.filter(t=>e?t.code.toLowerCase().includes(n)||t.content.toLowerCase().includes(n):!0);if(r.length===0){t.innerHTML=`<div class="prompt-empty">暂无匹配的提示词</div>`,N.selectedPromptIndex=-1;return}N.selectedPromptIndex=0,t.innerHTML=r.map((e,t)=>`
  ...
  `).join(``),t.querySelectorAll(`.prompt-item`).forEach(e=>{e.addEventListener(`click`,t=>{let n=e.dataset.code;t.ctrlKey||t.metaKey?Mi(n):Ni(n)})})}function ji(e){e.forEach((e,t)=>{t===N.selectedPromptIndex?e.classList.add(`selected`):e.classList.remove(`selected`)})}function Mi(e){let t=N.customPrompts.find(t=>t.code===e);if(!t)return;let n=document.getElementById(`userInput`),r=n.value,i=r.lastIndexOf(`/`),a=r;if(i!==-1){let e=-1;for(let t=i-1;t>=0;t--)if(r[t]===`
`){e=t;break}a=e===-1?r.substring(0,i).trimEnd():r.substring(0,e+1).trimEnd()}n.value=a+(a&&!a.endsWith(`
`)?`

`:``)+t.content,n.focus(),n.selectionStart=n.selectionEnd=n.value.length,Di(),Be(),console.log(`[SidePanel] 已追加提示词到输入框:`,t.code,t.content)}async function Ni(e){let t=N.customPrompts.find(t=>t.code===e);if(!t)return;if(N.isGenerating){console.log(`[SidePanel] 正在生成中，请稍候...`);return}Di();let n=document.getElementById(`chatContainer`).querySelector(`.welcome-message`);n&&n.remove();let r=t.content,i=N.enableSelectionQuery&&N.selectedContextText&&N.selectedContextText.trim(),a=N.quotedContextText&&N.quotedContextText.trim();if(a){let e=N.quotedContextText.trim();r=`[引用内容]\n${e}\n\n[用户问题]\n${t.content}`,Jr(`quoted`,e,!1),N.quotedContextText=``}else if(i){let e=N.selectedContextText.trim();r=`[选中内容]\n${e}\n\n[用户问题]\n${t.content}`,Jr(`selected`,e,!1),N.selectedContextText=``}(i||a)&&Ci();let o=Hr(r);Z(`user`,Hr(t.content)),N.messageHistory.push({role:`user`,content:o}),G(),Ye(t.content);let s=document.getElementById(`userInput`);s.value=``,s.style.height=`auto`,N.isGenerating=!0;let c=document.getElementById(`sendBtn`);c.disabled=!0;let l=ui(),u=N.activeSessionId,d=N.enableImageInput&&N.attachedImages.length>0&&N.imageModelName||N.currentModel;if(N.attachedImages.length>0){let e=document.getElementById(`imagePreviewBar`);e&&(e.innerHTML=``)}try{await Ke(),console.log(`[SidePanel] 发送消息调试信息:`),console.log(`  - isolateChat:`,N.isolateChat),console.log(`  - chatConfig:`,N.chatConfig),console.log(`  - messageHistory.length:`,N.messageHistory.length);let e=[{role:`system`,content:Ue()}];if(N.isolateChat){let t=N.messageHistory;N.chatConfig.maxMemoryMessages!==null&&N.chatConfig.maxMemoryMessages!==void 0&&N.chatConfig.maxMemoryMessages>0?(t=[...N.messageHistory.slice(0,-1).slice(-N.chatConfig.maxMemoryMessages),N.messageHistory[N.messageHistory.length-1]],console.log(`[SidePanel] 记忆历史限制生效:`,N.chatConfig.maxMemoryMessages,`条（不含当前消息），实际发送:`,t.length,`条`)):console.log(`[SidePanel] 记忆历史限制未生效:`,N.chatConfig.maxMemoryMessages),e=[...e,...t];for(let t=0;t<e.length-1;t++)e[t]={...e[t],content:Ur(e[t].content)}}else e.push({role:`user`,content:o});let t=await We(),n,r;try{let i=await hi(e,d,N.useTools,t);n=i.content,r=i.executionLog||[]}catch(e){throw Q(l),n=`❌ 请求失败：`+(e.message||`未知错误`),r=e.executionLog||[],Z(`assistant`,n,!0,r),N.messageHistory.push({role:`assistant`,content:n,executionLog:r}),G(),e}Q(l),await Qt(Z(`assistant`,n,!0,r)),N.messageHistory.push({role:`assistant`,content:n,executionLog:r}),G()}catch{}finally{N.generatingSessionIds.delete(u),c.disabled=!1,s.focus(),N.attachedImages=[]}}function Pi(){document.getElementById(`promptManageModal`).classList.add(`show`),$()}function Fi(){document.getElementById(`promptManageModal`).classList.remove(`show`);let e=document.getElementById(`editPromptIndex`),t=document.getElementById(`newPromptCode`),n=document.getElementById(`newPromptContent`),r=document.getElementById(`promptManageAddBtn`);e&&(e.value=``),t&&(t.value=``),n&&(n.value=``),r&&(r.textContent=`添加提示词`,r.style.background=`#667eea`)}function $(){let e=document.getElementById(`promptManageList`);if(N.customPrompts.length===0){e.innerHTML=`<div class="prompt-empty">暂无提示词，请添加</div>`;return}e.innerHTML=N.customPrompts.map((e,t)=>`
  ...
  `).join(``),e.querySelectorAll(`.move-up-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.index);if(n>0){let e=N.customPrompts[n];N.customPrompts[n]=N.customPrompts[n-1],N.customPrompts[n-1]=e,chrome.storage.local.set({customPrompts:N.customPrompts}),$()}})}),e.querySelectorAll(`.move-down-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.index);if(n<N.customPrompts.length-1){let e=N.customPrompts[n];N.customPrompts[n]=N.customPrompts[n+1],N.customPrompts[n+1]=e,chrome.storage.local.set({customPrompts:N.customPrompts}),$()}})}),e.querySelectorAll(`.menu-toggle-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.index);N.customPrompts[n].enabledInMenu=!N.customPrompts[n].enabledInMenu,chrome.storage.local.set({customPrompts:N.customPrompts}),$()})}),e.querySelectorAll(`.edit-btn`).forEach(e=>{e.addEventListener(`click`,()=>{zi(parseInt(e.dataset.index))})}),e.querySelectorAll(`.delete-btn`).forEach(e=>{e.addEventListener(`click`,()=>{Bi(parseInt(e.dataset.index))})}),wi()}function Ii(e){let t=document.getElementById(`promptErrorModal`),n=document.getElementById(`promptErrorMessage`);n.textContent=e,t.classList.add(`show`)}function Li(){document.getElementById(`promptErrorModal`).classList.remove(`show`)}function Ri(){let e=document.getElementById(`editPromptIndex`),t=document.getElementById(`newPromptCode`),n=document.getElementById(`newPromptContent`),r=t.value.trim(),i=n.value.trim();if(!r||!i){Ii(`请填写编码和内容`);return}let a=e?parseInt(e.value):-1,o=N.customPrompts.findIndex(e=>e.code===r);if(o!==-1&&o!==a){Ii(`编码已存在`);return}a>=0&&a<N.customPrompts.length?N.customPrompts[a]={...N.customPrompts[a],code:r,content:i}:N.customPrompts.push({code:r,content:i,enabledInMenu:!1}),chrome.storage.local.set({customPrompts:N.customPrompts}),t.value=``,n.value=``,e&&(e.value=``);let s=document.getElementById(`promptManageAddBtn`);s.textContent=`添加提示词`,s.style.background=`#667eea`,$()}function zi(e){let t=N.customPrompts[e];if(!t)return;let n=document.getElementById(`editPromptIndex`),r=document.getElementById(`newPromptCode`),i=document.getElementById(`newPromptContent`),a=document.getElementById(`promptManageAddBtn`);n&&(n.value=e),r.value=t.code,i.value=t.content,a.textContent=`保存修改`,a.style.background=`#ffa502`,r.scrollIntoView({behavior:`smooth`})}function Bi(e){let t=N.customPrompts[e];if(!t)return;N.pendingDeleteIndex=e;let n=document.getElementById(`deleteConfirmModal`),r=document.getElementById(`deleteConfirmMessage`);r.textContent=`确定要删除提示词 "/${t.code}" 吗？`,n.classList.add(`show`)}function Vi(){document.getElementById(`deleteConfirmModal`).classList.remove(`show`),N.pendingDeleteIndex=-1}function Hi(e){N.customPrompts.splice(e,1),chrome.storage.local.set({customPrompts:N.customPrompts}),$()}function Ui(){let e=document.getElementById(`promptManageCancelBtn`),t=document.getElementById(`promptManageAddBtn`),n=document.getElementById(`promptModalCloseBtn`);e&&e.addEventListener(`click`,Fi),t&&t.addEventListener(`click`,Ri),n&&n.addEventListener(`click`,Fi);let r=document.getElementById(`deleteCancelBtn`),i=document.getElementById(`deleteConfirmBtn`);r&&r.addEventListener(`click`,Vi),i&&i.addEventListener(`click`,()=>{N.pendingDeleteIndex>=0&&Hi(N.pendingDeleteIndex),Vi()});let a=document.getElementById(`deleteConfirmModal`);a&&a.addEventListener(`click`,e=>{e.target===a&&Vi()});let o=document.getElementById(`promptErrorConfirmBtn`);o&&o.addEventListener(`click`,Li);let s=document.getElementById(`promptErrorModal`);s&&s.addEventListener(`click`,e=>{e.target===s&&Li()})}function Wi(){let e=document.getElementById(`toolsPopupOverlay`);if(!e)return;N.currentCategory=`all`,N.currentSearch=``;let t=document.getElementById(`toolsSearchInput`);t&&(t.value=``),Zi(),Qi(),chrome.storage.local.get([`enableToolPreselect`],e=>{let t=document.getElementById(`toolsPreselectToggle`);t&&(t.checked=e.enableToolPreselect===void 0?!0:e.enableToolPreselect)}),document.querySelectorAll(`.category-btn`).forEach(e=>{e.classList.remove(`active`),e.classList.contains(`category-all`)?(e.classList.add(`active`),e.style.background=`linear-gradient(135deg, #667eea 0%, #764ba2 100%)`,e.style.color=`white`,e.style.borderColor=`transparent`):(e.style.background=`white`,e.style.color=`#555`,e.style.borderColor=`#ececec`)}),Ki(),e.classList.add(`show`),console.log(`[SidePanel] 打开工具弹窗`)}function Gi(){let e=document.getElementById(`toolsPopupOverlay`);e&&(document.querySelectorAll(`.category-btn`).forEach(e=>{e.classList.remove(`active`),e.style.background=``,e.style.color=``,e.style.borderColor=``}),e.classList.remove(`show`),console.log(`[SidePanel] 关闭工具弹窗`))}function Ki(){let t=document.getElementById(`toolsPopupList`);if(!t)return;t.innerHTML=``;let n={};e.forEach(e=>{if(N.currentCategory!==`all`&&e.category!==N.currentCategory)return;if(N.currentSearch){let t=e.name.toLowerCase().includes(N.currentSearch),n=e.description.toLowerCase().includes(N.currentSearch);if(!t&&!n)return}let t=e.category||`other`;n[t]||(n[t]=[]),n[t].push(e)});let r=ze;if(Re.forEach(i=>{let a=n[i];if(!a||a.length===0)return;let o=e.filter(e=>e.category===i),s=o.length,c=o.filter(e=>N.enabledTools.includes(e.id)).length,l=document.createElement(`div`);l.className=`popup-tool-category-group`,l.dataset.category=i;let u=document.createElement(`div`);u.className=`popup-tool-category`,u.dataset.category=i;let d=N.collapsedCategories[i]||!1;u.innerHTML=`
...
`||t[n-1]===` `?Ei(e):ki(e)}else Di();m()}),c.addEventListener(`scroll`,()=>{let e=`scrollPosition_`+(N.activeSessionId||`default`);chrome.storage.local.set({[e]:c.scrollTop})});let ne=document.getElementById(`headerMoreBtn`),D=document.getElementById(`headerMoreDropdown`);ne&&D&&(ne.addEventListener(`click`,e=>{e.stopPropagation(),D.classList.toggle(`show`)}),document.addEventListener(`click`,e=>{!D.contains(e.target)&&e.target!==ne&&D.classList.remove(`show`)})),o.addEventListener(`click`,e=>{e.stopPropagation(),D.classList.remove(`show`),wr()}),s&&s.addEventListener(`click`,e=>{e.stopPropagation(),D.classList.remove(`show`),_r()});let re=document.getElementById(`importChatBtn`);re&&re.addEventListener(`click`,e=>{e.stopPropagation(),D.classList.remove(`show`),Sr()});let ie=document.getElementById(`importSessionsFile`);ie&&ie.addEventListener(`change`,e=>{let t=e.target.files[0];t&&Cr(t)});let ae=document.getElementById(`settingsBtn`);ae&&ae.addEventListener(`click`,()=>{chrome.runtime.openOptionsPage()});let oe=document.getElementById(`headerAgentIndicator`);oe&&oe.addEventListener(`click`,async()=>{let e=chrome.runtime.getURL(`options.html#agent`),t=await chrome.tabs.query({url:chrome.runtime.getURL(`options.html`)});t.length>0?await chrome.tabs.update(t[0].id,{active:!0,url:e}):await chrome.tabs.create({url:e})});let se=document.getElementById(`prototypeLibraryBtn`);se&&se.addEventListener(`click`,e=>{e.stopPropagation(),D.classList.remove(`show`),Dt()});let ce=document.getElementById(`tokenStatsHeaderBtn`);ce&&ce.addEventListener(`click`,e=>{e.stopPropagation(),D.classList.remove(`show`),window.openTokenStats&&window.openTokenStats()}),ta(()=>N.activeSessionId,we);let le=document.getElementById(`isolateChatBtn`),O=document.getElementById(`enableToolsBtn`),ue=document.getElementById(`toolsConfigBtn`);chrome.storage.local.get([`isolateChat`,`enableSelectionQuery`,`enableTools`,`enabledTools`],t=>{t.isolateChat!==void 0&&(N.isolateChat=t.isolateChat),le.checked=N.isolateChat,t.enableSelectionQuery!==void 0&&(N.enableSelectionQuery=t.enableSelectionQuery);let n=document.getElementById(`enableSelectionQueryBtn`);if(n&&(n.checked=N.enableSelectionQuery),t.enableTools!==void 0&&(N.useTools=t.enableTools),t.enabledTools&&t.enabledTools.length>0){let n=new Set(e.map(e=>e.id)),r=t.enabledTools.filter(e=>n.has(e)),i=e.filter(e=>e.enabled&&!r.includes(e.id)).map(e=>e.id);N.enabledTools=[...r,...i],i.length>0&&chrome.storage.local.set({enabledTools:N.enabledTools})}else N.enabledTools=e.filter(e=>e.enabled).map(e=>e.id);N.enabledTools.length===0&&(N.useTools=!1),O&&(O.checked=N.useTools),x()}),le.addEventListener(`change`,()=>{N.isolateChat=le.checked,chrome.storage.local.set({isolateChat:N.isolateChat}),console.log(`[SidePanel] 记忆对话:`,N.isolateChat?`已启用`:`已禁用`)});let de=document.getElementById(`enableSelectionQueryBtn`);de&&de.addEventListener(`change`,()=>{N.enableSelectionQuery=de.checked,chrome.storage.local.set({enableSelectionQuery:N.enableSelectionQuery}),console.log(`[SidePanel] 划词问答:`,N.enableSelectionQuery?`已启用`:`已禁用`),!N.enableSelectionQuery&&N.selectedContextText&&mr()}),O&&O.addEventListener(`change`,()=>{N.useTools=O.checked,chrome.storage.local.set({enableTools:N.useTools}),N.useTools&&N.enabledTools.length===0&&(N.enabledTools=e.filter(e=>e.enabled).map(e=>e.id),chrome.storage.local.set({enabledTools:N.enabledTools})),console.log(`[SidePanel] 工具总开关:`,N.useTools?`已启用`:`已禁用`)}),ue&&ue.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),Wi()});let fe=document.getElementById(`toolsPopupOverlay`),pe=document.getElementById(`toolsPopupClose`),me=fe?fe.querySelector(`.modal-container`):null;pe&&pe.addEventListener(`click`,Gi),me&&me.addEventListener(`click`,e=>{e.stopPropagation()});let he=document.getElementById(`toolsSearchInput`);he&&he.addEventListener(`input`,e=>{N.currentSearch=e.target.value.toLowerCase(),Ki()});let ge=document.querySelectorAll(`.category-btn`);ge.forEach(e=>{e.addEventListener(`click`,()=>{ge.forEach(e=>{e.classList.remove(`active`),e.classList.contains(`category-all`)?(e.style.background=`#f5f3ff`,e.style.color=`#667eea`,e.style.borderColor=`#d4cfff`):(e.style.background=`white`,e.style.color=`#555`,e.style.borderColor=`#ececec`)}),e.classList.add(`active`),e.style.background=`linear-gradient(135deg, #667eea 0%, #764ba2 100%)`,e.style.color=`white`,e.style.borderColor=`transparent`,N.currentCategory=e.dataset.category,Ki()})});let k=document.getElementById(`toolsCategories`);k&&k.addEventListener(`wheel`,e=>{e.preventDefault(),k.scrollLeft+=e.deltaY*2},{passive:!1});let _e=document.getElementById(`toolsSelectAll`),ve=document.getElementById(`toolsSelectNone`);_e&&_e.addEventListener(`click`,()=>{Yi().forEach(e=>{let t=document.getElementById(`tool_`+e.id);t&&(t.checked=!0),N.enabledTools.includes(e.id)||N.enabledTools.push(e.id)}),Xi(),Zi(),Qi()}),ve&&ve.addEventListener(`click`,()=>{Yi().forEach(e=>{let t=document.getElementById(`tool_`+e.id);t&&(t.checked=!1);let n=N.enabledTools.indexOf(e.id);n>-1&&N.enabledTools.splice(n,1)}),Xi(),Zi(),Qi()});let ye=document.getElementById(`toolsPopupSave`);ye&&ye.addEventListener(`click`,()=>{$i(),Qi()});let be=document.getElementById(`toolsPreselectToggle`);be&&be.addEventListener(`change`,()=>{chrome.storage.local.set({enableToolPreselect:be.checked},()=>{console.log(`[SidePanel] 工具预筛选开关已更新:`,be.checked)})});let xe=document.getElementById(`toolsPopupCancel`);xe&&xe.addEventListener(`click`,()=>{Gi()});let Se=document.getElementById(`modalCancelBtn`),Ce=document.getElementById(`modalConfirmBtn`);function we(e,t=`确认操作`){return new Promise(n=>{let r=document.getElementById(`customConfirmOverlay`),i=document.getElementById(`customConfirmTitle`),a=document.getElementById(`customConfirmMessage`),o=document.getElementById(`customConfirmCancelBtn`),s=document.getElementById(`customConfirmOkBtn`);if(!r||!i||!a||!o||!s){n(confirm(e));return}let c=()=>{r.style.display=`none`,s.removeEventListener(`click`,l),o.removeEventListener(`click`,u),r.removeEventListener(`click`,d)},l=()=>{c(),n(!0)},u=()=>{c(),n(!1)},d=e=>{e.target===r&&(c(),n(!1))};i.textContent=t,a.textContent=e,r.style.display=`flex`,s.addEventListener(`click`,l),o.addEventListener(`click`,u),r.addEventListener(`click`,d)})}let A=document.getElementById(`toolStatsOverlay`),Te=document.getElementById(`toolStatsClose`),Ee=document.getElementById(`toolStatsBtn`);function De(){A&&(A.style.display=`flex`,M())}function Oe(){A&&(A.style.display=`none`)}Ee&&Ee.addEventListener(`click`,e=>{e.stopPropagation(),De()}),Te&&Te.addEventListener(`click`,Oe),A&&A.addEventListener(`click`,e=>{e.target===A&&Oe()});let ke=document.getElementById(`toolStatsRefreshBtn`);ke&&ke.addEventListener(`click`,M);let Ae=document.getElementById(`toolStatsClearBtn`);Ae&&Ae.addEventListener(`click`,async()=>{await we(`确定要清空所有工具使用统计吗？此操作不可撤销。`,`清空统计`)&&(await chrome.storage.local.remove([`toolUsageStats`]),M())});let j={column:`callCount`,asc:!1};async function M(){let t=document.getElementById(`toolStatsTable`),n=document.getElementById(`toolStatsTableBody`),r=document.getElementById(`toolStatsLoading`),i=document.getElementById(`toolStatsEmpty`),a=document.getElementById(`toolStatsSummary`),o=document.getElementById(`toolStatsUnusedSection`),s=document.getElementById(`toolStatsUnusedList`);if(!(!t||!n||!r||!i)){t.style.display=`none`,i.style.display=`none`,o&&(o.style.display=`none`),a&&(a.textContent=``),r.style.display=``;try{let n=(await chrome.storage.local.get([`toolUsageStats`])).toolUsageStats||{},c=Object.entries(n);if(c.length===0){r.style.display=`none`,i.style.display=``;return}let l={};e.forEach(e=>{l[e.id]=e.name?`${e.name}：${e.description||``}`:e.description||e.id}),je(c,l);let u=e.map(e=>e.id),d=new Set(c.map(([e])=>e)),f=u.filter(e=>!d.has(e)),p=c.length,m=f.length;a&&(a.textContent=`已使用 ${p} 个，未使用 ${m} 个`),o&&s&&m>0&&(s.innerHTML=f.sort((e,t)=>e.toLowerCase().localeCompare(t.toLowerCase())).map(e=>`<code title="${I(l[e]||e)}" style="padding: 3px 10px; background: #f5f5f5; color: #aaa; border: 1px solid #eee; border-radius: 4px; font-size: 11px;">${e}</code>`).join(``),o.style.display=``),r.style.display=`none`,t.style.display=``}catch(e){console.error(`[SidePanel] 加载统计失败:`,e),r.style.display=`none`,i.textContent=`加载失败`,i.style.display=``}}}function je(e,t){let n=document.getElementById(`toolStatsTableBody`);if(!n)return;let{column:r,asc:i}=j;n.innerHTML=[...e].sort((e,t)=>{let[n,a]=e,[o,s]=t,c=a.callCount>0?a.successCount/a.callCount*100:0,l=s.callCount>0?s.successCount/s.callCount*100:0,u=a.callCount>0?a.totalDuration/a.callCount:0,d=s.callCount>0?s.totalDuration/s.callCount:0,f=0;switch(r){case`name`:f=n.toLowerCase().localeCompare(o.toLowerCase());break;case`callCount`:f=a.callCount-s.callCount;break;case`successCount`:f=a.successCount-s.successCount;break;case`successRate`:f=c-l;break;case`duration`:f=u-d;break}return i?f:-f}).map(([e,n])=>{let r=n.callCount>0?n.successCount/n.callCount*100:0,i=n.callCount>0?n.totalDuration/n.callCount:0,a=t[e]||e,o=`#38a169`;r<60?o=`#e53e3e`:r<85&&(o=`#d69e2e`);let s=i<1e3?`${Math.round(i)}ms`:`${(i/1e3).toFixed(1)}s`;return`<tr>
      ...
      </tr>`}).join(``),Me()}function Me(){let{column:e,asc:t}=j,n=[`name`,`callCount`,`successCount`,`successRate`,`duration`],r={name:`sortByName`,callCount:`sortByCallCount`,successCount:`sortBySuccessCount`,successRate:`sortBySuccessRate`,duration:`sortByDuration`};n.forEach(n=>{let i=document.getElementById(r[n]);if(!i)return;let a=i.querySelector(`.sort-indicator`);a&&(n===e?(a.textContent=t?`▲`:`▼`,a.style.color=`#667eea`):(a.textContent=``,a.style.color=``))})}document.querySelectorAll(`#toolStatsTable th[data-sort]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.sort;j.column===t?j.asc=!j.asc:(j.column=t,j.asc=!1),M()})}),Se.addEventListener(`click`,()=>{Tr()}),Ce.addEventListener(`click`,()=>{Tr(),gr()});let Ne=document.getElementById(`confirmModal`);Ne.addEventListener(`click`,e=>{e.target===Ne&&Tr()});let Pe=document.getElementById(`selectionClose`);Pe&&Pe.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),console.log(`[SidePanel] 用户点击关闭选中内容按钮`),mr(),window.hideFloatingMenu(),N.lastSelectedText=``,N.currentSelectionRange=null}),i.addEventListener(`input`,()=>{})}),Ge().then(()=>na()),document.addEventListener(`DOMContentLoaded`,()=>{ia()}),document.addEventListener(`DOMContentLoaded`,Ze),document.addEventListener(`DOMContentLoaded`,Ui),document.addEventListener(`DOMContentLoaded`,ft),document.addEventListener(`DOMContentLoaded`,gt),document.addEventListener(`DOMContentLoaded`,Vt),document.addEventListener(`DOMContentLoaded`,xr);function da(){let e=document.getElementById(`imagePreviewBar`),t=document.getElementById(`screenshotBtn`);e&&(e.style.display=N.attachedImages.length>0?``:`none`),t&&(N.enableImageInput?t.style.removeProperty(`display`):t.style.display=`none`),userInput&&(N.enableImageInput?userInput.style.paddingRight=`84px`:userInput.style.paddingRight=``),N.enableImageInput||(N.attachedImages=[]),fa()}function fa(){let e=document.getElementById(`imagePreviewBar`);if(e){if(e.innerHTML=``,N.attachedImages.length===0){e.style.display=`none`;return}e.style.display=``,N.attachedImages.forEach((t,n)=>{let r=document.createElement(`div`);r.className=`image-preview-item`;let i=document.createElement(`img`);i.src=t.dataUrl,i.className=`image-preview-thumb`,i.title=`点击查看大图`,i.style.cursor=`zoom-in`,i.addEventListener(`click`,()=>{Nr(t.dataUrl,i)});let a=document.createElement(`button`);a.className=`image-preview-remove`,a.innerHTML=`×`,a.title=`移除图片`,a.addEventListener(`click`,e=>{e.stopPropagation(),N.attachedImages.splice(n,1),fa()}),r.appendChild(i),r.appendChild(a),e.appendChild(r)})}}async function pa(){if(!N.enableImageInput){F(`请先开启图片输入功能`);return}try{let e=await chrome.runtime.sendMessage({type:`CAPTURE_TAB`});e?.dataUrl&&(Br(await(await fetch(e.dataUrl)).blob()),F(`截图成功`))}catch(e){console.error(`[SidePanel] 全页面截图失败:`,e),F(`截图失败，请重试`)}}async function ma(){let e=await qe();if(!e){F(`无法获取当前标签页`);return}try{let t=await chrome.tabs.sendMessage(e,{type:`START_REGION_SELECTION`});if(!t)return;console.log(`[SidePanel] 区域选择结果:`,t);let n=await chrome.runtime.sendMessage({type:`CAPTURE_TAB`});if(!n?.dataUrl){F(`截图失败，请重试`);return}let r=await ha(n.dataUrl,t);if(!r){F(`裁剪失败，请重试`);return}Br(await(await fetch(r)).blob())}catch(e){console.error(`[SidePanel] 区域截图失败:`,e),F(`区域截图失败，请确保页面已加载且未被浏览器限制`)}}function ha(e,t){return new Promise((n,r)=>{let i=new Image;i.onload=()=>{let e=document.createElement(`canvas`),r=window.devicePixelRatio||1,a=t.x*r,o=t.y*r,s=t.width*r,c=t.height*r;e.width=s,e.height=c,e.getContext(`2d`).drawImage(i,a,o,s,c,0,0,s,c),n(e.toDataURL(`image/jpeg`,.85))},i.onerror=()=>r(Error(`图片加载失败`)),i.src=e})}
//# sourceMappingURL=side_panel-CuOw68Ub.js.map