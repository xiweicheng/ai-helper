...
4. **调用 plan_task 工具**：
   ...
   - 指定执行策略：sequential（顺序执行）、parallel（并行执行）或 conditional（条件执行）`:``,o;return o=e&&e.systemPrompt&&e.systemPrompt.trim()?e.systemPrompt:N.systemPrompt&&N.systemPrompt.trim()?N.systemPrompt:null,o?`${o}
...
`),t.dispatchEvent(new Event(`input`)),console.log(`[SidePanel] 已移除输入框中的选中内容前缀`))}N.lastSelectedText=``,N.currentSelectionRange=null}function Ga(){let e=document.getElementById(`promptManageList`).querySelectorAll(`.prompt-manage-item`);e.forEach(t=>{t.addEventListener(`dragstart`,e=>{N.draggedItemIndex=parseInt(t.dataset.index),t.classList.add(`dragging`),e.dataTransfer.effectAllowed=`move`,e.dataTransfer.setData(`text/plain`,N.draggedItemIndex)}),t.addEventListener(`dragend`,()=>{t.classList.remove(`dragging`),e.forEach(e=>e.classList.remove(`drag-over`)),N.draggedItemIndex=null}),t.addEventListener(`dragover`,e=>{e.preventDefault(),e.dataTransfer.dropEffect=`move`,t.classList.add(`drag-over`)}),t.addEventListener(`dragleave`,()=>{t.classList.remove(`drag-over`)}),t.addEventListener(`drop`,e=>{e.stopPropagation(),e.preventDefault();let n=parseInt(t.dataset.index);if(N.draggedItemIndex!==null&&N.draggedItemIndex!==n){let e=N.customPrompts[N.draggedItemIndex];N.customPrompts.splice(N.draggedItemIndex,1),N.customPrompts.splice(n,0,e),chrome.storage.local.set({customPrompts:N.customPrompts}),$()}t.classList.remove(`drag-over`)})})}function Ka(){let e=document.querySelector(`.input-toolbar-right`);if(!e)return;let t=document.createElement(`button`);t.className=`prompt-manage-btn`,t.title=`提示词管理`,t.innerHTML=`<svg t="1781177976746" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5076" width="16" height="16"><path d="M674.56 231.552l101.568 56.96-56.896-101.632 56.96-101.568-101.632 56.896-101.632-56.896 56.96 101.568-56.896 101.632 101.568-56.96zM186.944 629.76l-101.504-56.896 56.832 101.632-56.832 101.568 101.504-56.96 101.632 56.96-56.896-101.568 56.896-101.568-101.568 56.832zM85.44 85.312l56.832 101.568-56.832 101.632 101.504-56.96 101.632 56.96L231.68 186.88l56.896-101.568-101.568 56.896-101.568-56.896z m351.872 438.016l-99.2-99.136L424.32 337.984l99.072 99.264-86.08 86.144m-41.856-223.04L300.352 395.392a40.448 40.448 0 0 0 0 57.28l474.24 474.112a40.448 40.448 0 0 0 57.344 0l94.912-95.04a40.448 40.448 0 0 0 0-57.344L452.736 300.288a40.448 40.448 0 0 0-57.28 0z" p-id="5077" fill="#777"></path></svg>`,t.addEventListener(`click`,()=>{eo()}),e.appendChild(t)}function qa(e=``){let t=document.getElementById(`promptSelector`),n=document.getElementById(`promptDropdown`);t.style.display=`block`,n.classList.add(`show`),Xa(e)}function Q(){let e=document.getElementById(`promptSelector`),t=document.getElementById(`promptDropdown`);e.style.display=`none`,t.classList.remove(`show`),N.selectedPromptIndex=-1}function Ja(){let e=document.getElementById(`promptSelector`),t=document.getElementById(`promptDropdown`),n=document.getElementById(`userInput`);e.style.display!==`none`&&t.classList.contains(`show`)?Q():(qa(),n.focus())}function Ya(e=``){Xa(e)}function Xa(e=``){let t=document.getElementById(`promptList`),n=e.toLowerCase(),r=N.customPrompts.filter(t=>e?t.code.toLowerCase().includes(n)||t.content.toLowerCase().includes(n):!0);if(r.length===0){t.innerHTML=`<div class="prompt-empty">暂无匹配的提示词</div>`,N.selectedPromptIndex=-1;return}N.selectedPromptIndex=0,t.innerHTML=r.map((e,t)=>`
  ...
  `).join(``),t.querySelectorAll(`.prompt-item`).forEach(e=>{e.addEventListener(`click`,t=>{let n=e.dataset.code;t.ctrlKey||t.metaKey?Qa(n):$a(n)})})}function Za(e){e.forEach((e,t)=>{t===N.selectedPromptIndex?e.classList.add(`selected`):e.classList.remove(`selected`)})}function Qa(e){let t=N.customPrompts.find(t=>t.code===e);if(!t)return;let n=document.getElementById(`userInput`),r=n.value,i=r.lastIndexOf(`/`),a=r;if(i!==-1){let e=-1;for(let t=i-1;t>=0;t--)if(r[t]===`
`){e=t;break}a=e===-1?r.substring(0,i).trimEnd():r.substring(0,e+1).trimEnd()}n.value=a+(a&&!a.endsWith(`
`)?`

`:``)+t.content,n.focus(),n.selectionStart=n.selectionEnd=n.value.length,Q(),lt(),console.log(`[SidePanel] 已追加提示词到输入框:`,t.code,t.content)}async function $a(e){let t=N.customPrompts.find(t=>t.code===e);if(!t)return;if(N.isGenerating){console.log(`[SidePanel] 正在生成中，请稍候...`);return}Q();let n=document.getElementById(`chatContainer`).querySelector(`.welcome-message`);n&&n.remove();let r=t.content,i=N.enableSelectionQuery&&N.selectedContextText&&N.selectedContextText.trim(),a=N.quotedContextText&&N.quotedContextText.trim();if(a){let e=N.quotedContextText.trim(),{compressed:n,wasCompressed:i}=E(e);r=`[引用内容${i?`摘要`:``}]\n${n}\n\n[用户问题]\n${t.content}`,sa(`quoted`,e,!1),N.quotedContextText=``}else if(i){let e=N.selectedContextText.trim(),{compressed:n,wasCompressed:i}=E(e);r=`[选中内容${i?`摘要`:``}]\n${n}\n\n[用户问题]\n${t.content}`,sa(`selected`,e,!1),N.selectedContextText=``}(i||a)&&Wa();let o=ta(r);Y(`user`,ta(t.content)),N.messageHistory.push({role:`user`,content:o}),K(),vt(t.content);let c=document.getElementById(`userInput`);c.value=``,c.style.height=`auto`,N.isGenerating=!0;let l=Ea(),u=N.activeSessionId,d=N.enableImageInput&&N.attachedImages.length>0&&N.imageModelName||N.currentModel;if(N.attachedImages.length>0){let e=document.getElementById(`imagePreviewBar`);e&&(e.innerHTML=``)}try{await ht(),console.log(`[SidePanel] 发送消息调试信息:`),console.log(`  - isolateChat:`,N.isolateChat),console.log(`  - chatConfig:`,N.chatConfig),console.log(`  - messageHistory.length:`,N.messageHistory.length);let e=[{role:`system`,content:await ft()}];if(N.isolateChat){let t=N.messageHistory,n=N.chatConfig.contextWindow||0,r=g(d,N.enabledTools.length||50,n,N.customModelMap),i=Math.floor(r*.7),a=N.messageHistory.slice(0,-1),o=N.messageHistory[N.messageHistory.length-1],c=[],l=s([o]);for(let e=a.length-1;e>=0;e--){let t=a[e],n=s([t]);if(l+n<=i)c.unshift(t),l+=n;else break}if(c.length<a.length){let t=a.length-c.length,n=_(a.slice(0,t));n&&(e[0]={...e[0],content:e[0].content+`

`+n})}t=[...c,o],e=[...e,...t];for(let t=0;t<e.length-1;t++)e[t]={...e[t],content:na(e[t].content)}}else e.push({role:`user`,content:o});let t=await pt(),n,r;try{let i=await Ia(e,d,N.useTools,t);n=i.content,r=i.executionLog||[]}catch(e){throw Z(l),n=`❌ 请求失败：`+(e.message||`未知错误`),r=e.executionLog||[],Y(`assistant`,n,!0,r),N.messageHistory.push({role:`assistant`,content:n,executionLog:r}),K(),e}Z(l),await wn(Y(`assistant`,n,!0,r)),N.messageHistory.push({role:`assistant`,content:n,executionLog:r}),K()}catch{}finally{N.generatingSessionIds.delete(u),document.dispatchEvent(new CustomEvent(`generating-state-changed`)),c.focus(),N.attachedImages=[]}}function eo(){document.getElementById(`promptManageModal`).classList.add(`show`),$()}function to(){document.getElementById(`promptManageModal`).classList.remove(`show`);let e=document.getElementById(`editPromptIndex`),t=document.getElementById(`newPromptCode`),n=document.getElementById(`newPromptContent`),r=document.getElementById(`promptManageAddBtn`);e&&(e.value=``),t&&(t.value=``),n&&(n.value=``),r&&(r.textContent=`添加提示词`,r.style.background=`#667eea`)}function $(){let e=document.getElementById(`promptManageList`);if(N.customPrompts.length===0){e.innerHTML=`<div class="prompt-empty">暂无提示词，请添加</div>`;return}e.innerHTML=N.customPrompts.map((e,t)=>`
