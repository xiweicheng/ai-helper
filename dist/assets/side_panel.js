...
`).trim()}}async function An(){let t=await V();if(t.activeSessionId&&t.list.length>0){F.activeSessionId=t.activeSessionId,F.sessions=t.list;let n=t.list.find(e=>e.id===t.activeSessionId);if(n){if(F.messageHistory=n.messageHistory||[],F.currentModel=n.model||F.currentModel,F.useTools=n.useTools===void 0?F.useTools:n.useTools,n.enabledTools&&n.enabledTools.length>0){let t=new Set(e.map(e=>e.id)),r=n.enabledTools.filter(e=>t.has(e)),i=e.filter(e=>e.enabled&&!r.includes(e.id)).map(e=>e.id);F.enabledTools=[...r,...i]}else F.enabledTools=n.enabledTools||F.enabledTools;F.temperature=n.temperature===void 0?F.temperature:n.temperature,F.topP=n.topP===void 0?F.topP:n.topP}F.messageHistory.forEach(e=>{let t=e.wasRevised;if(!t&&e.executionLog)try{t=(typeof e.executionLog==`string`?JSON.parse(e.executionLog):e.executionLog).some(e=>e.nodeType===`reflection`&&e.reflectionType===`post`&&e.action?.decision===`revised`)}catch{}X(e.role,e.content,!1,e.executionLog||[],e.reflectionScore,t)});let r=document.querySelector(`.welcome-message`);r&&F.messageHistory.length>0&&r.remove(),Vt();let i=`scrollPosition_`+(F.activeSessionId||`default`);chrome.storage.local.get([i],e=>{e[i]!==void 0&&setTimeout(()=>{let t=document.getElementById(`chatContainer`);t.scrollTop=e[i]},100)}),H()}else{await un();let e=await V();e.activeSessionId&&(F.activeSessionId=e.activeSessionId,F.sessions=e.list),H()}}function U(){try{ln().catch(e=>{console.error(`[SidePanel] 保存当前会话失败:`,e)})}catch(e){console.error(`[SidePanel] 保存对话历史异常:`,e)}}function jn(){F.messageHistory&&F.messageHistory.length>0&&mn().then(()=>{F.messageHistory=[];let e=document.getElementById(`chatContainer`);if(e){e.innerHTML=``;let t=document.createElement(`div`);t.className=`welcome-message`,t.innerHTML=`
  ...
  `,document.body.appendChild(r),r.querySelector(`.ri-close`).addEventListener(`click`,()=>r.remove()),r.addEventListener(`click`,e=>{e.target===r&&r.remove()});let S=t.getBoundingClientRect(),C=r.querySelector(`.reflection-info-panel`),w=Math.min(window.innerHeight-40,560),T=S.right-380;T<10&&(T=10),T+380>window.innerWidth-10&&(T=window.innerWidth-380-10);let E=S.bottom+6;E+w>window.innerHeight-10&&(E=S.top-w-6),E<10&&(E=10),C.style.left=T+`px`,C.style.top=E+`px`,C.style.maxHeight=w+`px`}function er(e){let t=[...e].sort((e,t)=>new Date(e.timestamp)-new Date(t.timestamp)),n=t.length,r=``,i=null;return t.forEach((e,t)=>{let a=e.nodeType===`subtask`,o=e.nodeType===`tool_exec`,s=e.nodeType===`api_call`,c=e.nodeType===`preselect`,l=e.nodeType===`reflection`,u=o&&e.action?.name===`plan_task`;a&&(i=e.subtaskIndex);let d=``,f=``;l?f=`🎯`:c?f=`🔍`:u?(d=`plan-task-level`,f=`📋`):a?(d=`subtask-level`,f=`🔀`):o&&i!==null?(d=`tool-level`,f=`🔧`):s&&i!==null?(d=`api-level`,f=`📡`):o?f=`⚡`:s&&(f=`📡`);let p=`○`,m=e.status||`processing`;e.status===`success`?p=`✓`:e.status===`failed`&&(p=`✗`),l&&(m=`reflection ${m}`);let h=R(e.nodeName||`未知节点`);if(e.subtaskIndex!==null&&e.subtaskIndex>=0&&(h=`<span class="subtask-badge">${e.subtaskIndex+1}</span> ${h}`),e.subtaskCount&&(h+=` <span class="plan-badge">(${e.subtaskCount}个子任务, ${e.strategy===`sequential`?`顺序执行`:`并行执行`})</span>`),(s||c||l)&&e.apiRequest){let t=[];e.apiRequest.messageCount!==void 0&&e.apiRequest.messageCount!==null&&t.push(`💬<span title="本次模型API调用携带的消息数">${e.apiRequest.messageCount}条</span>`),!c&&e.apiRequest.toolCount!==void 0&&e.apiRequest.toolCount!==null&&t.push(`🔧<span title="本次模型API调用携带的工具定义数">${e.apiRequest.toolCount}个</span>`),t.length>0&&(h+=` <span class="api-info-badge">（${t.join(` `)}）</span>`)}r+=`
    ...
    `}),r}function tr(e){let t=[...e].sort((e,t)=>new Date(e.timestamp)-new Date(t.timestamp));if(!t.some(e=>e.taskGroup))return ar(t);let n=new Map,r=[];t.forEach(e=>{e.taskGroup?(n.has(e.taskGroup)||n.set(e.taskGroup,{groupId:e.taskGroup,groupIndex:e.taskGroupIndex,groupName:e.taskGroupName,entries:[],status:e.status}),n.get(e.taskGroup).entries.push(e),e.status&&(n.get(e.taskGroup).status=e.status)):r.push(e)});let i=nr(r,t.length);return n.forEach((e,n)=>{let r=e.status||`processing`;i+=`
      <div class="task-group-container" data-group-id="${n}">
        ...
        <div class="task-group-timeline">
          ${rr(e.entries,t.length)}
        </div>
      </div>
    `}),i}function nr(e,t){if(e.length===0)return``;let n=``;return n+=`
  ...
  `,document.body.appendChild(n),n.addEventListener(`click`,e=>{e.target===n&&n.remove()}),n.querySelector(`.log-close`).addEventListener(`click`,()=>{n.remove()});let u=n.querySelector(`.toggle-expand-btn`),d=n.querySelectorAll(`.timeline-content`),f=!1;u.addEventListener(`click`,()=>{f=!f,d.forEach(e=>{f?e.classList.add(`expanded`):e.classList.remove(`expanded`)});let e=u.querySelector(`svg`);f?(e.innerHTML=`<polyline points="17 11 12 6 7 11"></polyline><polyline points="17 18 12 13 7 18"></polyline>`,u.setAttribute(`title`,`收起全部节点`)):(e.innerHTML=`<polyline points="7 13 12 18 17 13"></polyline><polyline points="7 6 12 11 17 6"></polyline>`,u.setAttribute(`title`,`展开全部节点`))}),n.querySelectorAll(`.timeline-header`).forEach(e=>{e.addEventListener(`click`,()=>{e.parentElement.classList.toggle(`expanded`)})});let p=n.querySelectorAll(`.combo-stat`),m=n.querySelectorAll(`.timeline-item`);p.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.status,n=e.classList.contains(`active`);p.forEach(e=>e.classList.remove(`active`)),n?m.forEach(e=>{e.style.display=``}):(e.classList.add(`active`),m.forEach(e=>{if(t===`subtask`)e.classList.contains(`subtask-level`)?e.style.display=``:e.style.display=`none`;else{let n=e.querySelector(`.timeline-dot`);n&&n.classList.contains(t)?e.style.display=``:e.style.display=`none`}}))})})}function pr(e,t){try{let n=e.dataset.textContent_||e.dataset.rawContent||``;navigator.clipboard.writeText(n).then(()=>{let e=t.innerHTML;t.innerHTML=`
      ...
      `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}).catch(e=>{console.error(`[SidePanel] 复制失败:`,e);let r=document.createElement(`textarea`);r.value=n,r.style.position=`fixed`,r.style.left=`-999999px`,document.body.appendChild(r),r.select();try{document.execCommand(`copy`);let e=t.innerHTML;t.innerHTML=`
        ...
        `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}catch{L(`复制失败`,`error`)}document.body.removeChild(r)})}catch(e){console.error(`[SidePanel] 复制失败:`,e),L(`复制失败`,`error`)}}function mr(e){try{let t=e.dataset.textContent_||e.dataset.rawContent||``;if(!t){L(`无法获取消息内容`,`error`);return}let n=document.getElementById(`userInput`);n.value=t,Fe(),n.focus(),n.selectionStart=n.selectionEnd=n.value.length,console.log(`[SidePanel] 已加载消息内容到输入框，等待用户编辑后发送`)}catch(e){console.error(`[SidePanel] 编辑消息失败:`,e),L(`编辑失败: `+e.message,`error`)}}function hr(e,t){try{let n=e.dataset.rawMarkdown||e.dataset.rawContent||``;if(!n){let t=F.messageHistory.find(e=>e.role===`assistant`);if(t)n=t.content;else{let t=e.querySelector(`.markdown-body`);n=t?t.innerText:e.innerText}}navigator.clipboard.writeText(n).then(()=>{let e=t.innerHTML;t.innerHTML=`
        <svg viewBox="0 0 16 16" fill="currentColor">
          <path d="M13.78 4.22a.75.75 0 0 1 0 1.06l-7.25 7.25a.75.75 0 0 1-1.06 0L2.22 9.28a.75.75 0 0 1 1.06-1.06L6 10.94l6.72-6.72a.75.75 0 0 1 1.06 0z"/>
        </svg>
        <span>已复制</span>
      `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}).catch(e=>{let r=document.createElement(`textarea`);r.value=n,r.style.position=`fixed`,r.style.left=`-999999px`,document.body.appendChild(r),r.select();try{document.execCommand(`copy`);let e=t.innerHTML;t.innerHTML=`
          ...
          <span>已复制</span>
        `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}catch{L(`复制失败，请手动选择内容复制`,`error`)}document.body.removeChild(r)})}catch(e){console.error(`[SidePanel] 复制失败:`,e),L(`复制失败`,`error`)}}function gr(e,t){try{let n=e.dataset.rawMarkdown||e.dataset.rawContent||``;if(!n){let t=e.querySelector(`.markdown-body`);n=t?t.innerText:e.innerText}let r=`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
    ...
    `,i=new Blob([`﻿`,r],{type:`application/msword`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`word-${new Date().getTime()}.doc`,document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(a);let s=t.innerHTML;t.innerHTML=`
      ...
      <span>已导出</span>
    `,setTimeout(()=>{t.innerHTML=s},2e3),console.log(`[SidePanel] Word 文档导出成功`)}catch(e){console.error(`[SidePanel] 导出 Word 失败:`,e),L(`导出失败: `+e.message,`error`)}}function _r(e,t){try{let n=e.dataset.rawMarkdown||e.dataset.rawContent||``;if(!n){let t=e.querySelector(`.markdown-body`);n=t?t.innerText:e.innerText}let r=`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
...
`),t.dispatchEvent(new Event(`input`)),console.log(`[SidePanel] 已移除输入框中的选中内容前缀`))}F.lastSelectedText=``,F.currentSelectionRange=null}function br(){let e=document.getElementById(`promptManageList`).querySelectorAll(`.prompt-manage-item`);e.forEach(t=>{t.addEventListener(`dragstart`,e=>{F.draggedItemIndex=parseInt(t.dataset.index),t.classList.add(`dragging`),e.dataTransfer.effectAllowed=`move`,e.dataTransfer.setData(`text/plain`,F.draggedItemIndex)}),t.addEventListener(`dragend`,()=>{t.classList.remove(`dragging`),e.forEach(e=>e.classList.remove(`drag-over`)),F.draggedItemIndex=null}),t.addEventListener(`dragover`,e=>{e.preventDefault(),e.dataTransfer.dropEffect=`move`,t.classList.add(`drag-over`)}),t.addEventListener(`dragleave`,()=>{t.classList.remove(`drag-over`)}),t.addEventListener(`drop`,e=>{e.stopPropagation(),e.preventDefault();let n=parseInt(t.dataset.index);if(F.draggedItemIndex!==null&&F.draggedItemIndex!==n){let e=F.customPrompts[F.draggedItemIndex];F.customPrompts.splice(F.draggedItemIndex,1),F.customPrompts.splice(n,0,e),chrome.storage.local.set({customPrompts:F.customPrompts}),$()}t.classList.remove(`drag-over`)})})}function xr(){let e=document.querySelector(`.input-toolbar-right`);if(!e)return;let t=document.createElement(`button`);t.className=`prompt-manage-btn`,t.title=`提示词管理`,t.innerHTML=`<svg t="1781177976746" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5076" width="16" height="16"><path d="M674.56 231.552l101.568 56.96-56.896-101.632 56.96-101.568-101.632 56.896-101.632-56.896 56.96 101.568-56.896 101.632 101.568-56.96zM186.944 629.76l-101.504-56.896 56.832 101.632-56.832 101.568 101.504-56.96 101.632 56.96-56.896-101.568 56.896-101.568-101.568 56.832zM85.44 85.312l56.832 101.568-56.832 101.632 101.504-56.96 101.632 56.96L231.68 186.88l56.896-101.568-101.568 56.896-101.568-56.896z m351.872 438.016l-99.2-99.136L424.32 337.984l99.072 99.264-86.08 86.144m-41.856-223.04L300.352 395.392a40.448 40.448 0 0 0 0 57.28l474.24 474.112a40.448 40.448 0 0 0 57.344 0l94.912-95.04a40.448 40.448 0 0 0 0-57.344L452.736 300.288a40.448 40.448 0 0 0-57.28 0z" p-id="5077" fill="#777"></path></svg>`,t.addEventListener(`click`,()=>{kr()}),e.appendChild(t)}function Sr(e=``){let t=document.getElementById(`promptSelector`),n=document.getElementById(`promptDropdown`);t.style.display=`block`,n.classList.add(`show`),Tr(e)}function Q(){let e=document.getElementById(`promptSelector`),t=document.getElementById(`promptDropdown`);e.style.display=`none`,t.classList.remove(`show`),F.selectedPromptIndex=-1}function Cr(){let e=document.getElementById(`promptSelector`),t=document.getElementById(`promptDropdown`),n=document.getElementById(`userInput`);e.style.display!==`none`&&t.classList.contains(`show`)?Q():(Sr(),n.focus())}function wr(e=``){Tr(e)}function Tr(e=``){let t=document.getElementById(`promptList`),n=e.toLowerCase(),r=F.customPrompts.filter(t=>e?t.code.toLowerCase().includes(n)||t.content.toLowerCase().includes(n):!0);if(r.length===0){t.innerHTML=`<div class="prompt-empty">暂无匹配的提示词</div>`,F.selectedPromptIndex=-1;return}F.selectedPromptIndex=0,t.innerHTML=r.map((e,t)=>`
  ...
  `).join(``),t.querySelectorAll(`.prompt-item`).forEach(e=>{e.addEventListener(`click`,t=>{let n=e.dataset.code;t.ctrlKey||t.metaKey?Dr(n):Or(n)})})}function Er(e){e.forEach((e,t)=>{t===F.selectedPromptIndex?e.classList.add(`selected`):e.classList.remove(`selected`)})}function Dr(e){let t=F.customPrompts.find(t=>t.code===e);if(!t)return;let n=document.getElementById(`userInput`),r=n.value,i=r.lastIndexOf(`/`),a=r;if(i!==-1){let e=-1;for(let t=i-1;t>=0;t--)if(r[t]===`
`){e=t;break}a=e===-1?r.substring(0,i).trimEnd():r.substring(0,e+1).trimEnd()}n.value=a+(a&&!a.endsWith(`
`)?`

`:``)+t.content,n.focus(),n.selectionStart=n.selectionEnd=n.value.length,Q(),Fe(),console.log(`[SidePanel] 已追加提示词到输入框:`,t.code,t.content)}async function Or(e){let t=F.customPrompts.find(t=>t.code===e);if(!t)return;if(F.isGenerating){console.log(`[SidePanel] 正在生成中，请稍候...`);return}Q();let n=document.getElementById(`chatContainer`).querySelector(`.welcome-message`);n&&n.remove();let r=t.content,i=F.enableSelectionQuery&&F.selectedContextText&&F.selectedContextText.trim(),a=F.quotedContextText&&F.quotedContextText.trim();if(a){let e=F.quotedContextText.trim();r=`[引用内容]\n${e}\n\n[用户问题]\n${t.content}`,Y(`quoted`,e,!1),F.quotedContextText=``}else if(i){let e=F.selectedContextText.trim();r=`[选中内容]\n${e}\n\n[用户问题]\n${t.content}`,Y(`selected`,e,!1),F.selectedContextText=``}(i||a)&&yr();let o=Wn(r);X(`user`,Wn(t.content)),F.messageHistory.push({role:`user`,content:o}),U(),We(t.content);let s=document.getElementById(`userInput`);s.value=``,s.style.height=`auto`,F.isGenerating=!0;let c=document.getElementById(`sendBtn`);c.disabled=!0;let l=ur(),u=F.activeSessionId,d=F.enableImageInput&&F.attachedImages.length>0&&F.imageModelName||F.currentModel;try{await Ve(),console.log(`[SidePanel] 发送消息调试信息:`),console.log(`  - isolateChat:`,F.isolateChat),console.log(`  - chatConfig:`,F.chatConfig),console.log(`  - messageHistory.length:`,F.messageHistory.length);let e=[{role:`system`,content:Re()}];if(F.isolateChat){let t=F.messageHistory;F.chatConfig.maxMemoryMessages!==null&&F.chatConfig.maxMemoryMessages!==void 0&&F.chatConfig.maxMemoryMessages>0?(t=[...F.messageHistory.slice(0,-1).slice(-F.chatConfig.maxMemoryMessages),F.messageHistory[F.messageHistory.length-1]],console.log(`[SidePanel] 记忆历史限制生效:`,F.chatConfig.maxMemoryMessages,`条（不含当前消息），实际发送:`,t.length,`条`)):console.log(`[SidePanel] 记忆历史限制未生效:`,F.chatConfig.maxMemoryMessages),e=[...e,...t]}else e.push({role:`user`,content:o});let t=await ze(),n,r;try{let i=await dr(e,d,F.useTools,t);n=i.content,r=i.executionLog||[]}catch(e){throw Z(l),n=`❌ 请求失败：`+(e.message||`未知错误`),r=e.executionLog||[],X(`assistant`,n,!0,r),F.messageHistory.push({role:`assistant`,content:n,executionLog:r}),U(),e}Z(l),await qt(X(`assistant`,n,!0,r)),F.messageHistory.push({role:`assistant`,content:n,executionLog:r}),U()}catch{}finally{if(F.generatingSessionIds.delete(u),c.disabled=!1,s.focus(),F.attachedImages.length>0){F.attachedImages=[];let e=document.getElementById(`imagePreviewBar`);e&&(e.innerHTML=``)}}}function kr(){document.getElementById(`promptManageModal`).classList.add(`show`),$()}function Ar(){document.getElementById(`promptManageModal`).classList.remove(`show`);let e=document.getElementById(`editPromptIndex`),t=document.getElementById(`newPromptCode`),n=document.getElementById(`newPromptContent`),r=document.getElementById(`promptManageAddBtn`);e&&(e.value=``),t&&(t.value=``),n&&(n.value=``),r&&(r.textContent=`添加提示词`,r.style.background=`#667eea`)}function $(){let e=document.getElementById(`promptManageList`);if(F.customPrompts.length===0){e.innerHTML=`<div class="prompt-empty">暂无提示词，请添加</div>`;return}e.innerHTML=F.customPrompts.map((e,t)=>`
  ...
  `).join(``),e.querySelectorAll(`.move-up-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.index);if(n>0){let e=F.customPrompts[n];F.customPrompts[n]=F.customPrompts[n-1],F.customPrompts[n-1]=e,chrome.storage.local.set({customPrompts:F.customPrompts}),$()}})}),e.querySelectorAll(`.move-down-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.index);if(n<F.customPrompts.length-1){let e=F.customPrompts[n];F.customPrompts[n]=F.customPrompts[n+1],F.customPrompts[n+1]=e,chrome.storage.local.set({customPrompts:F.customPrompts}),$()}})}),e.querySelectorAll(`.menu-toggle-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.index);F.customPrompts[n].enabledInMenu=!F.customPrompts[n].enabledInMenu,chrome.storage.local.set({customPrompts:F.customPrompts}),$()})}),e.querySelectorAll(`.edit-btn`).forEach(e=>{e.addEventListener(`click`,()=>{Pr(parseInt(e.dataset.index))})}),e.querySelectorAll(`.delete-btn`).forEach(e=>{e.addEventListener(`click`,()=>{Fr(parseInt(e.dataset.index))})}),br()}function jr(e){let t=document.getElementById(`promptErrorModal`),n=document.getElementById(`promptErrorMessage`);n.textContent=e,t.classList.add(`show`)}function Mr(){document.getElementById(`promptErrorModal`).classList.remove(`show`)}function Nr(){let e=document.getElementById(`editPromptIndex`),t=document.getElementById(`newPromptCode`),n=document.getElementById(`newPromptContent`),r=t.value.trim(),i=n.value.trim();if(!r||!i){jr(`请填写编码和内容`);return}let a=e?parseInt(e.value):-1,o=F.customPrompts.findIndex(e=>e.code===r);if(o!==-1&&o!==a){jr(`编码已存在`);return}a>=0&&a<F.customPrompts.length?F.customPrompts[a]={...F.customPrompts[a],code:r,content:i}:F.customPrompts.push({code:r,content:i,enabledInMenu:!1}),chrome.storage.local.set({customPrompts:F.customPrompts}),t.value=``,n.value=``,e&&(e.value=``);let s=document.getElementById(`promptManageAddBtn`);s.textContent=`添加提示词`,s.style.background=`#667eea`,$()}function Pr(e){let t=F.customPrompts[e];if(!t)return;let n=document.getElementById(`editPromptIndex`),r=document.getElementById(`newPromptCode`),i=document.getElementById(`newPromptContent`),a=document.getElementById(`promptManageAddBtn`);n&&(n.value=e),r.value=t.code,i.value=t.content,a.textContent=`保存修改`,a.style.background=`#ffa502`,r.scrollIntoView({behavior:`smooth`})}function Fr(e){let t=F.customPrompts[e];if(!t)return;F.pendingDeleteIndex=e;let n=document.getElementById(`deleteConfirmModal`),r=document.getElementById(`deleteConfirmMessage`);r.textContent=`确定要删除提示词 "/${t.code}" 吗？`,n.classList.add(`show`)}function Ir(){document.getElementById(`deleteConfirmModal`).classList.remove(`show`),F.pendingDeleteIndex=-1}function Lr(e){F.customPrompts.splice(e,1),chrome.storage.local.set({customPrompts:F.customPrompts}),$()}function Rr(){let e=document.getElementById(`promptManageCancelBtn`),t=document.getElementById(`promptManageAddBtn`),n=document.getElementById(`promptModalCloseBtn`);e&&e.addEventListener(`click`,Ar),t&&t.addEventListener(`click`,Nr),n&&n.addEventListener(`click`,Ar);let r=document.getElementById(`deleteCancelBtn`),i=document.getElementById(`deleteConfirmBtn`);r&&r.addEventListener(`click`,Ir),i&&i.addEventListener(`click`,()=>{F.pendingDeleteIndex>=0&&Lr(F.pendingDeleteIndex),Ir()});let a=document.getElementById(`deleteConfirmModal`);a&&a.addEventListener(`click`,e=>{e.target===a&&Ir()});let o=document.getElementById(`promptErrorConfirmBtn`);o&&o.addEventListener(`click`,Mr);let s=document.getElementById(`promptErrorModal`);s&&s.addEventListener(`click`,e=>{e.target===s&&Mr()})}function zr(){let e=document.getElementById(`toolsPopupOverlay`);if(!e)return;F.currentCategory=`all`,F.currentSearch=``;let t=document.getElementById(`toolsSearchInput`);t&&(t.value=``),Kr(),qr(),chrome.storage.local.get([`enableToolPreselect`],e=>{let t=document.getElementById(`toolsPreselectToggle`);t&&(t.checked=e.enableToolPreselect===void 0?!0:e.enableToolPreselect)}),document.querySelectorAll(`.category-btn`).forEach(e=>{e.classList.remove(`active`),e.classList.contains(`category-all`)?(e.classList.add(`active`),e.style.background=`linear-gradient(135deg, #667eea 0%, #764ba2 100%)`,e.style.color=`white`,e.style.borderColor=`transparent`):(e.style.background=`white`,e.style.color=`#555`,e.style.borderColor=`#ececec`)}),Vr(),e.classList.add(`show`),console.log(`[SidePanel] 打开工具弹窗`)}function Br(){let e=document.getElementById(`toolsPopupOverlay`);e&&(document.querySelectorAll(`.category-btn`).forEach(e=>{e.classList.remove(`active`),e.style.background=``,e.style.color=``,e.style.borderColor=``}),e.classList.remove(`show`),console.log(`[SidePanel] 关闭工具弹窗`))}function Vr(){let t=document.getElementById(`toolsPopupList`);if(!t)return;t.innerHTML=``;let n={};e.forEach(e=>{if(F.currentCategory!==`all`&&e.category!==F.currentCategory)return;if(F.currentSearch){let t=e.name.toLowerCase().includes(F.currentSearch),n=e.description.toLowerCase().includes(F.currentSearch);if(!t&&!n)return}let t=e.category||`other`;n[t]||(n[t]=[]),n[t].push(e)});let r=Pe;if(Ne.forEach(i=>{let a=n[i];if(!a||a.length===0)return;let o=e.filter(e=>e.category===i),s=o.length,c=o.filter(e=>F.enabledTools.includes(e.id)).length,l=document.createElement(`div`);l.className=`popup-tool-category-group`,l.dataset.category=i;let u=document.createElement(`div`);u.className=`popup-tool-category`,u.dataset.category=i;let d=F.collapsedCategories[i]||!1;u.innerHTML=`
      <span class="category-expand-icon">${d?`▶`:`▼`}</span>
      <span class="category-name">${r[i]||i}</span>
      <span class="category-count">${c}/${s}</span>
    `,u.addEventListener(`click`,()=>{Hr(i)}),l.appendChild(u);let f=document.createElement(`div`);f.className=`popup-tool-items ${d?`collapsed`:``}`,a.forEach(e=>{let t=F.enabledTools.includes(e.id),n=document.createElement(`div`);n.className=`popup-tool-item`,n.dataset.category=i,n.innerHTML=`
      ...
      `;let r=n.querySelector(`input[type="checkbox"]`);r&&r.addEventListener(`change`,t=>{if(t.stopPropagation(),t.target.checked)F.enabledTools.includes(e.id)||F.enabledTools.push(e.id);else{let t=F.enabledTools.indexOf(e.id);t>-1&&F.enabledTools.splice(t,1)}Ur(i),Kr(),qr()}),f.appendChild(n)}),l.appendChild(f),t.appendChild(l)}),t.children.length===0){let e=document.createElement(`div`);e.className=`popup-tool-empty`,e.textContent=`没有找到匹配的工具`,t.appendChild(e)}}function Hr(e){F.collapsedCategories[e]=!F.collapsedCategories[e];let t=document.querySelector(`.popup-tool-category-group[data-category="${e}"]`);if(!t)return;let n=t.querySelector(`.popup-tool-category`).querySelector(`.category-expand-icon`),r=t.querySelector(`.popup-tool-items`);F.collapsedCategories[e]?(n.textContent=`▶`,r.classList.add(`collapsed`)):(n.textContent=`▼`,r.classList.remove(`collapsed`))}function Ur(t){let n=document.querySelector(`.popup-tool-category[data-category="${t}"]`);if(!n)return;let r=n.querySelector(`.category-count`);if(!r)return;let i=e.filter(e=>e.category===t),a=i.length,o=0;i.forEach(e=>{let t=document.getElementById(`tool_`+e.id);t&&t.checked&&o++}),r.textContent=`${o}/${a}`}function Wr(){return e.filter(e=>{if(F.currentCategory!==`all`&&e.category!==F.currentCategory)return!1;if(F.currentSearch){let t=e.name.toLowerCase().includes(F.currentSearch.toLowerCase()),n=e.description.toLowerCase().includes(F.currentSearch.toLowerCase());if(!t&&!n)return!1}return!0})}function Gr(){Ne.forEach(e=>{Ur(e)})}function Kr(){let t=[`all`,...Ne],n=new Set(e.map(e=>e.id)),r=F.enabledTools.filter(e=>n.has(e)).length;t.forEach(t=>{let n=document.getElementById(`badge-`+t);if(!n)return;let i=0,a=0;if(t===`all`)i=e.length,a=r;else{let n=e.filter(e=>e.category===t);i=n.length,a=n.filter(e=>F.enabledTools.includes(e.id)).length}n.textContent=`${a}/${i}`})}function qr(){let t=document.getElementById(`toolsEnabledCount`);if(!t)return;let n=e.length,r=new Set(e.map(e=>e.id));t.textContent=`(已启用 ${F.enabledTools.filter(e=>r.has(e)).length}/${n})`}function Jr(){let t=[];new Set(e.map(e=>e.id)),e.forEach(e=>{let n=document.getElementById(`tool_`+e.id);n?n.checked&&t.push(e.id):F.enabledTools.includes(e.id)&&t.push(e.id)}),F.enabledTools=t,F.useTools=F.enabledTools.length>0,chrome.storage.local.set({enabledTools:F.enabledTools},()=>{console.log(`[SidePanel] 工具配置已保存:`,F.enabledTools)}),ln().catch(()=>{});let n=document.getElementById(`toolsPreselectToggle`);n&&chrome.storage.local.set({enableToolPreselect:n.checked},()=>{console.log(`[SidePanel] 工具预筛选开关已保存:`,n.checked)}),Yr(),L(F.useTools?`已启用 ${F.enabledTools.length} 个工具`:`工具已全部禁用`,`success`)}function Yr(){let t=document.getElementById(`toolsToggleBtn`),n=document.getElementById(`toolsBadge`),r=new Set(e.map(e=>e.id)),i=F.enabledTools.filter(e=>r.has(e)).length;t&&(F.useTools&&i>0?(t.classList.add(`active`),t.title=`工具 (${i}个启用)`):(t.classList.remove(`active`),t.title=`工具 (未启用)`)),n&&(i>0?(n.textContent=i,n.style.display=`inline`):n.style.display=`none`)}chrome.storage.onChanged.addListener((e,t)=>{t===`local`&&e.chatMaxMemoryMessages&&(F.chatConfig.maxMemoryMessages=e.chatMaxMemoryMessages.newValue,Xr(),console.log(`[SidePanel] 记忆限制配置已更新:`,F.chatConfig.maxMemoryMessages))});function Xr(){let e=document.getElementById(`memoryLimitLabel`);e&&(F.chatConfig.maxMemoryMessages!==null&&F.chatConfig.maxMemoryMessages!==void 0&&F.chatConfig.maxMemoryMessages>0?e.textContent=`(${F.chatConfig.maxMemoryMessages})`:e.textContent=`(全)`,e.style.display=`inline`,e.style.cursor=`pointer`,e.title=`点击设置记忆历史限制条数`)}function Zr(e){e.preventDefault(),e.stopPropagation();let t=document.getElementById(`memoryLimitDropdown`);if(t.classList.toggle(`show`),t.classList.contains(`show`)){let e=F.chatConfig.maxMemoryMessages;t.querySelectorAll(`.memory-limit-option`).forEach(t=>{t.classList.remove(`selected`);let n=parseInt(t.dataset.value);(e===null&&n===0||e!==null&&e>0&&n===e)&&t.classList.add(`selected`)});let n=t.querySelector(`#memoryLimitInput`);e!==null&&e>0?n.value=e:n.value=``}}function Qr(){let e=document.getElementById(`memoryLimitDropdown`),t=document.getElementById(`memoryLimitLabel`),n=e?.querySelector(`#memoryLimitInput`);if(!e||!t)return;Xr(),t.addEventListener(`click`,Zr);let r=e.querySelectorAll(`.memory-limit-option`);r.forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let i=parseInt(e.dataset.value),a=i===0?null:i;r.forEach(e=>e.classList.remove(`selected`)),e.classList.add(`selected`),n&&(n.value=i===0?``:i),chrome.storage.local.set({chatMaxMemoryMessages:a},()=>{F.chatConfig.maxMemoryMessages=a,Xr(),L(`✅ 配置已更新`,`success`)})})}),n&&(n.addEventListener(`click`,e=>e.stopPropagation()),n.addEventListener(`mousedown`,e=>e.stopPropagation()),n.addEventListener(`change`,e=>{e.stopPropagation();let t=e.target.value.trim(),n=t&&parseInt(t)>0?parseInt(t):null;r.forEach(e=>e.classList.remove(`selected`)),chrome.storage.local.set({chatMaxMemoryMessages:n},()=>{F.chatConfig.maxMemoryMessages=n,Xr(),L(`✅ 配置已更新`,`success`)})})),document.addEventListener(`click`,n=>{!e.contains(n.target)&&n.target!==t&&e.classList.remove(`show`)})}function $r(e){document.querySelectorAll(`.model-option`).forEach(t=>{t.dataset.value===e?(t.classList.add(`selected`),t.querySelector(`.model-option-check`).textContent=`✓`):(t.classList.remove(`selected`),t.querySelector(`.model-option-check`).textContent=``)})}function ei(e,t){let n=document.getElementById(`tempDropdown`);if(!n){typeof t==`function`&&t();return}chrome.storage.local.get([`deletedPresetModels`],r=>{if((r.deletedPresetModels||[]).forEach(e=>{let t=n.querySelector(`.model-option[data-value="${e}"]`);t&&t.remove()}),!e||e.length===0){typeof t==`function`&&t();return}let i=[`deepseek-v4-pro`,`deepseek-v4-flash`];e.forEach(e=>{if(i.includes(e)||n.querySelector(`.model-option[data-value="${e}"]`))return;let t=document.createElement(`div`);t.className=`model-option`,t.dataset.value=e,t.innerHTML=`<span class="model-option-check"></span>${e}`,t.addEventListener(`click`,t=>{t.stopPropagation(),F.currentModel=e,$r(e),chrome.storage.local.set({modelName:e})}),n.querySelector(`.model-section`).appendChild(t)}),typeof t==`function`&&t()})}function ti(e,t=`📌 已选中`){if(!F.enableSelectionQuery)return;F.quotedContextText=``,F.selectedContextText=e;let n=document.getElementById(`selectionIndicator`),r=document.getElementById(`selectionText`),i=document.getElementById(`userInput`);if(n&&r&&i){let i;i=e.length>100?e.substring(0,100)+`...`:e.length>50?e.substring(0,50)+`...`:e,r.textContent=`${t}: ${i}`,n.classList.add(`show`)}}function ni(e,t,n=0,r=0){if(!F.enableSelectionQuery)return;let i=document.getElementById(`selectionFloatingMenu`),a=document.getElementById(`selectionMenuItems`);if(!i||!a)return;let o=F.customPrompts.filter(e=>e.enabledInMenu===!0);if(o.length===0)return;a.innerHTML=``,o.forEach(e=>{let n=e.content.length>10?e.content.substring(0,10)+`...`:e.content,r=document.createElement(`div`);r.className=`menu-item`,r.innerHTML=`
      <span>${n}</span>
      <span class="menu-item-code">/${e.code}</span>
    `,r.addEventListener(`click`,n=>{n.stopPropagation(),ri(e,t)}),a.appendChild(r)});let s=document.body.getBoundingClientRect(),c=40+o.length*36,l=r-s.top-c-30,u=n-s.left-20;l<s.top+10&&(l=r-s.top+30),u<s.left+10&&(u=n-s.left+20),u+180>s.right-10&&(u=n-s.left-180-30,u<s.left+10&&(u=n-s.left+20)),l+c>s.bottom-10&&(l=r-s.top-c-30,l<s.top+10&&(l=r-s.top+30)),i.style.top=l+`px`,i.style.left=u+`px`,i.style.maxHeight=s.bottom-l-20+`px`,i.classList.add(`show`)}window.hideFloatingMenu=function(){let e=document.getElementById(`selectionFloatingMenu`);e&&e.classList.remove(`show`),F.lastSelectedText=``,F.currentSelectionRange=null};async function ri(e,t){if(!F.enableSelectionQuery)return;if(window.hideFloatingMenu(),F.isGenerating){console.log(`[SidePanel] 正在生成中，请稍候...`);return}F.selectedContextText=t,kn();let n=document.getElementById(`chatContainer`).querySelector(`.welcome-message`);n&&n.remove(),Y(`selected`,t,!1);let r=`[选中内容]\n${t}\n\n[用户问题]\n${e.content}`;X(`user`,e.content),F.messageHistory.push({role:`user`,content:r}),U(),We(e.content),F.isGenerating=!0;let i=document.getElementById(`sendBtn`);i.disabled=!0;let a=ur(),o=F.activeSessionId,s=F.currentModel;try{await Ve(),console.log(`[SidePanel] 发送消息调试信息:`),console.log(`  - isolateChat:`,F.isolateChat),console.log(`  - chatConfig:`,F.chatConfig),console.log(`  - messageHistory.length:`,F.messageHistory.length);let e=[{role:`system`,content:Re()}];if(F.isolateChat){let t=F.messageHistory;F.chatConfig.maxMemoryMessages!==null&&F.chatConfig.maxMemoryMessages!==void 0&&F.chatConfig.maxMemoryMessages>0?(t=[...F.messageHistory.slice(0,-1).slice(-F.chatConfig.maxMemoryMessages),F.messageHistory[F.messageHistory.length-1]],console.log(`[SidePanel] 记忆历史限制生效:`,F.chatConfig.maxMemoryMessages,`条（不含当前消息），实际发送:`,t.length,`条`)):console.log(`[SidePanel] 记忆历史限制未生效:`,F.chatConfig.maxMemoryMessages),e=[...e,...t]}else e.push({role:`user`,content:r});let t=await ze(),n,i;try{let r=await dr(e,s,F.useTools,t);n=r.content,i=r.executionLog||[]}catch(e){throw Z(a),n=`❌ 请求失败：`+(e.message||`未知错误`),i=e.executionLog||[],X(`assistant`,n,!0,i),F.messageHistory.push({role:`assistant`,content:n,executionLog:i}),U(),e}Z(a),await qt(X(`assistant`,n,!0,i)),F.messageHistory.push({role:`assistant`,content:n,executionLog:i}),U()}catch{}finally{F.generatingSessionIds.delete(o),i.disabled=!1,document.getElementById(`userInput`).focus()}}function ii(e){let t=document.getElementById(`headerAgentDot`),n=document.getElementById(`headerAgentIndicator`);if(!(!t||!n))if(!e||!e.connected)t.className=`header-agent-dot disconnected`,n.title=`Agent 未连接 - 点击前往设置`;else{t.className=`header-agent-dot connected`;let r=[`Agent 已连接`];e.platformName&&r.push(e.platformName),e.arch&&r.push(e.arch),n.title=r.join(` | `)+` - 点击前往设置`}}document.addEventListener(`DOMContentLoaded`,async()=>{window.__tableBlocks=[],await He(),await En(),chrome.runtime.onMessage.addListener(e=>{e.type===`SELECTION_AI_SEARCH`&&e.prompt&&(console.log(`[SidePanel] 收到选中文本 AI 搜索:`,e.selectedText?.substring(0,50)),Kn(e.prompt,e.selectedText),chrome.storage.session.remove(`pendingSelectionSearch`).catch(()=>{})),e.type===`FILL_SIDEPANEL_INPUT`&&e.text&&(console.log(`[SidePanel] 收到追问填充:`,e.text?.substring(0,50)),qn(e.text),chrome.storage.session.remove(`pendingFillInput`).catch(()=>{})),e.type===`DIRECT_SEND`&&e.text&&(console.log(`[SidePanel] 收到直接发送:`,e.text?.substring(0,50)),Jn(e.text,e.selectedText||``),chrome.storage.session.remove(`pendingDirectSend`).catch(()=>{})),e.type===`AGENT_STATUS_CHANGE`&&(console.log(`[SidePanel] 收到 Agent 状态变化:`,e.connected,e.status),chrome.storage.local.get(`agentPlatform`,e=>{F.agentPlatform=e.agentPlatform||{connected:!1},ii(F.agentPlatform)})),e.type===`AGENT_CONNECTION_CHANGED`&&(console.log(`[SidePanel] 收到 Agent 连接状态变更:`,e.connected),F.agentPlatform={...F.agentPlatform,connected:e.connected},ii(F.agentPlatform))});let t=await chrome.storage.session.get(`pendingSelectionSearch`);if(t.pendingSelectionSearch&&t.pendingSelectionSearch.selectedText){let{prompt:e,selectedText:n}=t.pendingSelectionSearch;console.log(`[SidePanel] 有待处理的选中文本搜索:`,n?.substring(0,50)),setTimeout(()=>{Kn(e,n)},500),await chrome.storage.session.remove(`pendingSelectionSearch`)}let n=await chrome.storage.session.get(`pendingFillInput`);if(n.pendingFillInput&&n.pendingFillInput.text){let{text:e}=n.pendingFillInput;console.log(`[SidePanel] 有待填充的追问文本:`,e?.substring(0,50)),setTimeout(()=>{qn(e)},500),await chrome.storage.session.remove(`pendingFillInput`)}let r=await chrome.storage.session.get(`pendingDirectSend`);if(r.pendingDirectSend&&r.pendingDirectSend.text){let{text:e,selectedText:t}=r.pendingDirectSend;console.log(`[SidePanel] 有待直接发送的文本:`,e?.substring(0,50)),setTimeout(()=>{Jn(e,t||``)},500),await chrome.storage.session.remove(`pendingDirectSend`)}chrome.tabs.onActivated.addListener(async e=>{console.log(`[SidePanel] Tab 切换, 新 Tab ID:`,e.tabId),F.currentTabId=e.tabId}),chrome.tabs.onUpdated.addListener(async(e,t)=>{t.status===`complete`&&F.currentTabId===e&&console.log(`[SidePanel] 当前 Tab 页面更新:`,t)}),typeof marked<`u`&&(marked.setOptions({breaks:!0,gfm:!0}),console.log(`[SidePanel] Marked 库已加载`)),typeof mermaid<`u`&&(mermaid.initialize({startOnLoad:!1,theme:`default`,securityLevel:`loose`,fontFamily:`-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif`}),console.log(`[SidePanel] Mermaid 库已加载`));let i=document.getElementById(`userInput`),a=document.getElementById(`sendBtn`),o=document.getElementById(`clearChatBtn`),s=document.getElementById(`exportChatBtn`),c=document.getElementById(`chatContainer`),l=document.getElementById(`tempDisplay`),u=document.getElementById(`tempDropdown`),d=document.getElementById(`tempPresetList`),f=document.getElementById(`tempSlider`),p=document.getElementById(`tempNumberInput`);function m(){!i||F.isScrolling||(i.style.height=`auto`,i.style.height=Math.min(i.scrollHeight,100)+`px`)}chrome.storage.local.get([`temperature`,`topP`,`selectedTempIndex`],e=>{e.temperature!==void 0&&(F.temperature=e.temperature),e.topP!==void 0&&(F.topP=e.topP),e.selectedTempIndex!==void 0&&(F.selectedTempIndex=e.selectedTempIndex),h()});function h(){f&&(f.value=F.temperature),p&&(p.value=F.temperature.toFixed(2));let e=document.getElementById(`tempIconValue`);e&&(e.textContent=F.temperature.toFixed(2)),g()}function g(){d.innerHTML=I.map((e,t)=>`
...
`||t[n-1]===` `?Sr(e):wr(e)}else Q();m()}),c.addEventListener(`scroll`,()=>{let e=`scrollPosition_`+(F.activeSessionId||`default`);chrome.storage.local.set({[e]:c.scrollTop})});let ne=document.getElementById(`headerMoreBtn`),D=document.getElementById(`headerMoreDropdown`);ne&&D&&(ne.addEventListener(`click`,e=>{e.stopPropagation(),D.classList.toggle(`show`)}),document.addEventListener(`click`,e=>{!D.contains(e.target)&&e.target!==ne&&D.classList.remove(`show`)})),o.addEventListener(`click`,e=>{e.stopPropagation(),D.classList.remove(`show`),Nn()}),s&&s.addEventListener(`click`,e=>{e.stopPropagation(),D.classList.remove(`show`),Mn()});let re=document.getElementById(`settingsBtn`);re&&re.addEventListener(`click`,()=>{chrome.runtime.openOptionsPage()});let ie=document.getElementById(`headerAgentIndicator`);ie&&ie.addEventListener(`click`,async()=>{let e=chrome.runtime.getURL(`options.html#agent`),t=await chrome.tabs.query({url:chrome.runtime.getURL(`options.html`)});t.length>0?await chrome.tabs.update(t[0].id,{active:!0,url:e}):await chrome.tabs.create({url:e})});let ae=document.getElementById(`prototypeLibraryBtn`);ae&&ae.addEventListener(`click`,e=>{e.stopPropagation(),D.classList.remove(`show`),St()});let oe=document.getElementById(`isolateChatBtn`),O=document.getElementById(`enableToolsBtn`),se=document.getElementById(`toolsConfigBtn`);chrome.storage.local.get([`isolateChat`,`enableSelectionQuery`,`enableTools`,`enabledTools`],t=>{t.isolateChat!==void 0&&(F.isolateChat=t.isolateChat),oe.checked=F.isolateChat,t.enableSelectionQuery!==void 0&&(F.enableSelectionQuery=t.enableSelectionQuery);let n=document.getElementById(`enableSelectionQueryBtn`);if(n&&(n.checked=F.enableSelectionQuery),t.enableTools!==void 0&&(F.useTools=t.enableTools),t.enabledTools&&t.enabledTools.length>0){let n=new Set(e.map(e=>e.id)),r=t.enabledTools.filter(e=>n.has(e)),i=e.filter(e=>e.enabled&&!r.includes(e.id)).map(e=>e.id);F.enabledTools=[...r,...i],i.length>0&&chrome.storage.local.set({enabledTools:F.enabledTools})}else F.enabledTools=e.filter(e=>e.enabled).map(e=>e.id);F.enabledTools.length===0&&(F.useTools=!1),O&&(O.checked=F.useTools),x()}),oe.addEventListener(`change`,()=>{F.isolateChat=oe.checked,chrome.storage.local.set({isolateChat:F.isolateChat}),console.log(`[SidePanel] 记忆对话:`,F.isolateChat?`已启用`:`已禁用`)});let ce=document.getElementById(`enableSelectionQueryBtn`);ce&&ce.addEventListener(`change`,()=>{F.enableSelectionQuery=ce.checked,chrome.storage.local.set({enableSelectionQuery:F.enableSelectionQuery}),console.log(`[SidePanel] 划词问答:`,F.enableSelectionQuery?`已启用`:`已禁用`),!F.enableSelectionQuery&&F.selectedContextText&&kn()}),O&&O.addEventListener(`change`,()=>{F.useTools=O.checked,chrome.storage.local.set({enableTools:F.useTools}),F.useTools&&F.enabledTools.length===0&&(F.enabledTools=e.filter(e=>e.enabled).map(e=>e.id),chrome.storage.local.set({enabledTools:F.enabledTools})),console.log(`[SidePanel] 工具总开关:`,F.useTools?`已启用`:`已禁用`)}),se&&se.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),zr()});let le=document.getElementById(`toolsPopupOverlay`),ue=document.getElementById(`toolsPopupClose`),de=le?le.querySelector(`.modal-container`):null;ue&&ue.addEventListener(`click`,Br),de&&de.addEventListener(`click`,e=>{e.stopPropagation()});let fe=document.getElementById(`toolsSearchInput`);fe&&fe.addEventListener(`input`,e=>{F.currentSearch=e.target.value.toLowerCase(),Vr()});let pe=document.querySelectorAll(`.category-btn`);pe.forEach(e=>{e.addEventListener(`click`,()=>{pe.forEach(e=>{e.classList.remove(`active`),e.classList.contains(`category-all`)?(e.style.background=`#f5f3ff`,e.style.color=`#667eea`,e.style.borderColor=`#d4cfff`):(e.style.background=`white`,e.style.color=`#555`,e.style.borderColor=`#ececec`)}),e.classList.add(`active`),e.style.background=`linear-gradient(135deg, #667eea 0%, #764ba2 100%)`,e.style.color=`white`,e.style.borderColor=`transparent`,F.currentCategory=e.dataset.category,Vr()})});let k=document.getElementById(`toolsCategories`);k&&k.addEventListener(`wheel`,e=>{e.preventDefault(),k.scrollLeft+=e.deltaY*2},{passive:!1});let me=document.getElementById(`toolsSelectAll`),A=document.getElementById(`toolsSelectNone`);me&&me.addEventListener(`click`,()=>{Wr().forEach(e=>{let t=document.getElementById(`tool_`+e.id);t&&(t.checked=!0),F.enabledTools.includes(e.id)||F.enabledTools.push(e.id)}),Gr(),Kr(),qr()}),A&&A.addEventListener(`click`,()=>{Wr().forEach(e=>{let t=document.getElementById(`tool_`+e.id);t&&(t.checked=!1);let n=F.enabledTools.indexOf(e.id);n>-1&&F.enabledTools.splice(n,1)}),Gr(),Kr(),qr()});let he=document.getElementById(`toolsPopupSave`);he&&he.addEventListener(`click`,()=>{Jr(),qr()});let j=document.getElementById(`toolsPreselectToggle`);j&&j.addEventListener(`change`,()=>{chrome.storage.local.set({enableToolPreselect:j.checked},()=>{console.log(`[SidePanel] 工具预筛选开关已更新:`,j.checked)})});let ge=document.getElementById(`toolsPopupCancel`);ge&&ge.addEventListener(`click`,()=>{Br()});let _e=document.getElementById(`modalCancelBtn`),ve=document.getElementById(`modalConfirmBtn`);function ye(e,t=`确认操作`){return new Promise(n=>{let r=document.getElementById(`customConfirmOverlay`),i=document.getElementById(`customConfirmTitle`),a=document.getElementById(`customConfirmMessage`),o=document.getElementById(`customConfirmCancelBtn`),s=document.getElementById(`customConfirmOkBtn`);if(!r||!i||!a||!o||!s){n(confirm(e));return}let c=()=>{r.style.display=`none`,s.removeEventListener(`click`,l),o.removeEventListener(`click`,u),r.removeEventListener(`click`,d)},l=()=>{c(),n(!0)},u=()=>{c(),n(!1)},d=e=>{e.target===r&&(c(),n(!1))};i.textContent=t,a.textContent=e,r.style.display=`flex`,s.addEventListener(`click`,l),o.addEventListener(`click`,u),r.addEventListener(`click`,d)})}let M=document.getElementById(`toolStatsOverlay`),be=document.getElementById(`toolStatsClose`),xe=document.getElementById(`toolStatsBtn`);function Se(){M&&(M.style.display=`flex`,P())}function Ce(){M&&(M.style.display=`none`)}xe&&xe.addEventListener(`click`,e=>{e.stopPropagation(),Se()}),be&&be.addEventListener(`click`,Ce),M&&M.addEventListener(`click`,e=>{e.target===M&&Ce()});let we=document.getElementById(`toolStatsRefreshBtn`);we&&we.addEventListener(`click`,P);let Te=document.getElementById(`toolStatsClearBtn`);Te&&Te.addEventListener(`click`,async()=>{await ye(`确定要清空所有工具使用统计吗？此操作不可撤销。`,`清空统计`)&&(await chrome.storage.local.remove([`toolUsageStats`]),P())});let N={column:`callCount`,asc:!1};async function P(){let t=document.getElementById(`toolStatsTable`),n=document.getElementById(`toolStatsTableBody`),r=document.getElementById(`toolStatsLoading`),i=document.getElementById(`toolStatsEmpty`),a=document.getElementById(`toolStatsSummary`),o=document.getElementById(`toolStatsUnusedSection`),s=document.getElementById(`toolStatsUnusedList`);if(!(!t||!n||!r||!i)){t.style.display=`none`,i.style.display=`none`,o&&(o.style.display=`none`),a&&(a.textContent=``),r.style.display=``;try{let n=(await chrome.storage.local.get([`toolUsageStats`])).toolUsageStats||{},c=Object.entries(n);if(c.length===0){r.style.display=`none`,i.style.display=``;return}let l={};e.forEach(e=>{l[e.id]=e.name?`${e.name}：${e.description||``}`:e.description||e.id}),Ee(c,l);let u=e.map(e=>e.id),d=new Set(c.map(([e])=>e)),f=u.filter(e=>!d.has(e)),p=c.length,m=f.length;a&&(a.textContent=`已使用 ${p} 个，未使用 ${m} 个`),o&&s&&m>0&&(s.innerHTML=f.sort((e,t)=>e.toLowerCase().localeCompare(t.toLowerCase())).map(e=>`<code title="${R(l[e]||e)}" style="padding: 3px 10px; background: #f5f5f5; color: #aaa; border: 1px solid #eee; border-radius: 4px; font-size: 11px;">${e}</code>`).join(``),o.style.display=``),r.style.display=`none`,t.style.display=``}catch(e){console.error(`[SidePanel] 加载统计失败:`,e),r.style.display=`none`,i.textContent=`加载失败`,i.style.display=``}}}function Ee(e,t){let n=document.getElementById(`toolStatsTableBody`);if(!n)return;let{column:r,asc:i}=N;n.innerHTML=[...e].sort((e,t)=>{let[n,a]=e,[o,s]=t,c=a.callCount>0?a.successCount/a.callCount*100:0,l=s.callCount>0?s.successCount/s.callCount*100:0,u=a.callCount>0?a.totalDuration/a.callCount:0,d=s.callCount>0?s.totalDuration/s.callCount:0,f=0;switch(r){case`name`:f=n.toLowerCase().localeCompare(o.toLowerCase());break;case`callCount`:f=a.callCount-s.callCount;break;case`successCount`:f=a.successCount-s.successCount;break;case`successRate`:f=c-l;break;case`duration`:f=u-d;break}return i?f:-f}).map(([e,n])=>{let r=n.callCount>0?n.successCount/n.callCount*100:0,i=n.callCount>0?n.totalDuration/n.callCount:0,a=t[e]||e,o=`#38a169`;r<60?o=`#e53e3e`:r<85&&(o=`#d69e2e`);let s=i<1e3?`${Math.round(i)}ms`:`${(i/1e3).toFixed(1)}s`;return`<tr>
      ...
      </tr>`}).join(``),De()}function De(){let{column:e,asc:t}=N,n=[`name`,`callCount`,`successCount`,`successRate`,`duration`],r={name:`sortByName`,callCount:`sortByCallCount`,successCount:`sortBySuccessCount`,successRate:`sortBySuccessRate`,duration:`sortByDuration`};n.forEach(n=>{let i=document.getElementById(r[n]);if(!i)return;let a=i.querySelector(`.sort-indicator`);a&&(n===e?(a.textContent=t?`▲`:`▼`,a.style.color=`#667eea`):(a.textContent=``,a.style.color=``))})}document.querySelectorAll(`#toolStatsTable th[data-sort]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.sort;N.column===t?N.asc=!N.asc:(N.column=t,N.asc=!1),P()})}),_e.addEventListener(`click`,()=>{Pn()}),ve.addEventListener(`click`,()=>{Pn(),jn()});let Oe=document.getElementById(`confirmModal`);Oe.addEventListener(`click`,e=>{e.target===Oe&&Pn()});let ke=document.getElementById(`selectionClose`);ke&&ke.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),console.log(`[SidePanel] 用户点击关闭选中内容按钮`),kn(),window.hideFloatingMenu(),F.lastSelectedText=``,F.currentSelectionRange=null}),i.addEventListener(`input`,()=>{})}),Be().then(()=>Xr()),document.addEventListener(`DOMContentLoaded`,()=>{Qr()}),document.addEventListener(`DOMContentLoaded`,Ke),document.addEventListener(`DOMContentLoaded`,Rr),document.addEventListener(`DOMContentLoaded`,st),document.addEventListener(`DOMContentLoaded`,dt),document.addEventListener(`DOMContentLoaded`,It);function ai(){let e=document.getElementById(`imagePreviewBar`),t=document.getElementById(`screenshotBtn`);e&&(e.style.display=F.attachedImages.length>0?``:`none`),t&&(F.enableImageInput?t.style.removeProperty(`display`):t.style.display=`none`),F.enableImageInput||(F.attachedImages=[]),oi()}function oi(){let e=document.getElementById(`imagePreviewBar`);e&&(e.innerHTML=``,F.attachedImages.forEach((t,n)=>{let r=document.createElement(`div`);r.className=`image-preview-item`;let i=document.createElement(`img`);i.src=t.dataUrl,i.className=`image-preview-thumb`,i.title=`点击查看大图`,i.style.cursor=`zoom-in`,i.addEventListener(`click`,()=>{Bn(t.dataUrl)});let a=document.createElement(`button`);a.className=`image-preview-remove`,a.innerHTML=`×`,a.title=`移除图片`,a.addEventListener(`click`,e=>{e.stopPropagation(),F.attachedImages.splice(n,1),oi()}),r.appendChild(i),r.appendChild(a),e.appendChild(r)}))}
//# sourceMappingURL=side_panel-BuVRQd3-.js.map