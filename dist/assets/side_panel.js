...
`).trim()}}async function ot(){let e=await z();if(e.activeSessionId&&e.list.length>0){P.activeSessionId=e.activeSessionId,P.sessions=e.list;let t=e.list.find(t=>t.id===e.activeSessionId);t&&(P.messageHistory=t.messageHistory||[],P.currentModel=t.model||P.currentModel,P.useTools=t.useTools===void 0?P.useTools:t.useTools,P.enabledTools=t.enabledTools||P.enabledTools,P.temperature=t.temperature===void 0?P.temperature:t.temperature,P.topP=t.topP===void 0?P.topP:t.topP),P.messageHistory.forEach(e=>{q(e.role,e.content,!1,e.executionLog||[])});let n=document.querySelector(`.welcome-message`);n&&P.messageHistory.length>0&&n.remove(),Ie(),chrome.storage.local.get([`scrollPosition`],e=>{e.scrollPosition!==void 0&&setTimeout(()=>{let t=document.getElementById(`chatContainer`);t.scrollTop=e.scrollPosition},100)}),H()}else{await Ke();let e=await z();e.activeSessionId&&(P.activeSessionId=e.activeSessionId,P.sessions=e.list),H()}}function W(){try{V().catch(e=>{console.error(`[SidePanel] 保存当前会话失败:`,e)})}catch(e){console.error(`[SidePanel] 保存对话历史异常:`,e)}}function st(){P.messageHistory&&P.messageHistory.length>0&&Xe().then(()=>{P.messageHistory=[];let e=document.getElementById(`chatContainer`);if(e){e.innerHTML=``;let t=document.createElement(`div`);t.className=`welcome-message`,t.innerHTML=`
  ...
  `,e.appendChild(n),e.scrollTop=e.scrollHeight;let r=n.querySelector(`.stop-task-btn`),i=n.querySelector(`.loading-text`);r&&r.addEventListener(`click`,e=>{e.stopPropagation(),r.disabled=!0,r.style.opacity=`0.6`,r.style.cursor=`not-allowed`,i&&(i.textContent=`停止中...`),chrome.runtime.sendMessage({type:`CANCEL_REACT`,tabId:null,sessionId:P.activeSessionId})}),P.executionLogListener=(e,n,r)=>e.sessionId&&e.sessionId!==P.activeSessionId?!1:e.type===`EXECUTION_STATUS_UPDATE`?(console.log(`[SidePanel] 收到执行状态更新:`,e.nodeName,e.status,`日志数量:`,e.executionLog?.length),Ct(t,e.nodeName,e.status,e.executionLog),!1):!1,chrome.runtime.onMessage.addListener(P.executionLogListener),chrome.storage.local.get(`enableExecutionLog`,e=>{if(e.enableExecutionLog){let e=n.querySelector(`.execution-status-container`);e&&(e.style.display=`flex`)}});let a=n.querySelector(`.execution-log-toggle-btn`);return a&&a.addEventListener(`click`,e=>{e.stopPropagation(),St(t)}),t}function J(e){let t=document.getElementById(e);if(t){let e=t.querySelector(`.loading-text`);e&&(e.textContent=`思考中...`),t.remove()}P.executionLogListener&&=(chrome.runtime.onMessage.removeListener(P.executionLogListener),null),P.currentExecutionStatus=null;let n=document.querySelector(`.execution-log-panel.realtime-mode`);n&&n.remove()}async function Tt(e,t,n=!1,r={}){let i=(await pe()).loopTimeout;return new Promise((a,o)=>{let s=[],c=Math.round(i/1e3),l=setTimeout(()=>{g(),chrome.runtime.sendMessage({type:`CANCEL_REACT`,tabId:P.currentTabId,sessionId:P.activeSessionId}).catch(e=>{console.log(`[SidePanel] 发送取消请求失败:`,e.message)}),o({message:`请求超时（${c}秒）`,executionLog:s})},i),u=Date.now(),d=0,f=null,p=()=>{f===null&&l!==null&&(f=Date.now(),clearTimeout(l),l=null,console.log(`[SidePanel] 前端超时已暂停（澄清工具执行中）`))},m=()=>{if(f!==null){let e=Date.now()-f;d+=e,f=null;let t=Date.now()-u,n=i+d-t;if(n<=0){g(),o({message:`请求超时（${c}秒）`,executionLog:s});return}l=setTimeout(()=>{g(),chrome.runtime.sendMessage({type:`CANCEL_REACT`,tabId:P.currentTabId,sessionId:P.activeSessionId}).catch(e=>{console.log(`[SidePanel] 发送取消请求失败:`,e.message)}),o({message:`请求超时（${c}秒）`,executionLog:s})},n),console.log(`[SidePanel] 前端超时已恢复，暂停时长:`,Math.round(e/1e3),`s，剩余时间:`,Math.round(n/1e3),`s`)}},h=e=>(console.log(`[SidePanel] 收到消息:`,e),e.sessionId&&e.sessionId!==P.activeSessionId?!1:e.type===`EXECUTION_STATUS_UPDATE`?(s=e.executionLog||[],!1):e.type===`CLARIFY_START`?(p(),!1):e.type===`CLARIFY_END`?(m(),!1):e.type===`API_COMPLETE`?(l&&clearTimeout(l),chrome.runtime.onMessage.removeListener(h),a({content:e.content,executionLog:e.executionLog||s}),!1):e.type===`API_ERROR`?(l&&clearTimeout(l),chrome.runtime.onMessage.removeListener(h),o({message:e.error,executionLog:e.executionLog||s}),!1):!1);chrome.runtime.onMessage.addListener(h);let g=()=>{chrome.runtime.onMessage.removeListener(h)};console.log(`[SidePanel] 发送 CALL_API 消息，useTools:`,n,`tabId:`,P.currentTabId,`sessionId:`,P.activeSessionId,`apiParams:`,r,`timeout:`,i),chrome.runtime.sendMessage({type:`CALL_API`,sessionId:P.activeSessionId,messages:e,model:t,useTools:n,tabId:P.currentTabId,apiParams:r})})}function Et(e){let t=document.querySelector(`.execution-log-panel`);t&&t.remove();let n=document.createElement(`div`);n.className=`execution-log-panel`;let r=e.reduce((e,t)=>e+(t.duration||0),0),i=e.filter(e=>e.status===`success`).length,a=e.filter(e=>e.status===`failed`).length,o=e.filter(e=>e.nodeType===`subtask`).length,s=e.filter(e=>e.nodeType===`subtask`&&e.status===`success`).length;n.innerHTML=`
    <div class="log-container">
      <div class="log-header">
        <div class="log-title">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
  ...
  `,document.body.appendChild(n),n.addEventListener(`click`,e=>{e.target===n&&n.remove()}),n.querySelector(`.log-close`).addEventListener(`click`,()=>{n.remove()});let c=n.querySelector(`.toggle-expand-btn`),l=n.querySelectorAll(`.timeline-content`),u=!1;c.addEventListener(`click`,()=>{u=!u,l.forEach(e=>{u?e.classList.add(`expanded`):e.classList.remove(`expanded`)});let e=c.querySelector(`svg`);u?(e.innerHTML=`<polyline points="17 11 12 6 7 11"></polyline><polyline points="17 18 12 13 7 18"></polyline>`,c.setAttribute(`title`,`收起全部节点`)):(e.innerHTML=`<polyline points="7 13 12 18 17 13"></polyline><polyline points="7 6 12 11 17 6"></polyline>`,c.setAttribute(`title`,`展开全部节点`))}),n.querySelectorAll(`.timeline-header`).forEach(e=>{e.addEventListener(`click`,()=>{e.parentElement.classList.toggle(`expanded`)})});let d=n.querySelectorAll(`.combo-stat`),f=n.querySelectorAll(`.timeline-item`);d.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.status,n=e.classList.contains(`active`);d.forEach(e=>e.classList.remove(`active`)),n?f.forEach(e=>{e.style.display=``}):(e.classList.add(`active`),f.forEach(e=>{if(t===`subtask`)e.classList.contains(`subtask-level`)?e.style.display=``:e.style.display=`none`;else{let n=e.querySelector(`.timeline-dot`);n&&n.classList.contains(t)?e.style.display=``:e.style.display=`none`}}))})})}function Dt(e,t){try{let n=e.dataset.rawContent||``;navigator.clipboard.writeText(n).then(()=>{let e=t.innerHTML;t.innerHTML=`
      ...
      `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}).catch(e=>{console.error(`[SidePanel] 复制失败:`,e);let r=document.createElement(`textarea`);r.value=n,r.style.position=`fixed`,r.style.left=`-999999px`,document.body.appendChild(r),r.select();try{document.execCommand(`copy`);let e=t.innerHTML;t.innerHTML=`
        ...
        `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}catch{I(`复制失败`,`error`)}document.body.removeChild(r)})}catch(e){console.error(`[SidePanel] 复制失败:`,e),I(`复制失败`,`error`)}}function Ot(e){try{let t=e.dataset.rawContent||``;if(!t){I(`无法获取消息内容`,`error`);return}let n=document.getElementById(`userInput`);n.value=t,oe(),n.focus(),n.selectionStart=n.selectionEnd=n.value.length,console.log(`[SidePanel] 已加载消息内容到输入框，等待用户编辑后发送`)}catch(e){console.error(`[SidePanel] 编辑消息失败:`,e),I(`编辑失败: `+e.message,`error`)}}function kt(e,t){try{let n=e.dataset.rawMarkdown||e.dataset.rawContent||``;if(!n){let t=P.messageHistory.find(e=>e.role===`assistant`);if(t)n=t.content;else{let t=e.querySelector(`.markdown-body`);n=t?t.innerText:e.innerText}}navigator.clipboard.writeText(n).then(()=>{let e=t.innerHTML;t.innerHTML=`
        <svg viewBox="0 0 16 16" fill="currentColor">
          <path d="M13.78 4.22a.75.75 0 0 1 0 1.06l-7.25 7.25a.75.75 0 0 1-1.06 0L2.22 9.28a.75.75 0 0 1 1.06-1.06L6 10.94l6.72-6.72a.75.75 0 0 1 1.06 0z"/>
        </svg>
        <span>已复制</span>
      `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}).catch(e=>{let r=document.createElement(`textarea`);r.value=n,r.style.position=`fixed`,r.style.left=`-999999px`,document.body.appendChild(r),r.select();try{document.execCommand(`copy`);let e=t.innerHTML;t.innerHTML=`
          ...
          <span>已复制</span>
        `,t.classList.add(`copied`),setTimeout(()=>{t.innerHTML=e,t.classList.remove(`copied`)},2e3)}catch{I(`复制失败，请手动选择内容复制`,`error`)}document.body.removeChild(r)})}catch(e){console.error(`[SidePanel] 复制失败:`,e),I(`复制失败`,`error`)}}function At(e,t){try{let n=e.dataset.rawMarkdown||e.dataset.rawContent||``;if(!n){let t=e.querySelector(`.markdown-body`);n=t?t.innerText:e.innerText}let r=`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
    ...
    `,i=new Blob([`﻿`,r],{type:`application/msword`}),a=URL.createObjectURL(i),o=document.createElement(`a`);o.href=a,o.download=`word-${new Date().getTime()}.doc`,document.body.appendChild(o),o.click(),document.body.removeChild(o),URL.revokeObjectURL(a);let s=t.innerHTML;t.innerHTML=`
      ...
      <span>已导出</span>
    `,setTimeout(()=>{t.innerHTML=s},2e3),console.log(`[SidePanel] Word 文档导出成功`)}catch(e){console.error(`[SidePanel] 导出 Word 失败:`,e),I(`导出失败: `+e.message,`error`)}}function jt(e,t){try{let n=e.dataset.rawMarkdown||e.dataset.rawContent||``;if(!n){let t=e.querySelector(`.markdown-body`);n=t?t.innerText:e.innerText}let r=`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
...
`),t.dispatchEvent(new Event(`input`)),console.log(`[SidePanel] 已移除输入框中的选中内容前缀`))}P.lastSelectedText=``,P.currentSelectionRange=null}function Pt(){let e=document.getElementById(`promptManageList`).querySelectorAll(`.prompt-manage-item`);e.forEach(t=>{t.addEventListener(`dragstart`,e=>{P.draggedItemIndex=parseInt(t.dataset.index),t.classList.add(`dragging`),e.dataTransfer.effectAllowed=`move`,e.dataTransfer.setData(`text/plain`,P.draggedItemIndex)}),t.addEventListener(`dragend`,()=>{t.classList.remove(`dragging`),e.forEach(e=>e.classList.remove(`drag-over`)),P.draggedItemIndex=null}),t.addEventListener(`dragover`,e=>{e.preventDefault(),e.dataTransfer.dropEffect=`move`,t.classList.add(`drag-over`)}),t.addEventListener(`dragleave`,()=>{t.classList.remove(`drag-over`)}),t.addEventListener(`drop`,e=>{e.stopPropagation(),e.preventDefault();let n=parseInt(t.dataset.index);if(P.draggedItemIndex!==null&&P.draggedItemIndex!==n){let e=P.customPrompts[P.draggedItemIndex];P.customPrompts.splice(P.draggedItemIndex,1),P.customPrompts.splice(n,0,e),chrome.storage.local.set({customPrompts:P.customPrompts}),X()}t.classList.remove(`drag-over`)})})}function Ft(){let e=document.querySelector(`.input-toolbar-right`);if(!e)return;let t=document.createElement(`button`);t.className=`prompt-manage-btn`,t.title=`提示词管理`,t.innerHTML=`<svg t="1781177976746" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="5076" width="16" height="16"><path d="M674.56 231.552l101.568 56.96-56.896-101.632 56.96-101.568-101.632 56.896-101.632-56.896 56.96 101.568-56.896 101.632 101.568-56.96zM186.944 629.76l-101.504-56.896 56.832 101.632-56.832 101.568 101.504-56.96 101.632 56.96-56.896-101.568 56.896-101.568-101.568 56.832zM85.44 85.312l56.832 101.568-56.832 101.632 101.504-56.96 101.632 56.96L231.68 186.88l56.896-101.568-101.568 56.896-101.568-56.896z m351.872 438.016l-99.2-99.136L424.32 337.984l99.072 99.264-86.08 86.144m-41.856-223.04L300.352 395.392a40.448 40.448 0 0 0 0 57.28l474.24 474.112a40.448 40.448 0 0 0 57.344 0l94.912-95.04a40.448 40.448 0 0 0 0-57.344L452.736 300.288a40.448 40.448 0 0 0-57.28 0z" p-id="5077" fill="#777"></path></svg>`,t.addEventListener(`click`,()=>{Ut()}),e.appendChild(t)}function It(e=``){let t=document.getElementById(`promptSelector`),n=document.getElementById(`promptDropdown`);t.style.display=`block`,n.classList.add(`show`),zt(e)}function Y(){let e=document.getElementById(`promptSelector`),t=document.getElementById(`promptDropdown`);e.style.display=`none`,t.classList.remove(`show`),P.selectedPromptIndex=-1}function Lt(){let e=document.getElementById(`promptSelector`),t=document.getElementById(`promptDropdown`),n=document.getElementById(`userInput`);e.style.display!==`none`&&t.classList.contains(`show`)?Y():(It(),n.focus())}function Rt(e=``){zt(e)}function zt(e=``){let t=document.getElementById(`promptList`),n=e.toLowerCase(),r=P.customPrompts.filter(t=>e?t.code.toLowerCase().includes(n)||t.content.toLowerCase().includes(n):!0);if(r.length===0){t.innerHTML=`<div class="prompt-empty">暂无匹配的提示词</div>`,P.selectedPromptIndex=-1;return}P.selectedPromptIndex=0,t.innerHTML=r.map((e,t)=>`
  ...
  `).join(``),t.querySelectorAll(`.prompt-item`).forEach(e=>{e.addEventListener(`click`,t=>{let n=e.dataset.code;t.ctrlKey||t.metaKey?Vt(n):Ht(n)})})}function Bt(e){e.forEach((e,t)=>{t===P.selectedPromptIndex?e.classList.add(`selected`):e.classList.remove(`selected`)})}function Vt(e){let t=P.customPrompts.find(t=>t.code===e);if(!t)return;let n=document.getElementById(`userInput`),r=n.value,i=r.lastIndexOf(`/`),a=r;if(i!==-1){let e=-1;for(let t=i-1;t>=0;t--)if(r[t]===`
`){e=t;break}a=e===-1?r.substring(0,i).trimEnd():r.substring(0,e+1).trimEnd()}n.value=a+(a&&!a.endsWith(`
`)?`

`:``)+t.content,n.focus(),n.selectionStart=n.selectionEnd=n.value.length,Y(),oe(),console.log(`[SidePanel] 已追加提示词到输入框:`,t.code,t.content)}async function Ht(e){let t=P.customPrompts.find(t=>t.code===e);if(!t)return;if(P.isGenerating){console.log(`[SidePanel] 正在生成中，请稍候...`);return}Y();let n=document.getElementById(`chatContainer`).querySelector(`.welcome-message`);n&&n.remove();let r=t.content,i=P.enableSelectionQuery&&P.selectedContextText&&P.selectedContextText.trim(),a=P.quotedContextText&&P.quotedContextText.trim();if(a){let e=P.quotedContextText.trim();r=`[引用内容]\n${e}\n\n[用户问题]\n${t.content}`,K(`quoted`,e,!1),P.quotedContextText=``}else if(i){let e=P.selectedContextText.trim();r=`[选中内容]\n${e}\n\n[用户问题]\n${t.content}`,K(`selected`,e,!1),P.selectedContextText=``}(i||a)&&Nt(),q(`user`,t.content),P.messageHistory.push({role:`user`,content:r}),W(),me(t.content);let o=document.getElementById(`userInput`);o.value=``,o.style.height=`auto`,P.isGenerating=!0;let s=document.getElementById(`sendBtn`);s.disabled=!0;let c=wt(),l=P.currentModel;try{await de(),console.log(`[SidePanel] 发送消息调试信息:`),console.log(`  - isolateChat:`,P.isolateChat),console.log(`  - chatConfig:`,P.chatConfig),console.log(`  - messageHistory.length:`,P.messageHistory.length);let e=[{role:`system`,content:ce()}];if(P.isolateChat){let t=P.messageHistory;P.chatConfig.maxMemoryMessages!==null&&P.chatConfig.maxMemoryMessages!==void 0&&P.chatConfig.maxMemoryMessages>0?(t=[...P.messageHistory.slice(0,-1).slice(-P.chatConfig.maxMemoryMessages),P.messageHistory[P.messageHistory.length-1]],console.log(`[SidePanel] 记忆历史限制生效:`,P.chatConfig.maxMemoryMessages,`条（不含当前消息），实际发送:`,t.length,`条`)):console.log(`[SidePanel] 记忆历史限制未生效:`,P.chatConfig.maxMemoryMessages),e=[...e,...t]}else e.push({role:`user`,content:r});let t=await le(),n,i;try{let r=await Tt(e,l,P.useTools,t);n=r.content,i=r.executionLog||[]}catch(e){throw J(c),n=`❌ 请求失败：`+(e.message||`未知错误`),i=e.executionLog||[],q(`assistant`,n,!0,i),P.messageHistory.push({role:`assistant`,content:n,executionLog:i}),W(),e}J(c),await He(q(`assistant`,n,!0,i)),P.messageHistory.push({role:`assistant`,content:n,executionLog:i}),W()}catch{}finally{P.isGenerating=!1,s.disabled=!1,o.focus()}}function Ut(){document.getElementById(`promptManageModal`).classList.add(`show`),X()}function Wt(){document.getElementById(`promptManageModal`).classList.remove(`show`);let e=document.getElementById(`editPromptIndex`),t=document.getElementById(`newPromptCode`),n=document.getElementById(`newPromptContent`),r=document.getElementById(`promptManageAddBtn`);e&&(e.value=``),t&&(t.value=``),n&&(n.value=``),r&&(r.textContent=`添加提示词`,r.style.background=`#667eea`)}function X(){let e=document.getElementById(`promptManageList`);if(P.customPrompts.length===0){e.innerHTML=`<div class="prompt-empty">暂无提示词，请添加</div>`;return}e.innerHTML=P.customPrompts.map((e,t)=>`
  ...
  `).join(``),e.querySelectorAll(`.move-up-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.index);if(n>0){let e=P.customPrompts[n];P.customPrompts[n]=P.customPrompts[n-1],P.customPrompts[n-1]=e,chrome.storage.local.set({customPrompts:P.customPrompts}),X()}})}),e.querySelectorAll(`.move-down-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.index);if(n<P.customPrompts.length-1){let e=P.customPrompts[n];P.customPrompts[n]=P.customPrompts[n+1],P.customPrompts[n+1]=e,chrome.storage.local.set({customPrompts:P.customPrompts}),X()}})}),e.querySelectorAll(`.menu-toggle-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=parseInt(e.dataset.index);P.customPrompts[n].enabledInMenu=!P.customPrompts[n].enabledInMenu,chrome.storage.local.set({customPrompts:P.customPrompts}),X()})}),e.querySelectorAll(`.edit-btn`).forEach(e=>{e.addEventListener(`click`,()=>{Jt(parseInt(e.dataset.index))})}),e.querySelectorAll(`.delete-btn`).forEach(e=>{e.addEventListener(`click`,()=>{Yt(parseInt(e.dataset.index))})}),Pt()}function Gt(e){let t=document.getElementById(`promptErrorModal`),n=document.getElementById(`promptErrorMessage`);n.textContent=e,t.classList.add(`show`)}function Kt(){document.getElementById(`promptErrorModal`).classList.remove(`show`)}function qt(){let e=document.getElementById(`editPromptIndex`),t=document.getElementById(`newPromptCode`),n=document.getElementById(`newPromptContent`),r=t.value.trim(),i=n.value.trim();if(!r||!i){Gt(`请填写编码和内容`);return}let a=e?parseInt(e.value):-1,o=P.customPrompts.findIndex(e=>e.code===r);if(o!==-1&&o!==a){Gt(`编码已存在`);return}a>=0&&a<P.customPrompts.length?P.customPrompts[a]={...P.customPrompts[a],code:r,content:i}:P.customPrompts.push({code:r,content:i,enabledInMenu:!1}),chrome.storage.local.set({customPrompts:P.customPrompts}),t.value=``,n.value=``,e&&(e.value=``);let s=document.getElementById(`promptManageAddBtn`);s.textContent=`添加提示词`,s.style.background=`#667eea`,X()}function Jt(e){let t=P.customPrompts[e];if(!t)return;let n=document.getElementById(`editPromptIndex`),r=document.getElementById(`newPromptCode`),i=document.getElementById(`newPromptContent`),a=document.getElementById(`promptManageAddBtn`);n&&(n.value=e),r.value=t.code,i.value=t.content,a.textContent=`保存修改`,a.style.background=`#ffa502`,r.scrollIntoView({behavior:`smooth`})}function Yt(e){let t=P.customPrompts[e];if(!t)return;P.pendingDeleteIndex=e;let n=document.getElementById(`deleteConfirmModal`),r=document.getElementById(`deleteConfirmMessage`);r.textContent=`确定要删除提示词 "/${t.code}" 吗？`,n.classList.add(`show`)}function Xt(){document.getElementById(`deleteConfirmModal`).classList.remove(`show`),P.pendingDeleteIndex=-1}function Zt(e){P.customPrompts.splice(e,1),chrome.storage.local.set({customPrompts:P.customPrompts}),X()}function Qt(){let e=document.getElementById(`promptManageCancelBtn`),t=document.getElementById(`promptManageAddBtn`),n=document.getElementById(`promptModalCloseBtn`);e&&e.addEventListener(`click`,Wt),t&&t.addEventListener(`click`,qt),n&&n.addEventListener(`click`,Wt);let r=document.getElementById(`deleteCancelBtn`),i=document.getElementById(`deleteConfirmBtn`);r&&r.addEventListener(`click`,Xt),i&&i.addEventListener(`click`,()=>{P.pendingDeleteIndex>=0&&Zt(P.pendingDeleteIndex),Xt()});let a=document.getElementById(`deleteConfirmModal`);a&&a.addEventListener(`click`,e=>{e.target===a&&Xt()});let o=document.getElementById(`promptErrorConfirmBtn`);o&&o.addEventListener(`click`,Kt);let s=document.getElementById(`promptErrorModal`);s&&s.addEventListener(`click`,e=>{e.target===s&&Kt()})}function $t(){let e=document.getElementById(`toolsPopupOverlay`);if(!e)return;P.currentCategory=`all`,P.currentSearch=``;let t=document.getElementById(`toolsSearchInput`);t&&(t.value=``),Z(),Q(),document.querySelectorAll(`.category-btn`).forEach(e=>{e.classList.remove(`active`),e.classList.contains(`category-all`)?(e.classList.add(`active`),e.style.background=`linear-gradient(135deg, #667eea 0%, #764ba2 100%)`,e.style.color=`white`,e.style.borderColor=`transparent`):(e.style.background=`white`,e.style.color=`#555`,e.style.borderColor=`#ececec`)}),tn(),e.classList.add(`show`),console.log(`[SidePanel] 打开工具弹窗`)}function en(){let e=document.getElementById(`toolsPopupOverlay`);e&&(document.querySelectorAll(`.category-btn`).forEach(e=>{e.classList.remove(`active`),e.style.background=``,e.style.color=``,e.style.borderColor=``}),e.classList.remove(`show`),console.log(`[SidePanel] 关闭工具弹窗`))}function tn(){let t=document.getElementById(`toolsPopupList`);if(!t)return;t.innerHTML=``;let n={};e.forEach(e=>{if(P.currentCategory!==`all`&&e.category!==P.currentCategory)return;if(P.currentSearch){let t=e.name.toLowerCase().includes(P.currentSearch),n=e.description.toLowerCase().includes(P.currentSearch);if(!t&&!n)return}let t=e.category||`other`;n[t]||(n[t]=[]),n[t].push(e)});let r={page_interaction:`🖱️ 页面交互`,form_operation:`📝 表单操作`,info_extract:`📄 信息提取`,page_analysis:`🔍 页面分析`,tab_management:`📑 标签页管理`,bookmark_history:`🔖 书签历史`,storage_management:`💾 存储管理`,network_request:`🌐 网络请求`,media_process:`📷 媒体处理`,debug_dev:`🔧 调试开发`,ai_collaboration:`🤖 AI协作`,system_integration:`⚙️ 系统集成`,memory:`🧠 记忆检索`};if([`page_interaction`,`form_operation`,`info_extract`,`page_analysis`,`tab_management`,`bookmark_history`,`storage_management`,`network_request`,`media_process`,`debug_dev`,`ai_collaboration`,`system_integration`,`memory`].forEach(i=>{let a=n[i];if(!a||a.length===0)return;let o=e.filter(e=>e.category===i),s=o.length,c=o.filter(e=>P.enabledTools.includes(e.id)).length,l=document.createElement(`div`);l.className=`popup-tool-category-group`,l.dataset.category=i;let u=document.createElement(`div`);u.className=`popup-tool-category`,u.dataset.category=i;let d=P.collapsedCategories[i]||!1;u.innerHTML=`
      <span class="category-expand-icon">${d?`▶`:`▼`}</span>
      <span class="category-name">${r[i]||i}</span>
      <span class="category-count">${c}/${s}</span>
    `,u.addEventListener(`click`,()=>{nn(i)}),l.appendChild(u);let f=document.createElement(`div`);f.className=`popup-tool-items ${d?`collapsed`:``}`,a.forEach(e=>{let t=P.enabledTools.includes(e.id),n=document.createElement(`div`);n.className=`popup-tool-item`,n.dataset.category=i,n.innerHTML=`
      ...
      `;let r=n.querySelector(`input[type="checkbox"]`);r&&r.addEventListener(`change`,t=>{if(t.stopPropagation(),t.target.checked)P.enabledTools.includes(e.id)||P.enabledTools.push(e.id);else{let t=P.enabledTools.indexOf(e.id);t>-1&&P.enabledTools.splice(t,1)}rn(i),Z(),Q()}),f.appendChild(n)}),l.appendChild(f),t.appendChild(l)}),t.children.length===0){let e=document.createElement(`div`);e.className=`popup-tool-empty`,e.textContent=`没有找到匹配的工具`,t.appendChild(e)}}function nn(e){P.collapsedCategories[e]=!P.collapsedCategories[e];let t=document.querySelector(`.popup-tool-category-group[data-category="${e}"]`);if(!t)return;let n=t.querySelector(`.popup-tool-category`).querySelector(`.category-expand-icon`),r=t.querySelector(`.popup-tool-items`);P.collapsedCategories[e]?(n.textContent=`▶`,r.classList.add(`collapsed`)):(n.textContent=`▼`,r.classList.remove(`collapsed`))}function rn(t){let n=document.querySelector(`.popup-tool-category[data-category="${t}"]`);if(!n)return;let r=n.querySelector(`.category-count`);if(!r)return;let i=e.filter(e=>e.category===t),a=i.length,o=0;i.forEach(e=>{let t=document.getElementById(`tool_`+e.id);t&&t.checked&&o++}),r.textContent=`${o}/${a}`}function an(){return e.filter(e=>{if(P.currentCategory!==`all`&&e.category!==P.currentCategory)return!1;if(P.currentSearch){let t=e.name.toLowerCase().includes(P.currentSearch.toLowerCase()),n=e.description.toLowerCase().includes(P.currentSearch.toLowerCase());if(!t&&!n)return!1}return!0})}function on(){[`page_interaction`,`form_operation`,`info_extract`,`page_analysis`,`tab_management`,`bookmark_history`,`storage_management`,`network_request`,`media_process`,`debug_dev`,`ai_collaboration`,`system_integration`,`memory`].forEach(e=>{rn(e)})}function Z(){[`all`,`page_interaction`,`form_operation`,`info_extract`,`page_analysis`,`tab_management`,`bookmark_history`,`storage_management`,`network_request`,`media_process`,`debug_dev`,`ai_collaboration`,`system_integration`,`memory`].forEach(t=>{let n=document.getElementById(`badge-`+t);if(!n)return;let r=0,i=0;if(t===`all`)r=e.length,i=P.enabledTools.length;else{let n=e.filter(e=>e.category===t);r=n.length,i=n.filter(e=>P.enabledTools.includes(e.id)).length}n.textContent=`${i}/${r}`})}function Q(){let t=document.getElementById(`toolsEnabledCount`);if(!t)return;let n=e.length;t.textContent=`(已启用 ${P.enabledTools.length}/${n})`}function sn(){let t=[];e.forEach(e=>{let n=document.getElementById(`tool_`+e.id);n?n.checked&&t.push(e.id):P.enabledTools.includes(e.id)&&t.push(e.id)}),P.enabledTools=t,P.useTools=P.enabledTools.length>0,chrome.storage.local.set({enabledTools:P.enabledTools},()=>{console.log(`[SidePanel] 工具配置已保存:`,P.enabledTools)}),cn(),I(P.useTools?`已启用 ${P.enabledTools.length} 个工具`:`工具已全部禁用`,`success`)}function cn(){let e=document.getElementById(`toolsToggleBtn`),t=document.getElementById(`toolsBadge`);e&&(P.useTools&&P.enabledTools.length>0?(e.classList.add(`active`),e.title=`工具 (${P.enabledTools.length}个启用)`):(e.classList.remove(`active`),e.title=`工具 (未启用)`)),t&&(P.enabledTools.length>0?(t.textContent=P.enabledTools.length,t.style.display=`inline`):t.style.display=`none`)}chrome.storage.onChanged.addListener((e,t)=>{t===`local`&&e.chatMaxMemoryMessages&&(P.chatConfig.maxMemoryMessages=e.chatMaxMemoryMessages.newValue,$(),console.log(`[SidePanel] 记忆限制配置已更新:`,P.chatConfig.maxMemoryMessages))});function $(){let e=document.getElementById(`memoryLimitLabel`);e&&(P.chatConfig.maxMemoryMessages!==null&&P.chatConfig.maxMemoryMessages!==void 0&&P.chatConfig.maxMemoryMessages>0?e.textContent=`(${P.chatConfig.maxMemoryMessages})`:e.textContent=`(全)`,e.style.display=`inline`,e.style.cursor=`pointer`,e.title=`点击设置记忆历史限制条数`)}function ln(e){e.preventDefault(),e.stopPropagation();let t=document.getElementById(`memoryLimitDropdown`);if(t.classList.toggle(`show`),t.classList.contains(`show`)){let e=P.chatConfig.maxMemoryMessages;t.querySelectorAll(`.memory-limit-option`).forEach(t=>{t.classList.remove(`selected`);let n=parseInt(t.dataset.value);(e===null&&n===0||e!==null&&e>0&&n===e)&&t.classList.add(`selected`)});let n=t.querySelector(`#memoryLimitInput`);e!==null&&e>0?n.value=e:n.value=``}}function un(){let e=document.getElementById(`memoryLimitDropdown`),t=document.getElementById(`memoryLimitLabel`),n=e?.querySelector(`#memoryLimitInput`);if(!e||!t)return;$(),t.addEventListener(`click`,ln);let r=e.querySelectorAll(`.memory-limit-option`);r.forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let i=parseInt(e.dataset.value),a=i===0?null:i;r.forEach(e=>e.classList.remove(`selected`)),e.classList.add(`selected`),n&&(n.value=i===0?``:i),chrome.storage.local.set({chatMaxMemoryMessages:a},()=>{P.chatConfig.maxMemoryMessages=a,$(),I(`✅ 配置已更新`,`success`)})})}),n&&(n.addEventListener(`click`,e=>e.stopPropagation()),n.addEventListener(`mousedown`,e=>e.stopPropagation()),n.addEventListener(`change`,e=>{e.stopPropagation();let t=e.target.value.trim(),n=t&&parseInt(t)>0?parseInt(t):null;r.forEach(e=>e.classList.remove(`selected`)),chrome.storage.local.set({chatMaxMemoryMessages:n},()=>{P.chatConfig.maxMemoryMessages=n,$(),I(`✅ 配置已更新`,`success`)})})),document.addEventListener(`click`,n=>{!e.contains(n.target)&&n.target!==t&&e.classList.remove(`show`)})}function dn(e){document.querySelectorAll(`.model-option`).forEach(t=>{t.dataset.value===e?(t.classList.add(`selected`),t.querySelector(`.model-option-check`).textContent=`✓`):(t.classList.remove(`selected`),t.querySelector(`.model-option-check`).textContent=``)})}function fn(e,t){let n=document.getElementById(`tempDropdown`);if(!n||!e){typeof t==`function`&&t();return}let r=[`deepseek-v4-pro`,`deepseek-v4-flash`];e.forEach(e=>{if(r.includes(e)||n.querySelector(`.model-option[data-value="${e}"]`))return;let t=document.createElement(`div`);t.className=`model-option`,t.dataset.value=e,t.innerHTML=`<span class="model-option-check"></span>${e}`,t.addEventListener(`click`,t=>{t.stopPropagation(),P.currentModel=e,dn(e),chrome.storage.local.set({modelName:e})}),n.querySelector(`.model-section`).appendChild(t)}),typeof t==`function`&&t()}function pn(e,t=`📌 已选中`){if(!P.enableSelectionQuery)return;P.quotedContextText=``,P.selectedContextText=e;let n=document.getElementById(`selectionIndicator`),r=document.getElementById(`selectionText`),i=document.getElementById(`userInput`);if(n&&r&&i){let i;i=e.length>100?e.substring(0,100)+`...`:e.length>50?e.substring(0,50)+`...`:e,r.textContent=`${t}: ${i}`,n.classList.add(`show`)}}function mn(e,t,n=0,r=0){if(!P.enableSelectionQuery)return;let i=document.getElementById(`selectionFloatingMenu`),a=document.getElementById(`selectionMenuItems`);if(!i||!a)return;let o=P.customPrompts.filter(e=>e.enabledInMenu===!0);if(o.length===0)return;a.innerHTML=``,o.forEach(e=>{let n=e.content.length>10?e.content.substring(0,10)+`...`:e.content,r=document.createElement(`div`);r.className=`menu-item`,r.innerHTML=`
      <span>${n}</span>
      <span class="menu-item-code">/${e.code}</span>
    `,r.addEventListener(`click`,n=>{n.stopPropagation(),hn(e,t)}),a.appendChild(r)});let s=document.body.getBoundingClientRect(),c=40+o.length*36,l=r-s.top-c-10,u=n-s.left-20;l<s.top+10&&(l=r-s.top+15),u<s.left+10&&(u=n-s.left+20),u+180>s.right-10&&(u=n-s.left-180-10,u<s.left+10&&(u=n-s.left+20)),l+c>s.bottom-10&&(l=r-s.top-c-10,l<s.top+10&&(l=r-s.top+15)),i.style.top=l+`px`,i.style.left=u+`px`,i.style.maxHeight=s.bottom-l-20+`px`,i.classList.add(`show`)}window.hideFloatingMenu=function(){let e=document.getElementById(`selectionFloatingMenu`);e&&e.classList.remove(`show`),P.lastSelectedText=``,P.currentSelectionRange=null};async function hn(e,t){if(!P.enableSelectionQuery)return;if(window.hideFloatingMenu(),P.isGenerating){console.log(`[SidePanel] 正在生成中，请稍候...`);return}P.selectedContextText=t,U();let n=document.getElementById(`chatContainer`).querySelector(`.welcome-message`);n&&n.remove(),K(`selected`,t,!1);let r=`[选中内容]\n${t}\n\n[用户问题]\n${e.content}`;q(`user`,e.content),P.messageHistory.push({role:`user`,content:r}),W(),me(e.content),P.isGenerating=!0;let i=document.getElementById(`sendBtn`);i.disabled=!0;let a=wt(),o=P.currentModel;try{await de(),console.log(`[SidePanel] 发送消息调试信息:`),console.log(`  - isolateChat:`,P.isolateChat),console.log(`  - chatConfig:`,P.chatConfig),console.log(`  - messageHistory.length:`,P.messageHistory.length);let e=[{role:`system`,content:ce()}];if(P.isolateChat){let t=P.messageHistory;P.chatConfig.maxMemoryMessages!==null&&P.chatConfig.maxMemoryMessages!==void 0&&P.chatConfig.maxMemoryMessages>0?(t=[...P.messageHistory.slice(0,-1).slice(-P.chatConfig.maxMemoryMessages),P.messageHistory[P.messageHistory.length-1]],console.log(`[SidePanel] 记忆历史限制生效:`,P.chatConfig.maxMemoryMessages,`条（不含当前消息），实际发送:`,t.length,`条`)):console.log(`[SidePanel] 记忆历史限制未生效:`,P.chatConfig.maxMemoryMessages),e=[...e,...t]}else e.push({role:`user`,content:r});let t=await le(),n,i;try{let r=await Tt(e,o,P.useTools,t);n=r.content,i=r.executionLog||[]}catch(e){throw J(a),n=`❌ 请求失败：`+(e.message||`未知错误`),i=e.executionLog||[],q(`assistant`,n,!0,i),P.messageHistory.push({role:`assistant`,content:n,executionLog:i}),W(),e}J(a),await He(q(`assistant`,n,!0,i)),P.messageHistory.push({role:`assistant`,content:n,executionLog:i}),W()}catch{}finally{P.isGenerating=!1,i.disabled=!1,document.getElementById(`userInput`).focus()}}document.addEventListener(`DOMContentLoaded`,async()=>{window.__tableBlocks=[],await fe(),chrome.runtime.onMessage.addListener(e=>{e.type===`SELECTION_AI_SEARCH`&&e.prompt&&(console.log(`[SidePanel] 收到选中文本 AI 搜索:`,e.selectedText?.substring(0,50)),dt(e.prompt,e.selectedText),chrome.storage.session.remove(`pendingSelectionSearch`).catch(()=>{})),e.type===`FILL_SIDEPANEL_INPUT`&&e.text&&(console.log(`[SidePanel] 收到追问填充:`,e.text?.substring(0,50)),ft(e.text),chrome.storage.session.remove(`pendingFillInput`).catch(()=>{})),e.type===`DIRECT_SEND`&&e.text&&(console.log(`[SidePanel] 收到直接发送:`,e.text?.substring(0,50)),pt(e.text,e.selectedText||``),chrome.storage.session.remove(`pendingDirectSend`).catch(()=>{}))});let t=await chrome.storage.session.get(`pendingSelectionSearch`);if(t.pendingSelectionSearch&&t.pendingSelectionSearch.selectedText){let{prompt:e,selectedText:n}=t.pendingSelectionSearch;console.log(`[SidePanel] 有待处理的选中文本搜索:`,n?.substring(0,50)),setTimeout(()=>{dt(e,n)},500),await chrome.storage.session.remove(`pendingSelectionSearch`)}let n=await chrome.storage.session.get(`pendingFillInput`);if(n.pendingFillInput&&n.pendingFillInput.text){let{text:e}=n.pendingFillInput;console.log(`[SidePanel] 有待填充的追问文本:`,e?.substring(0,50)),setTimeout(()=>{ft(e)},500),await chrome.storage.session.remove(`pendingFillInput`)}let r=await chrome.storage.session.get(`pendingDirectSend`);if(r.pendingDirectSend&&r.pendingDirectSend.text){let{text:e,selectedText:t}=r.pendingDirectSend;console.log(`[SidePanel] 有待直接发送的文本:`,e?.substring(0,50)),setTimeout(()=>{pt(e,t||``)},500),await chrome.storage.session.remove(`pendingDirectSend`)}chrome.tabs.onActivated.addListener(async e=>{console.log(`[SidePanel] Tab 切换, 新 Tab ID:`,e.tabId),P.currentTabId=e.tabId}),chrome.tabs.onUpdated.addListener(async(e,t)=>{t.status===`complete`&&P.currentTabId===e&&console.log(`[SidePanel] 当前 Tab 页面更新:`,t)}),typeof marked<`u`&&(marked.setOptions({breaks:!0,gfm:!0}),console.log(`[SidePanel] Marked 库已加载`)),typeof mermaid<`u`&&(mermaid.initialize({startOnLoad:!1,theme:`default`,securityLevel:`loose`,fontFamily:`-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif`}),console.log(`[SidePanel] Mermaid 库已加载`));let i=document.getElementById(`userInput`),a=document.getElementById(`sendBtn`),o=document.getElementById(`clearChatBtn`),s=document.getElementById(`exportChatBtn`),c=document.getElementById(`chatContainer`),l=document.getElementById(`tempDisplay`),u=document.getElementById(`tempDropdown`),d=document.getElementById(`tempPresetList`),f=document.getElementById(`tempSlider`),p=document.getElementById(`tempNumberInput`);function m(){!i||P.isScrolling||(i.style.height=`auto`,i.style.height=Math.min(i.scrollHeight,100)+`px`)}chrome.storage.local.get([`temperature`,`topP`,`selectedTempIndex`],e=>{e.temperature!==void 0&&(P.temperature=e.temperature),e.topP!==void 0&&(P.topP=e.topP),e.selectedTempIndex!==void 0&&(P.selectedTempIndex=e.selectedTempIndex),h()});function h(){f&&(f.value=P.temperature),p&&(p.value=P.temperature.toFixed(2));let e=document.getElementById(`tempIconValue`);e&&(e.textContent=P.temperature.toFixed(2)),g()}function g(){d.innerHTML=F.map((e,t)=>`
    ...
    `).join(``),d.querySelectorAll(`.temp-preset-item`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation(),_(parseInt(e.dataset.index))})})}function _(e){let t=F[e];t&&(P.selectedTempIndex=e,P.temperature=t.temp,h(),chrome.storage.local.set({temperature:P.temperature,topP:P.topP,selectedTempIndex:P.selectedTempIndex}))}f.addEventListener(`input`,e=>{e.stopPropagation();let t=parseFloat(e.target.value);isNaN(t)&&(t=0),t=Math.max(0,Math.min(1,t)),P.temperature=t,p.value=t.toFixed(2);let n=document.getElementById(`tempIconValue`);n&&(n.textContent=t.toFixed(2));let r=0,i=Math.abs(F[0].temp-t);for(let e=1;e<F.length;e++){let n=Math.abs(F[e].temp-t);n<i&&(i=n,r=e)}P.selectedTempIndex=r,g(),chrome.storage.local.set({temperature:P.temperature,topP:P.topP,selectedTempIndex:P.selectedTempIndex})}),p.addEventListener(`change`,e=>{e.stopPropagation();let t=parseFloat(e.target.value);isNaN(t)&&(t=0),t=Math.max(0,Math.min(1,t)),P.temperature=t,f.value=t,p.value=t.toFixed(2);let n=0,r=Math.abs(F[0].temp-t);for(let e=1;e<F.length;e++){let i=Math.abs(F[e].temp-t);i<r&&(r=i,n=e)}P.selectedTempIndex=n,g(),chrome.storage.local.set({temperature:P.temperature,topP:P.topP,selectedTempIndex:P.selectedTempIndex})}),l&&u&&(l.addEventListener(`click`,e=>{e.stopPropagation(),u.classList.toggle(`show`)}),document.addEventListener(`click`,e=>{let t=document.querySelector(`.temp-selector`);t&&!t.contains(e.target)&&u.classList.remove(`show`)})),c.addEventListener(`mousedown`,e=>{P.lastMouseX=e.clientX,P.lastMouseY=e.clientY}),c.addEventListener(`mouseup`,e=>{P.lastMouseX=e.clientX,P.lastMouseY=e.clientY,P.enableSelectionQuery&&setTimeout(()=>{let e=window.getSelection();if(e&&!e.isCollapsed&&e.toString().trim()){let t=e.toString().trim();c.contains(e.anchorNode)&&t!==P.lastSelectedText&&(P.lastSelectedText=t,P.currentSelectionRange=e.getRangeAt(0).cloneRange(),pn(t),mn(e,t,P.lastMouseX,P.lastMouseY))}else c.contains(e.anchorNode)||(P.lastSelectedText=``,P.currentSelectionRange=null,window.hideFloatingMenu())},10)});let v=``;setInterval(async()=>{try{if(!P.enableSelectionQuery)return;let e=await new Promise(e=>{chrome.tabs.query({active:!0,currentWindow:!0},t=>e(t))});if(e&&e.length>0){let t=await new Promise(t=>{chrome.tabs.sendMessage(e[0].id,{action:`getSelectedText`},e=>{chrome.runtime.lastError?(console.debug(`[SidePanel] content script 未加载或无法通信:`,chrome.runtime.lastError.message),t(null)):t(e)})});if(!t)return;let n=t?.text||``;n&&n.trim()?n!==v&&(v=n,pn(n.trim())):v=``}}catch{}},500),chrome.storage.local.get([`modelName`,`customModels`,`customPrompts`,`systemPrompt`,`inputHistory`],e=>{let t=e.modelName;t&&(P.currentModel=t),P.customPrompts=e.customPrompts||[],P.systemPrompt=e.systemPrompt||``,P.inputHistory=e.inputHistory||[],Ft(),fn(e.customModels,()=>{t&&dn(t)})}),chrome.storage.onChanged.addListener((e,t)=>{if(t===`local`){if(e.customModels){let t=e.customModels.newValue||[],n=u.querySelector(`.model-section`);n&&n.querySelectorAll(`.model-option`).forEach(e=>{let t=e.dataset.value;t!==`deepseek-v4-pro`&&t!==`deepseek-v4-flash`&&e.remove()}),fn(t)}if(e.modelName){let t=e.modelName.newValue;t&&(P.currentModel=t,dn(t))}}}),ot(),document.addEventListener(`session-switched`,()=>{let e=document.getElementById(`chatContainer`);if(e)if(e.innerHTML=``,!P.messageHistory||P.messageHistory.length===0){let t=document.createElement(`div`);t.className=`welcome-message`,t.innerHTML=`
        <div class="icon-wrapper">
          <div class="icon">💬</div>
        </div>
        <h2>开始对话</h2>
        <p>输入您的问题，AI 助手将为您解答</p>
      `,e.appendChild(t)}else P.messageHistory.forEach(e=>{q(e.role,e.content,!1,e.executionLog||[])}),Ie()}),document.querySelectorAll(`.model-option`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=e.dataset.value;P.currentModel=n,dn(n),chrome.storage.local.set({modelName:n})})}),document.addEventListener(`click`,e=>{let t=document.getElementById(`promptDropdown`),n=document.getElementById(`promptSelector`),r=document.getElementById(`selectionFloatingMenu`);if(n.contains(e.target)||(t.classList.remove(`show`),Y()),r&&!r.contains(e.target)){let t=window.getSelection(),n=c.contains(e.target),r=t&&!t.isCollapsed&&c.contains(t.anchorNode);(!n||!r)&&window.hideFloatingMenu()}}),a.addEventListener(`click`,G);let y=document.getElementById(`promptTriggerBtn`);y&&y.addEventListener(`click`,e=>{e.stopPropagation(),y.blur(),Lt()}),document.addEventListener(`keydown`,e=>{if((e.ctrlKey||e.metaKey)&&e.key===`t`){e.preventDefault();let t=document.getElementById(`toolsPopup`);t&&t.style.display!==`none`?en():$t()}}),i.addEventListener(`keydown`,e=>{let t=document.getElementById(`promptSelector`),n=document.getElementById(`promptDropdown`);if(t.style.display!==`none`&&n.classList.contains(`show`)){let t=n.querySelectorAll(`.prompt-item`),r=t.length;if(r!==0&&e.key===`ArrowDown`){e.preventDefault(),P.selectedPromptIndex<0?P.selectedPromptIndex=0:P.selectedPromptIndex=(P.selectedPromptIndex+1)%r,Bt(t);return}if(e.key===`ArrowUp`){e.preventDefault(),P.selectedPromptIndex<0||P.selectedPromptIndex===0?P.selectedPromptIndex=r-1:--P.selectedPromptIndex,Bt(t);return}if(e.key===`Enter`&&(e.ctrlKey||e.metaKey)&&P.selectedPromptIndex>=0){e.preventDefault();let n=t[P.selectedPromptIndex].dataset.code;Vt(n);return}if(e.key===`Enter`&&P.selectedPromptIndex>=0){e.preventDefault();let n=t[P.selectedPromptIndex].dataset.code;Ht(n);return}if(e.key===`Escape`){Y();return}}if(e.key===`Escape`){P.inputHistoryIndex>=0&&(P.inputHistoryIndex=-1),i.value&&(i.value=``,i.style.height=`auto`,i.dispatchEvent(new Event(`input`))),e.preventDefault();return}if(!(t.style.display!==`none`&&n.classList.contains(`show`))&&!P.isGenerating){if(e.key===`ArrowUp`){e.preventDefault(),P.inputHistoryIndex===-1?P.inputHistoryIndex=P.inputHistory.length-1:P.inputHistoryIndex>0&&P.inputHistoryIndex--,P.inputHistoryIndex<0&&(P.inputHistoryIndex=0),P.inputHistoryIndex>=0&&P.inputHistory.length>0&&(i.value=P.inputHistory[P.inputHistoryIndex],i.dispatchEvent(new Event(`input`)));return}if(e.key===`ArrowDown`){e.preventDefault(),P.inputHistoryIndex>=0&&P.inputHistoryIndex<P.inputHistory.length-1?(P.inputHistoryIndex++,i.value=P.inputHistory[P.inputHistoryIndex],i.dispatchEvent(new Event(`input`))):(P.inputHistoryIndex=-1,i.value=``,i.dispatchEvent(new Event(`input`)));return}}e.key===`Enter`&&!e.shiftKey&&(e.preventDefault(),G())}),i.addEventListener(`wheel`,e=>{P.isScrolling=!0;let t=i.style.height||i.offsetHeight+`px`;i.style.height=t,i.scrollHeight<=i.clientHeight+10&&(e.preventDefault(),e.stopPropagation()),setTimeout(()=>{P.isScrolling=!1},100)},{passive:!1}),i.addEventListener(`input`,e=>{let t=i.value;document.getElementById(`promptSelector`),document.getElementById(`promptDropdown`);let n=t.lastIndexOf(`/`);if(n!==-1){let e=t.substring(n+1);n===0||t[n-1]===`
`||t[n-1]===` `?It(e):Rt(e)}else Y();m()}),c.addEventListener(`scroll`,()=>{chrome.storage.local.set({scrollPosition:c.scrollTop})}),o.addEventListener(`click`,()=>{lt()}),s&&s.addEventListener(`click`,ct);let ee=document.getElementById(`settingsBtn`);ee&&ee.addEventListener(`click`,()=>{chrome.runtime.openOptionsPage()});let b=document.getElementById(`isolateChatBtn`),x=document.getElementById(`enableToolsBtn`),S=document.getElementById(`toolsConfigBtn`);chrome.storage.local.get([`isolateChat`,`enableSelectionQuery`,`enableTools`,`enabledTools`],t=>{t.isolateChat!==void 0&&(P.isolateChat=t.isolateChat),b.checked=P.isolateChat,t.enableSelectionQuery!==void 0&&(P.enableSelectionQuery=t.enableSelectionQuery);let n=document.getElementById(`enableSelectionQueryBtn`);n&&(n.checked=P.enableSelectionQuery),t.enableTools!==void 0&&(P.useTools=t.enableTools),t.enabledTools&&t.enabledTools.length>0?P.enabledTools=t.enabledTools:P.enabledTools=e.filter(e=>e.enabled).map(e=>e.id),P.enabledTools.length===0&&(P.useTools=!1),x&&(x.checked=P.useTools)}),b.addEventListener(`change`,()=>{P.isolateChat=b.checked,chrome.storage.local.set({isolateChat:P.isolateChat}),console.log(`[SidePanel] 记忆对话:`,P.isolateChat?`已启用`:`已禁用`)});let C=document.getElementById(`enableSelectionQueryBtn`);C&&C.addEventListener(`change`,()=>{P.enableSelectionQuery=C.checked,chrome.storage.local.set({enableSelectionQuery:P.enableSelectionQuery}),console.log(`[SidePanel] 划词问答:`,P.enableSelectionQuery?`已启用`:`已禁用`),!P.enableSelectionQuery&&P.selectedContextText&&U()}),x&&x.addEventListener(`change`,()=>{P.useTools=x.checked,chrome.storage.local.set({enableTools:P.useTools}),P.useTools&&P.enabledTools.length===0&&(P.enabledTools=e.filter(e=>e.enabled).map(e=>e.id),chrome.storage.local.set({enabledTools:P.enabledTools})),console.log(`[SidePanel] 工具总开关:`,P.useTools?`已启用`:`已禁用`)}),S&&S.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),$t()});let te=document.getElementById(`toolsPopupOverlay`),w=document.getElementById(`toolsPopupClose`),T=te?te.querySelector(`.modal-container`):null;w&&w.addEventListener(`click`,en),T&&T.addEventListener(`click`,e=>{e.stopPropagation()});let E=document.getElementById(`toolsSearchInput`);E&&E.addEventListener(`input`,e=>{P.currentSearch=e.target.value.toLowerCase(),tn()});let D=document.querySelectorAll(`.category-btn`);D.forEach(e=>{e.addEventListener(`click`,()=>{D.forEach(e=>{e.classList.remove(`active`),e.classList.contains(`category-all`)?(e.style.background=`#f5f3ff`,e.style.color=`#667eea`,e.style.borderColor=`#d4cfff`):(e.style.background=`white`,e.style.color=`#555`,e.style.borderColor=`#ececec`)}),e.classList.add(`active`),e.style.background=`linear-gradient(135deg, #667eea 0%, #764ba2 100%)`,e.style.color=`white`,e.style.borderColor=`transparent`,P.currentCategory=e.dataset.category,tn()})});let O=document.getElementById(`toolsCategories`);O&&O.addEventListener(`wheel`,e=>{e.preventDefault(),O.scrollLeft+=e.deltaY*2},{passive:!1});let k=document.getElementById(`toolsSelectAll`),A=document.getElementById(`toolsSelectNone`);k&&k.addEventListener(`click`,()=>{an().forEach(e=>{let t=document.getElementById(`tool_`+e.id);t&&(t.checked=!0),P.enabledTools.includes(e.id)||P.enabledTools.push(e.id)}),on(),Z(),Q()}),A&&A.addEventListener(`click`,()=>{an().forEach(e=>{let t=document.getElementById(`tool_`+e.id);t&&(t.checked=!1);let n=P.enabledTools.indexOf(e.id);n>-1&&P.enabledTools.splice(n,1)}),on(),Z(),Q()});let ne=document.getElementById(`toolsPopupSave`);ne&&ne.addEventListener(`click`,()=>{sn(),Q()});let j=document.getElementById(`toolsPopupCancel`);j&&j.addEventListener(`click`,()=>{en()});let re=document.getElementById(`modalCancelBtn`),ie=document.getElementById(`modalConfirmBtn`);re.addEventListener(`click`,()=>{ut()}),ie.addEventListener(`click`,()=>{ut(),st()});let M=document.getElementById(`confirmModal`);M.addEventListener(`click`,e=>{e.target===M&&ut()});let N=document.getElementById(`selectionClose`);N&&N.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),console.log(`[SidePanel] 用户点击关闭选中内容按钮`),U(),window.hideFloatingMenu(),P.lastSelectedText=``,P.currentSelectionRange=null}),i.addEventListener(`input`,()=>{})}),ue().then(()=>$()),document.addEventListener(`DOMContentLoaded`,()=>{un()}),document.addEventListener(`DOMContentLoaded`,ge),document.addEventListener(`DOMContentLoaded`,Qt),document.addEventListener(`DOMContentLoaded`,je);
//# sourceMappingURL=side_panel-DBJus8Zw.js.map