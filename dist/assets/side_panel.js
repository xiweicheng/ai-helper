...
`||t[n-1]===` `?$n(e):tr(e)}else X();m()}),c.addEventListener(`scroll`,()=>{let e=`scrollPosition_`+(P.activeSessionId||`default`);chrome.storage.local.set({[e]:c.scrollTop})});let C=document.getElementById(`headerMoreBtn`),w=document.getElementById(`headerMoreDropdown`);C&&w&&(C.addEventListener(`click`,e=>{e.stopPropagation(),w.classList.toggle(`show`)}),document.addEventListener(`click`,e=>{!w.contains(e.target)&&e.target!==C&&w.classList.remove(`show`)})),o.addEventListener(`click`,e=>{e.stopPropagation(),w.classList.remove(`show`),wn()}),s&&s.addEventListener(`click`,e=>{e.stopPropagation(),w.classList.remove(`show`),Cn()});let ee=document.getElementById(`settingsBtn`);ee&&ee.addEventListener(`click`,()=>{chrome.runtime.openOptionsPage()});let te=document.getElementById(`prototypeLibraryBtn`);te&&te.addEventListener(`click`,e=>{e.stopPropagation(),w.classList.remove(`show`),_t()});let T=document.getElementById(`isolateChatBtn`),E=document.getElementById(`enableToolsBtn`),ne=document.getElementById(`toolsConfigBtn`);chrome.storage.local.get([`isolateChat`,`enableSelectionQuery`,`enableTools`,`enabledTools`],t=>{t.isolateChat!==void 0&&(P.isolateChat=t.isolateChat),T.checked=P.isolateChat,t.enableSelectionQuery!==void 0&&(P.enableSelectionQuery=t.enableSelectionQuery);let n=document.getElementById(`enableSelectionQueryBtn`);if(n&&(n.checked=P.enableSelectionQuery),t.enableTools!==void 0&&(P.useTools=t.enableTools),t.enabledTools&&t.enabledTools.length>0){let n=new Set(e.map(e=>e.id)),r=t.enabledTools.filter(e=>n.has(e)),i=e.filter(e=>e.enabled&&!r.includes(e.id)).map(e=>e.id);P.enabledTools=[...r,...i],i.length>0&&chrome.storage.local.set({enabledTools:P.enabledTools})}else P.enabledTools=e.filter(e=>e.enabled).map(e=>e.id);P.enabledTools.length===0&&(P.useTools=!1),E&&(E.checked=P.useTools),x()}),T.addEventListener(`change`,()=>{P.isolateChat=T.checked,chrome.storage.local.set({isolateChat:P.isolateChat}),console.log(`[SidePanel] 记忆对话:`,P.isolateChat?`已启用`:`已禁用`)});let D=document.getElementById(`enableSelectionQueryBtn`);D&&D.addEventListener(`change`,()=>{P.enableSelectionQuery=D.checked,chrome.storage.local.set({enableSelectionQuery:P.enableSelectionQuery}),console.log(`[SidePanel] 划词问答:`,P.enableSelectionQuery?`已启用`:`已禁用`),!P.enableSelectionQuery&&P.selectedContextText&&bn()}),E&&E.addEventListener(`change`,()=>{P.useTools=E.checked,chrome.storage.local.set({enableTools:P.useTools}),P.useTools&&P.enabledTools.length===0&&(P.enabledTools=e.filter(e=>e.enabled).map(e=>e.id),chrome.storage.local.set({enabledTools:P.enabledTools})),console.log(`[SidePanel] 工具总开关:`,P.useTools?`已启用`:`已禁用`)}),ne&&ne.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),gr()});let re=document.getElementById(`toolsPopupOverlay`),ie=document.getElementById(`toolsPopupClose`),ae=re?re.querySelector(`.modal-container`):null;ie&&ie.addEventListener(`click`,_r),ae&&ae.addEventListener(`click`,e=>{e.stopPropagation()});let oe=document.getElementById(`toolsSearchInput`);oe&&oe.addEventListener(`input`,e=>{P.currentSearch=e.target.value.toLowerCase(),vr()});let se=document.querySelectorAll(`.category-btn`);se.forEach(e=>{e.addEventListener(`click`,()=>{se.forEach(e=>{e.classList.remove(`active`),e.classList.contains(`category-all`)?(e.style.background=`#f5f3ff`,e.style.color=`#667eea`,e.style.borderColor=`#d4cfff`):(e.style.background=`white`,e.style.color=`#555`,e.style.borderColor=`#ececec`)}),e.classList.add(`active`),e.style.background=`linear-gradient(135deg, #667eea 0%, #764ba2 100%)`,e.style.color=`white`,e.style.borderColor=`transparent`,P.currentCategory=e.dataset.category,vr()})});let O=document.getElementById(`toolsCategories`);O&&O.addEventListener(`wheel`,e=>{e.preventDefault(),O.scrollLeft+=e.deltaY*2},{passive:!1});let ce=document.getElementById(`toolsSelectAll`),le=document.getElementById(`toolsSelectNone`);ce&&ce.addEventListener(`click`,()=>{xr().forEach(e=>{let t=document.getElementById(`tool_`+e.id);t&&(t.checked=!0),P.enabledTools.includes(e.id)||P.enabledTools.push(e.id)}),Sr(),Cr(),Q()}),le&&le.addEventListener(`click`,()=>{xr().forEach(e=>{let t=document.getElementById(`tool_`+e.id);t&&(t.checked=!1);let n=P.enabledTools.indexOf(e.id);n>-1&&P.enabledTools.splice(n,1)}),Sr(),Cr(),Q()});let ue=document.getElementById(`toolsPopupSave`);ue&&ue.addEventListener(`click`,()=>{wr(),Q()});let k=document.getElementById(`toolsPreselectToggle`);k&&k.addEventListener(`change`,()=>{chrome.storage.local.set({enableToolPreselect:k.checked},()=>{console.log(`[SidePanel] 工具预筛选开关已更新:`,k.checked)})});let de=document.getElementById(`toolsPopupCancel`);de&&de.addEventListener(`click`,()=>{_r()});let fe=document.getElementById(`modalCancelBtn`),A=document.getElementById(`modalConfirmBtn`);function pe(e,t=`确认操作`){return new Promise(n=>{let r=document.getElementById(`customConfirmOverlay`),i=document.getElementById(`customConfirmTitle`),a=document.getElementById(`customConfirmMessage`),o=document.getElementById(`customConfirmCancelBtn`),s=document.getElementById(`customConfirmOkBtn`);if(!r||!i||!a||!o||!s){n(confirm(e));return}let c=()=>{r.style.display=`none`,s.removeEventListener(`click`,l),o.removeEventListener(`click`,u),r.removeEventListener(`click`,d)},l=()=>{c(),n(!0)},u=()=>{c(),n(!1)},d=e=>{e.target===r&&(c(),n(!1))};i.textContent=t,a.textContent=e,r.style.display=`flex`,s.addEventListener(`click`,l),o.addEventListener(`click`,u),r.addEventListener(`click`,d)})}let j=document.getElementById(`toolStatsOverlay`),me=document.getElementById(`toolStatsClose`),he=document.getElementById(`toolStatsBtn`);function ge(){j&&(j.style.display=`flex`,N())}function _e(){j&&(j.style.display=`none`)}he&&he.addEventListener(`click`,e=>{e.stopPropagation(),ge()}),me&&me.addEventListener(`click`,_e),j&&j.addEventListener(`click`,e=>{e.target===j&&_e()});let ve=document.getElementById(`toolStatsRefreshBtn`);ve&&ve.addEventListener(`click`,N);let ye=document.getElementById(`toolStatsClearBtn`);ye&&ye.addEventListener(`click`,async()=>{await pe(`确定要清空所有工具使用统计吗？此操作不可撤销。`,`清空统计`)&&(await chrome.storage.local.remove([`toolUsageStats`]),N())});let M={column:`callCount`,asc:!1};async function N(){let t=document.getElementById(`toolStatsTable`),n=document.getElementById(`toolStatsTableBody`),r=document.getElementById(`toolStatsLoading`),i=document.getElementById(`toolStatsEmpty`),a=document.getElementById(`toolStatsSummary`),o=document.getElementById(`toolStatsUnusedSection`),s=document.getElementById(`toolStatsUnusedList`);if(!(!t||!n||!r||!i)){t.style.display=`none`,i.style.display=`none`,o&&(o.style.display=`none`),a&&(a.textContent=``),r.style.display=``;try{let n=(await chrome.storage.local.get([`toolUsageStats`])).toolUsageStats||{},c=Object.entries(n);if(c.length===0){r.style.display=`none`,i.style.display=``;return}let l={};e.forEach(e=>{l[e.id]=e.name?`${e.name}：${e.description||``}`:e.description||e.id}),be(c,l);let u=e.map(e=>e.id),d=new Set(c.map(([e])=>e)),f=u.filter(e=>!d.has(e)),p=c.length,m=f.length;a&&(a.textContent=`已使用 ${p} 个，未使用 ${m} 个`),o&&s&&m>0&&(s.innerHTML=f.sort((e,t)=>e.toLowerCase().localeCompare(t.toLowerCase())).map(e=>`<code title="${L(l[e]||e)}" style="padding: 3px 10px; background: #f5f5f5; color: #aaa; border: 1px solid #eee; border-radius: 4px; font-size: 11px;">${e}</code>`).join(``),o.style.display=``),r.style.display=`none`,t.style.display=``}catch(e){console.error(`[SidePanel] 加载统计失败:`,e),r.style.display=`none`,i.textContent=`加载失败`,i.style.display=``}}}function be(e,t){let n=document.getElementById(`toolStatsTableBody`);if(!n)return;let{column:r,asc:i}=M;n.innerHTML=[...e].sort((e,t)=>{let[n,a]=e,[o,s]=t,c=a.callCount>0?a.successCount/a.callCount*100:0,l=s.callCount>0?s.successCount/s.callCount*100:0,u=a.callCount>0?a.totalDuration/a.callCount:0,d=s.callCount>0?s.totalDuration/s.callCount:0,f=0;switch(r){case`name`:f=n.toLowerCase().localeCompare(o.toLowerCase());break;case`callCount`:f=a.callCount-s.callCount;break;case`successCount`:f=a.successCount-s.successCount;break;case`successRate`:f=c-l;break;case`duration`:f=u-d;break}return i?f:-f}).map(([e,n])=>{let r=n.callCount>0?n.successCount/n.callCount*100:0,i=n.callCount>0?n.totalDuration/n.callCount:0,a=t[e]||e,o=`#38a169`;r<60?o=`#e53e3e`:r<85&&(o=`#d69e2e`);let s=i<1e3?`${Math.round(i)}ms`:`${(i/1e3).toFixed(1)}s`;return`<tr>
      ...
      </tr>`}).join(``),xe()}function xe(){let{column:e,asc:t}=M,n=[`name`,`callCount`,`successCount`,`successRate`,`duration`],r={name:`sortByName`,callCount:`sortByCallCount`,successCount:`sortBySuccessCount`,successRate:`sortBySuccessRate`,duration:`sortByDuration`};n.forEach(n=>{let i=document.getElementById(r[n]);if(!i)return;let a=i.querySelector(`.sort-indicator`);a&&(n===e?(a.textContent=t?`▲`:`▼`,a.style.color=`#667eea`):(a.textContent=``,a.style.color=``))})}document.querySelectorAll(`#toolStatsTable th[data-sort]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.sort;M.column===t?M.asc=!M.asc:(M.column=t,M.asc=!1),N()})}),fe.addEventListener(`click`,()=>{Tn()}),A.addEventListener(`click`,()=>{Tn(),Sn()});let Se=document.getElementById(`confirmModal`);Se.addEventListener(`click`,e=>{e.target===Se&&Tn()});let Ce=document.getElementById(`selectionClose`);Ce&&Ce.addEventListener(`click`,e=>{e.preventDefault(),e.stopPropagation(),console.log(`[SidePanel] 用户点击关闭选中内容按钮`),bn(),window.hideFloatingMenu(),P.lastSelectedText=``,P.currentSelectionRange=null}),i.addEventListener(`input`,()=>{})}),Fe().then(()=>$()),document.addEventListener(`DOMContentLoaded`,()=>{Dr()}),document.addEventListener(`DOMContentLoaded`,Ve),document.addEventListener(`DOMContentLoaded`,hr),document.addEventListener(`DOMContentLoaded`,nt),document.addEventListener(`DOMContentLoaded`,ot),document.addEventListener(`DOMContentLoaded`,At);
//# sourceMappingURL=side_panel-Cg3kx2TQ.js.map