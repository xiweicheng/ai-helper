...
- **新增** `manage_ai_agent`：
  ...
  - `required: ['action']`
- **保留** 其他所有工具（agent_read_file/agent_write_file/agent_list_dir/agent_delete_file/agent_download_file/agent_exec_command/agent_search_files/agent_search_content/agent_skill_load/agent_workflow_run）不变
...
- `CONFIRMATION_REQUIRED_TOOLS` 保持不变（manage_tab 整体 `requiresConfirmation: false`，不进入此集合）
...
#### 4.3 更新 `requestToolConfirmation` 中的 close_tab 特殊处理（L48）
...
## 涉及文件清单
1. `src/background/tools/tab-tools.js` - 合并 manage_tab + search_browser_data
2. `src/background/tools/agent-tools.js` - 合并 agent_trash + manage_ai_agent
3. `src/background/constants.js` - 新增 CONFIRMATION_ACTION_MAP
4. `src/background/tool-executor.js` - 新执行器 + TOOL_HANDLERS 更新 + 旧名迁移
5. `src/background/react-loop.js` - action 级确认逻辑 + TOOL_DISPLAY_NAMES 更新
